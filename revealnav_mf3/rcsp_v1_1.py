"""MF3ZL-RCSP v1.1 with a versioned zero-relative-delta correction.

The v1 model rejected an otherwise valid event whenever the frozen native and
runner-up embeddings were identical.  In v1.1 that case is represented as
"no relative semantic evidence": the normalized difference is the zero
vector, while the causal policy-side head remains active.  Instruction,
history, native, and runner embeddings themselves must still be finite and
non-zero.  The original ``revealnav_mf3.rcsp`` module is intentionally left
unchanged for the sealed v1 protocol.
"""
...
CHECKPOINT_SCHEMA = "revealnav-mf3zl-rcsp-checkpoint/1.1-zero-relative-delta"
...
class RelativeSemanticSwitchPolicy(nn.Module):
    """Rank-4 policy with an explicit zero relative-semantic state."""
    ...
    def forward(
        ...
        instruction_norm = torch.linalg.vector_norm(instruction, dim=1)
        history_norm = torch.linalg.vector_norm(history, dim=1)
        difference_norm = torch.linalg.vector_norm(difference, dim=1)
        # Absolute instruction/history embeddings are required evidence.  An
        # identical native/runner pair is different: it carries no semantic
        # direction, so retain the row and make only that term neutral.
        if torch.any(instruction_norm <= 1e-8) or torch.any(history_norm <= 1e-8):
            raise ValueError("RCSP absolute semantic input has zero norm")
        instruction_unit = instruction / instruction_norm[:, None]
        history_unit = history / history_norm[:, None]
        nonzero_difference = difference_norm > 1e-8
        difference_unit = torch.where(
            nonzero_difference[:, None],
            difference / torch.clamp(difference_norm[:, None], min=1e-8),
