...
def freeze():
    ...
    pid=int(subprocess.check_output(['tmux','display-message','-p','-t','q35n_action_aligned_history_v10','#{pane_pid}'],text=True).strip())
    owner=identity(pid)
    assert owner['argv']==[str(PY),'-I','-S','-B',str(TRAIN/'lease_run.py'),str(TRAIN/'RUNBOOK.json')]
    assert owner['cwd']==str(ROOT)
    assert sha(ANALYZER)=='75533b85ac5350ec8323ecfe19162834dc0430274e4ab555f92652860cb7227e'
...
def close_report():
    ...
    result=dict(status='COMPLETE',unix=time.time(),before=before,after=after,paired=paired,
      positive_development_signal=positive,scientific_gain_verified=False,controller_enabled=False,frame_stride=8,
    ...
    assert after['result']['selected_history_audited_actions']==after['result']['audited_actions']
    report='# 普通导航：两帧视觉与八步动作对齐\n\n同100条、5屋INTERNAL_DEV；batch1、纯模型、500动作。不是独立论文确认。\n\n'
    report+='| 模型 | SR | SPL | nDTW | OSR |\n|---|---:|---:|---:|---:|\n'
    for label,case in [('当前最佳4000步',before),('最佳4000步＋两帧动作对齐1000步',after)]:
