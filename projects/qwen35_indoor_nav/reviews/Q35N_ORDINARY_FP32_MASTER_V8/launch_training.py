...
T=LINE/'sft_acceptance/ordinary_onpolicy_fp32_master_v8'
...
def main():
    ...
    owners=[]
    for gpu,pid,start,pane in [(3,2923578,163010826,'%153'),(4,2923728,163011283,'%149'),(5,2923915,163011622,'%235')]:
    ...
    subprocess.run(['tmux','new-session','-d','-s','q35n_fp32_master_v8','-c',str(ROOT),cmd],check=True)
    m=runpy.run_path(str(HERE/'workflow.py'));m['freeze']()
    subprocess.run(['tmux','new-session','-d','-s','q35n_fp32_master_v8_review','-c',str(ROOT),
