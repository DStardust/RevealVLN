...
def freeze():
    ...
    pid=int(subprocess.check_output(['tmux','display-message','-p','-t','q35n_route_teacher_v11','#{pane_pid}'],text=True).strip())
    owner=identity(pid)
...
def close_report():
    ...
    result=dict(status='COMPLETE',unix=time.time(),before=before,after=after,paired=paired,
      ...
      source_and_input_locks_unchanged=True,build_audit=read(TRAIN/'BUILD_AUDIT.json'),precision_bridge=read(TRAIN/'PRECISION_BRIDGE_AUDIT.json'),route_teacher=True)
    save(HERE/'RESULT.json',result)
    report='# 普通导航：有序路点教师纠错监督\n\n同100条、5屋INTERNAL_DEV；batch1、纯模型、500动作。不是独立论文确认。\n\n'
    report+='| 模型 | SR | SPL | nDTW | OSR |\n|---|---:|---:|---:|---:|\n'
    for label,case in [('当前最佳4000步',before),('最佳4000步＋有序路点纠错1000步',after)]:
    ...
    audit=read(TRAIN/'BUILD_AUDIT.json')
