...
TRAIN=LINE/'sft_acceptance/ordinary_expanded_continue_v2'
CASE=LINE/'closed_loop_bench/ordinary_expanded_continue_dev_v2'
...
def freeze():
    ...
    pid=int(subprocess.check_output(['tmux','display-message','-p','-t','q35n_expanded_continue_v2','#{pane_pid}'],text=True).strip())
    owner=identity(pid)
...
def close_report():
    ...
    for label,case in [('首段4000更新',before),('第二段累计8000更新',after)]:
