...
T=LINE/'sft_acceptance/ordinary_onpolicy_adapt_v6r1'
...
def main():
    ...
    assert json.loads((HERE/'RESUME_CHECKPOINT_ACCEPTANCE.json').read_text())['status']=='PASS'
    assert json.loads((T/'PROTOCOL_FILESTORE.json').read_text())['accounting']['deadline_unix']-time.time()>700
    owners=[]
    for gpu,pid,start,pane in [(3,2903771,162937144,'%153'),(4,2904009,162937683,'%149'),(5,2904096,162938049,'%235')]:
    ...
    subprocess.run(['tmux','new-session','-d','-s','q35n_onpolicy_adapt_v6r1','-c',str(ROOT),cmd],check=True)
    m=runpy.run_path(str(HERE/'workflow.py'));m['freeze']()
    subprocess.run(['tmux','new-session','-d','-s','q35n_onpolicy_adapt_v6r1_review','-c',str(ROOT),
