...
def freeze():
    ...
    pid=int(subprocess.check_output(['tmux','display-message','-p','-t','q35n_onpolicy_adapt_v6r1','#{pane_pid}'],text=True).strip())
    owner=identity(pid)
    ...
    assert read(HERE/'RESUME_CHECKPOINT_ACCEPTANCE.json')['status']=='PASS'
    paths=[HERE/'RESUME_CHECKPOINT_ACCEPTANCE.json',HERE/'FIRST_CHECKPOINT_ACCEPTANCE.json']+list(HERE.glob('*.py'))+[TRAIN/'PROTOCOL_FILESTORE.json',TRAIN/'RUNBOOK.json',TRAIN/'MAIN_AGENT_APPROVAL.json',CASE/'PENDING_SEAL.json',ANALYZER]
...
def close_report():
    ...
    result=dict(status='COMPLETE',unix=time.time(),before=before,after=after,paired=paired,
      ...
      source_and_input_locks_unchanged=True,build_audit=read(TRAIN/'BUILD_AUDIT.json'),transport_resume=read(TRAIN/'RESUME_AUDIT.json'))
    save(HERE/'RESULT.json',result)
    report='# 普通导航：真实偏离状态纠错训练结果\n\n同100条、5屋INTERNAL_DEV；batch1、纯模型、500动作。不是独立论文确认。\n\n'
    report+='| 模型 | SR | SPL | nDTW | OSR |\n|---|---:|---:|---:|---:|\n'
    for label,case in [('当前最佳4000步',before),('最佳4000步＋纠错1000步',after)]:
