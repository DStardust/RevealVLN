...
def check(n):
    ...
    selected=[x for x in history if 950<x['cursor']['updates']<=n]
    assert selected and all(950<x['cursor']['updates']<=n and x['resume_updates']==950 for x in selected)
    ...
    assert all(x['cursor']==dict(epoch=0,position=950,updates=950,decisions=sum(map(len,plan[x['rank']][:950]))) and x['charged_base']==94032 for x in resume)
    if n==1000:
    ...
    result=dict(status='PASS',unix=time.time(),checkpoint_sha256=rec['sha256'],cursor=state['cursor'],
      ...
      original_optimizer_and_source_preserved=True,navigation_gain_verified=False)
