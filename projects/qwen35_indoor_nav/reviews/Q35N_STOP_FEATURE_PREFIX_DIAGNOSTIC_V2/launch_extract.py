...
def main():
    ...
    env.update(HF_HUB_DISABLE_TELEMETRY='1',PYTHONDONTWRITEBYTECODE='1',PYTHONNOUSERSITE='1',OMP_NUM_THREADS='4',OPENBLAS_NUM_THREADS='1',PYTORCH_CUDA_ALLOC_CONF='expandable_segments:True',CUBLAS_WORKSPACE_CONFIG=':4096:8',CUDA_VISIBLE_DEVICES='1',TMPDIR=str(c.LINE/'.tp12'),TORCHINDUCTOR_CACHE_DIR=str(HERE/'inductor_cache'),TRITON_CACHE_DIR=str(HERE/'triton_cache'),HF_HUB_OFFLINE='1',TRANSFORMERS_OFFLINE='1',TOKENIZERS_PARALLELISM='false')
    ...
    try:
        with (HERE/'extract.log').open('x') as log:
            ...
            while proc.poll() is None:
                ...
                assert time.monotonic()-began<=600 and info['memory_mib']<=25*1024 and rss<=16*1024**3 and size<=1024**3,'RESOURCE_BUDGET'
                time.sleep(5)
