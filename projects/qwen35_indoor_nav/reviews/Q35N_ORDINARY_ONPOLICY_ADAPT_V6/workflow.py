...
CASE=LINE/'closed_loop_bench/ordinary_onpolicy_adapt_dev_v6'
...
def freeze():
    ...
    pid=int(subprocess.check_output(['tmux','display-message','-p','-t','q35n_onpolicy_adapt_v6','#{pane_pid}'],text=True).strip())
    owner=identity(pid)
...
def close_report():
    ...
    result=dict(status='COMPLETE',unix=time.time(),before=before,after=after,paired=paired,
      ...
      source_and_input_locks_unchanged=True,build_audit=read(TRAIN/'BUILD_AUDIT.json'))
