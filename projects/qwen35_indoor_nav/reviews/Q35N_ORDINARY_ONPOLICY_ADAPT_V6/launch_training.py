...
T=LINE/'sft_acceptance/ordinary_onpolicy_adapt_v6'
...
def main():
    ...
    owners=[]
    for gpu,pid,start,pane in [(3,2756725,161974880,'%153'),(4,2756886,161975346,'%149'),(5,2756973,161975685,'%235')]:
    ...
    subprocess.run(['tmux','new-session','-d','-s','q35n_onpolicy_adapt_v6','-c',str(ROOT),cmd],check=True)
    m=runpy.run_path(str(HERE/'workflow.py'));m['freeze']()
    subprocess.run(['tmux','new-session','-d','-s','q35n_onpolicy_adapt_v6_review','-c',str(ROOT),
