...
def main():
    ...
    for x in rows(DATA/'SUPERVISION_ONLY.jsonl'):
        ...
        signature=(bool(e['success']),x['target'],x['original_action'])
        if x['record_id'] in seen:
            assert seen[x['record_id']]==signature,'DUPLICATE_INPUT_OUTCOME_OR_TARGET_DIFFERS'
            continue
        seen[x['record_id']]=signature
    ...
    result=dict(status='COMPLETE',unix=time.time(),data_split='FIT only',original_episodes=64,
        original_successful_episodes=16,eligible_new_inputs=4983,eligible_occurrences=occurrences,deduplication='causal record_id; duplicate target/action/outcome checked exact',by_original_outcome=dict(counts),
