...
def check(n):
    ...
    anchors=[json.loads(s) for s in (TRAIN/'formal/attempt_001/train.log').read_text().splitlines() if s.startswith('{') and 'REFERENCE_READY' in s]
    assert len(anchors)==3 and {x['rank'] for x in anchors}=={0,1,2}
    assert all(x['parity']['all_argmax_identical'] and x['parity']['mean_abs_kl']<=1e-3 for x in anchors)
    assert all('mean_kl' in x['metrics'] and math.isfinite(x['metrics']['mean_kl']) and x['metrics']['mean_kl']>=-2e-5 for x in selected)
    assert any(x['metrics']['mean_kl']>1e-8 for x in selected),'NO_REFERENCE_DRIFT_MEASURED'
    assert all(math.isclose(x['metrics']['mean_loss'],x['metrics']['mean_ce']+x['metrics']['mean_kl'],rel_tol=1e-6,abs_tol=1e-6) for x in selected)
    assert all(x['teacher_forward_decisions']==x['global_plan_decisions'] and x['total_policy_forward_decisions']==2*x['global_plan_decisions'] for x in selected)
    if n==1000:
        assert final['teacher_forward_decisions']==99047 and final['total_policy_forward_decisions']==198094
    action_changes={}
    ...
    result=dict(status='PASS',unix=time.time(),checkpoint_sha256=rec['sha256'],cursor=state['cursor'],action_parameter_changes=action_changes,
      ...
      original_optimizer_values_preserved=True,action_master_precision='FP32',reference_constant_and_restoration_runtime_checks=True,teacher_forward_decisions=total,total_policy_forward_decisions=2*total,mean_kl=selected[-1]['metrics']['mean_kl'],navigation_gain_verified=False)
