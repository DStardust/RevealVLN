...
T=LINE/'sft_acceptance/ordinary_onpolicy_fp32_master_v8r1'
...
def main():
    ...
    owners=[]
    for gpu,pid,start,pane in [(3,3010415,163769328,'%153'),(4,3010508,163769817,'%149'),(5,3010643,163770154,'%235')]:
    ...
    subprocess.run(['tmux','new-session','-d','-s','q35n_fp32_master_v8r1','-c',str(ROOT),cmd],check=True)
    m=runpy.run_path(str(HERE/'workflow.py'));m['freeze']()
    subprocess.run(['tmux','new-session','-d','-s','q35n_fp32_master_v8r1_review','-c',str(ROOT),
