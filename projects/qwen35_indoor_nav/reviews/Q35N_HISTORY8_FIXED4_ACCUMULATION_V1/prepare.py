...
def main():
    ...
    for path in [LINE/'sft_acceptance/ordinary_stop_row_v12/FEATURES.pt',
        ...
        files[str(path)]=sha(path)
    for path in [LINE/'sft_acceptance/ordinary_history8_paired_train_v1'/name for name in ('runtime.py','model.py','loss.py','batching_fixed4.py')] + [LINE/'sft_acceptance/ordinary_onpolicy_fp32_master_v8/initial_fp32_master_from_best4000.pt']:
        files[str(path)]=sha(path)
    for path in [LINE/'closed_loop_bench/ordinary_stop_calibration_fit_v2/tree_size.py', LINE/'reviews/Q35N_HISTORY8_ACCUMULATION_INTERFACE_V1/LAUNCH_FAILURE.json', LINE/'reviews/Q35N_HISTORY8_ACCUMULATION_INTERFACE_V1/LAUNCH_RESULT.json', LINE/'reviews/Q35N_HISTORY8_ACCUMULATION_INTERFACE_R1/RESULT.json', LINE/'reviews/Q35N_HISTORY8_ACCUMULATION_INTERFACE_R1/LAUNCH_RESULT.json', LINE/'sft_acceptance/ordinary_history8_paired_train_v1/SELECTED_SAMPLES.jsonl', LINE/'sft_acceptance/ordinary_history8_paired_train_v1/BATCH_POLICY_REVISION_FIXED4_ZH.md']:
        files[str(path)]=sha(path)
