...
def shared_check():
    ...
    assert r['global_decisions']==r['charged_compute_decisions']==98979
    assert tp['frame_stride']==1 and not tp['reference_kl_enabled']
    assert tp['resume_from']['sha256']=='ba17db7ea899d76015d819776fc7bab8ba6d205a088bc5ad1c5eaad770813ffb'
    assert sha(tp['resume_from']['path'])==tp['resume_from']['sha256']
    build=read(TRAIN/'BUILD_AUDIT.json');assert build['ordinary_draws']==89172 and build['recovery_draws']==9807
    assert build['removed_unavailable_recovery_draws']==68 and build['optimizer_moments_rng_preserved']
    assert read(LINE/'data_pipeline/ordinary_route_teacher_v11/run_001/RESULT.json')['data_gate']
    ...
    return dict(matched_protocol_keys=fixed,checkpoint_sha256=b['checkpoint_sha256'],training_stage_updates=1000,
        optimizer_step=5000,training_decisions=98979,recovery_unique_inputs=4951,removed_unavailable_draws=68,route_teacher=True,holders_restored=True)
...
def final_check():
    ...
    assert monitor['monitor_version']=='ordinary_route_teacher_v11'
