...
TRAIN=LINE/'sft_acceptance/ordinary_action_aligned_history_v10'
...
def shared_check():
    ...
    assert tp['frame_stride']==b['frame_stride']==8 and not tp['reference_kl_enabled']
    assert tp['resume_from']['sha256']=='aa4e3d3bb9be97e4b906bc56834c3cc008c17eeef52ed51c24ac0bbb052466e5'
    assert sha(tp['resume_from']['path'])==tp['resume_from']['sha256']
    assert sha(TRAIN/'RECOVERY_VIEW.json')==tp['input_view_sha256']
    assert read(TRAIN/'VIEW_AUDIT.json')['planned_view_target_conflicts']==0
    ...
    return dict(matched_protocol_keys=fixed,checkpoint_sha256=b['checkpoint_sha256'],training_stage_updates=1000,
        optimizer_step=5000,training_decisions=99047,frame_stride=8,encoded_images_cap=2,reference_kl_enabled=False,holders_restored=True)
...
def selected_rgb_check():
    # Independent full pre-action history, without importing the policy selector.
    total=0
    for folder in sorted((CASE/'run_001/lanes').glob('lane_*')):
        records=lambda name:[json.loads(x) for x in (folder/name).read_text().splitlines() if x.strip()]
        observed={x['index']:[x['rgb_sha256']] for x in records('INTERFACE.jsonl')}
        actions=collections.defaultdict(list)
        policies=records('POLICY_STEPS.jsonl');steps=records('STEPS_PRIVILEGED.jsonl')
        assert len(policies)==len(steps)
        for p,s in zip(policies,steps):
            i=s['index'];t=len(actions[i]);assert p['index']==i and p['step']==s['step']==t+1
            selected=[0] if t==0 else [max(0,t-8),t]
            assert p['input_frame_indices']==selected and p['frame_stride']==8
            assert p['input_rgb_sha256']==[observed[i][j] for j in selected]
            assert p['images']==len(selected) and p['executed_history']==min(8,t)
            assert p['action']==s['action']
            observed[i].append(s['rgb_sha256']);actions[i].append(s['action']);total+=1
    r=read(CASE/'run_001/RESULT.json')
    assert total==r['audited_actions']==r['selected_history_audited_actions']
    return dict(selected_rgb_audited_actions=total,all_selected_indices_causal=True,encoded_images_max=2,frame_stride=8)
def final_check():
    ...
    assert monitor['monitor_version']=='ordinary_action_aligned_history_v10'
    ...
    return dict(before=am,after=bm,delta=delta,wins=wins,losses=losses,audited_actions=ba,
        positive_development_signal=positive,independent_confirmation=False,actual_selected_rgb_audit=selected_rgb_check(),
