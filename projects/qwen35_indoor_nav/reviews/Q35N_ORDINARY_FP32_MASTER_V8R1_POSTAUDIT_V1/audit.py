...
def shared_check():
    ...
    return dict(matched_protocol_keys=fixed,checkpoint_sha256=b['checkpoint_sha256'],training_stage_updates=1000,
        optimizer_step=5000,training_decisions=99047,holders_restored=True)
def aggregate(case):
    ...
    for x,i in zip(rows,inputs):
        ...
        close(x['spl'],x['success']*x['start_distance_m']/max(x['start_distance_m'],x['path_length_m']))
...
def final_check():
    ...
    assert monitor['monitor_version']=='ordinary_fp32_master_v8r1'
...
if __name__=='__main__':
    assert sys.argv[1:] in (['pre'],['final'])
    stage=sys.argv[1];out=HERE/(stage.upper()+'_RESULT.json');assert not out.exists()
    result=dict(status='PASS',stage=stage,unix=time.time(),scope='Repeatedly exposed fixed100 INTERNAL_DEV; engineering only',
        shared=shared_check(),sources=source_check(),extra_training_updates=0,extra_simulator_actions=0,code_sha256=sha(__file__))
