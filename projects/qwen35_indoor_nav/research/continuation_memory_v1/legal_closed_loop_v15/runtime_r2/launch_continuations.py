...
def main():
    protocol=c.read(CODE/'PROTOCOL.json')
    assert c.read(HERE/'CONTINUATION_CPU_TEST_RESULT.json')['passed']
    assert c.read(CODE/'CPU_TEST_RESULT.json')['passed']
    prior=protocol['orchestration_revision']
    assert c.sha(HERE/prior['prior_run']/'STATE_SEAL.json')==prior['prior_seal_sha256']
    ...
    for path in [*CODE.glob('*.py'),CODE/'PROTOCOL.json']:
    ...
    c.write(run/'PREFLIGHT.json',dict(selected=selected,shared_gpu_allowed=True,expected_rollouts=len(protocol['rollouts']),
        train_result_sha256=c.sha(HERE/'train_run_001/RESULT.json'),exclusive_reservation_claimed=False,scheduled_ranks=[r['rank'] for r in protocol['rollouts']],
    ...
    with (run/'worker.log').open('x') as log:
        try:
            proc=subprocess.Popen([str(LINE/'.envs/q35n_qwen_g2_v1/bin/python3'),'-I','-B',
                str(CODE/'evaluate_continuations.py'),str(run)],cwd=c.ROOT,
