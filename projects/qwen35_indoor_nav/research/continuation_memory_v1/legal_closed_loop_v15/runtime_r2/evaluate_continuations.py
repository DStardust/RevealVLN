...
def main(run):
    protocol=c.read(CODE/'PROTOCOL.json');train=HERE/'train_run_001'
    ...
    try:
        ...
        c.write(run/'METHOD_IDENTITY.json',dict(base_state_sha256=initial['sha256'],head_states=head_initial,scheduled_ranks=[r['rank'] for r in protocol['rollouts']],
            protocol_sha256=c.sha(CODE/'PROTOCOL.json'),future_queries_used=False,
        ...
        proc=subprocess.Popen([str(c.LINE/'.envs/q35n_habitat_v017_g0r/bin/python3'),'-I','-B',
            str(CODE/'continuation_service.py'),str(child.fileno()),str(run)],pass_fds=(child.fileno(),),
