...
def main(fd,run):
    ...
    sock=socket.socket(fileno=fd);stream=sock.makefile('rw');backend=None;backend_family=None;active=None;completed=0
    def send(payload):
        ...
        with AuditedContentStore(run/'content',20*2**30) as store:
            for line in stream:
                ...
                if request.get('op')=='reset':
                    assert active is None and set(request)=={'op','rank'} and request['rank']==protocol['rollouts'][completed]['rank']
                    active=protocol['rollouts'][completed];condition=protocol['conditions'][active['condition']]
                    family=families[condition['family_id']]
                    rank=active['rank'];folder=run/'rollouts'/f'{rank:03d}'
                ...
                c.write(folder/'TASK_RESULT.json',dict(rank=rank,condition=condition,model=active['model'],
