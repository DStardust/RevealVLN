...
sys.path.append(str(Path(__file__).resolve().parent.parent/'recovery_action_v1'))
...
def main(run, output, ids, identity_pairs):
    ...
    for arm,record in u.read(run/'PROTOCOL.json')['heads'].items():
        ...
        assert saved['architecture']==record['architecture'] and saved['seed']==record['seed']
        head=ConfirmationMemory(cfg.hidden_size,record['architecture']).cuda().eval();head.load_state_dict(saved['model']);head.requires_grad_(False)
        heads[arm]=head
    def head_hashes():
    ...
    for index in ids:
        episode=planned[index];arms=(['PRISTINE','ZERO_ADAPTER','NONZERO_PROBE'] if index==ids[0] else ['PRISTINE','ZERO_ADAPTER']) if identity_pairs else ['NATIVE']+(list(u.read(run/'PROTOCOL.json')['heads'])[index%len(u.read(run/'PROTOCOL.json')['heads']):]+list(u.read(run/'PROTOCOL.json')['heads'])[:index%len(u.read(run/'PROTOCOL.json')['heads'])]);group=output/'episodes'/str(index)
        ...
        for arm in [x for x in arms[1:] if x!='NONZERO_PROBE']:
            ...
            try:audits[arm]=audit_pair(a,b,results[reference],results[arm],zero=identity_pairs)
                ...
                raise
        seed_pair_audits={}
        if not identity_pairs and not capture:
            for seed in u.read(run/'PROTOCOL.json')['seeds']:
                left=f'LOCAL_s{seed}';right=f'CONCAT_s{seed}'
                a=[json.loads(x) for x in (group/left/'TRACE.jsonl').read_text().splitlines()]
                b=[json.loads(x) for x in (group/right/'TRACE.jsonl').read_text().splitlines()]
                try:seed_pair_audits[str(seed)]=audit_pair(a,b,results[left],results[right])
                except ValueError as error:
                    u.write(group/'FIRST_DIVERGENCE.json',dict(error=str(error),reference=left,arm=right))
                    raise
        u.write(group/'COMPLETE.json',dict(seed_pair_audits=seed_pair_audits,id=index,house=episode['house'],outcomes=results,trace_hashes=traces,
