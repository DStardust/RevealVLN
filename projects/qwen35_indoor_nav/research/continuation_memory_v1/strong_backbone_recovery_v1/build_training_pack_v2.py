...
def main(data,features,output):
    ...
    for family in source['families']:
        ...
        feature_index=u.read(features/'FEATURE_INDEX.json')
        result_path=Path(feature_index[fid])/'RESULT.json';result=u.read(result_path)
        assert result['action_boundary']=='AFTER_NATIVE_ASSISTANT_HEADER_V2','OLD_ACTION_POSITION_CACHE'
        ...
        for row in result['rows']:
            ...
            loaded=torch.load(row['path'],weights_only=True)
            assert loaded['action_boundary']=='AFTER_NATIVE_ASSISTANT_HEADER_V2','OLD_ACTION_POSITION_CACHE'
            cache[row['history'],row['continuation'],row['task']]=(row,loaded)
    ...
    for split,groups in pools.items():
        torch.save(dict(split=split,scope='STREAMVLN_EXECUTED_HISTORY_FEATURES',future_in_policy_features=False,groups=groups,
            action_boundary='AFTER_NATIVE_ASSISTANT_HEADER_V2',debug_only=True,old_admission_unchanged=True,not_blind_test=True),output/(split+'.pt'))
