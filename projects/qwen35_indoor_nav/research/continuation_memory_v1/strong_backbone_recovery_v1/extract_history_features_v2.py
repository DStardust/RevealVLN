...
def main(data,output,family_id):
    ...
    def feature_hook(module,inputs,result):
        current['last_feature']=result.last_hidden_state[0,-1].detach()
    hook=model.model.register_forward_hook(feature_hook)
    class ExecutedActions(transformers.LogitsProcessor):
        def __init__(self,actions):self.actions=actions;self.boundary=ActionBoundary(header)
        def __call__(self,input_ids,scores):
            offset=self.boundary.offset(input_ids)
            if offset is None:return scores
            if offset==0:
                current['base_logits']=scores[0,ids].detach().cpu().float().clone()
                current['feature']=current['last_feature'].cpu().float().clone()
                current['pending']=False
            target=ids[self.actions[offset]] if offset<len(self.actions) else tokenizer.eos_token_id
        ...
        for task_id,task in tasks.items():
            ...
            for step in range(0,len(trace['actions']),4):
                ...
                value=model.generate(inputs=tokens,images=visual,depths=None,poses=None,intrinsics=None,
                    env_id=0,time_ids=[time_ids],task_type=[0],do_sample=False,num_beams=1,max_new_tokens=len(header)+len(executed)+1,
                    use_cache=True,return_dict_in_generate=True,past_key_values=past,
                    logits_processor=transformers.LogitsProcessorList([forced]))
                assert not current['pending'];assert value.sequences[0,:len(header)].tolist()==header,'ASSISTANT_HEADER_NOT_PRESERVED';actual=helper.parse_actions(tokenizer.decode(value.sequences[0]))
                ...
                input_hashes.append(dict(step=step,ids_sha256=hashlib.sha256(tokens.cpu().numpy().tobytes()).hexdigest(),
                    current_rgb_hash=trace['observations'][step]['rgb_hash'],action_prefix_ids=header,pixel_values_sha256=hashlib.sha256(visual.detach().contiguous().view(torch.uint8).cpu().numpy().tobytes()).hexdigest()))
            name=item['history']+'__'+item['continuation']+'__'+task_id;dest=output/(name+'.pt')
            torch.save(dict(memory_features=torch.stack(memory_features),action_boundary='AFTER_NATIVE_ASSISTANT_HEADER_V2',assistant_header_ids=header,memory_input='Frozen current projected visual mean + instruction embedding mean + actual previous action embedding, separately normalized; no future input',features=torch.stack(features),base_action_logits=torch.stack(base_logits),query_steps=queries,
    ...
    u.write(output/'RESULT.json',dict(status='FEATURES_COMPLETE',family=family_id,rows=rows,base_updates=0,
        ...
        source_sha256=u.sha(__file__),action_boundary='AFTER_NATIVE_ASSISTANT_HEADER_V2',assistant_header_ids=header,memory_cadence='Upstream four-action query schedule; actor features after naturally generated assistant header, before first teacher-forced action',
