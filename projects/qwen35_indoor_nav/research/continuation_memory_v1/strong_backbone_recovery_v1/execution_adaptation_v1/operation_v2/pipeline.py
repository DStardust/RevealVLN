...
HERE = REVISION.parent / 'operation_v1'
ADAPT = HERE.parent
BASE = ADAPT.parent
...
def main(run, resume):
    ...
    state = dict(status='RUNNING', phase='TRAIN', planned_models=len(p['models']), planned_steps=p['training_steps'],
        planned_groups=p['planned_groups'], planned_executions=p['planned_groups'] * 7)
    ...
    try:
        ...
        done = completed_models(train, p)
        pending = deque(arm for arm in p['models'] if arm not in done)
        def train_next(gpu):
            if pending:
                arm = pending.popleft(); entry = p['models'][arm]
                command = [str(u.PYTHON), '-I', '-B', str(HERE / 'model_worker.py'), '--run', str(train),
                    '--seed', str(entry['seed']), '--mode', entry['mode'], '--resume']
                spawn(command, gpu, train / arm, 'TRAIN')
        for gpu in available:
            train_next(gpu)
        wait(train_next)
        if set(completed_models(train, p)) != set(p['models']):
            raise ValueError('MISSING_FINAL_MODELS')
        state['phase'] = 'PREPARE_UNSEEN'; save()
        if not (unseen / 'SOURCE_LOCK.json').exists():
            if unseen.exists():
                unseen.rename(run / 'failed_attempts' / ('partial_prepare_' + str(time.time_ns())))
            spawn([str(u.PYTHON), '-I', '-B', str(ADAPT / 'prepare_evaluation.py'),
                '--training-run', str(train), '--run', str(unseen)], None, unseen, 'EVALUATE')
            wait()
        if not (unseen / 'EXECUTOR_BINDING.json').exists():
            # Bind the import-only replacement in a separate immutable receipt.
            u.write(unseen / 'EXECUTOR_BINDING.json', dict(path=str(REVISION / 'evaluate_worker.py'),
                sha256=u.sha(REVISION / 'evaluate_worker.py'), operation_source_lock=u.sha(run / 'SOURCE_LOCK.json')))
        ...
        u.write(run / 'RESULT.json', dict(status='COMPLETE', training_models=len(p['models']),
            planned_optimizer_steps=len(p['models']) * p['training_steps'],
