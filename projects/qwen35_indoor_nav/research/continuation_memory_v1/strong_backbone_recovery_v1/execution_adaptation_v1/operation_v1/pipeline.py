...
def main(run, resume):
    ...
    wall_before = sum(u.read(path)['wall_seconds'] for path in (run / 'sessions').glob('*.json'))
    ...
    try:
        ...
        def evaluate(gpu, ids):
            ...
            spawn([str(u.PYTHON), '-I', '-B', str(ADAPT / 'evaluate_worker.py'), '--run', str(unseen),
