...
def main(run, output, ids):
    ...
    if len(protocol['heads']) != 12 or len(seed_pairs) != 12:
    ...
    for seed in protocol['seeds']:
        for mode in ('CURRENT', 'DELTA'):
            first, all_tokens = [protocol['heads'][f'{mode}_{scope}_s{seed}'] for scope in ('FIRST', 'ALL')]
            if first['path'] != all_tokens['path'] or first['sha256'] != all_tokens['sha256']:
                raise ValueError('SCOPE_PAIR_WEIGHTS_DIFFER')
