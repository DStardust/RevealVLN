...
- 原 369 条评测保持运行。独立服务等待其完成并释放本项目租约后，再使用最多 8 卡运行本轮 1040 次执行。等待最多 8 小时；本轮评测最多 24 GPU 会话小时、12 小时运行墙钟。前驱失败则明确阻断，不抢卡、不删旧失败、不按分数重试。
...
systemctl status q35n-strong-scope-validation-20260926-01.service
tail -n 80 "$BASE/standalone_jobs/strong-scope-validation-20260926-01/job.log"
```

监控网站地址不变：`http://127.0.0.1:18770/`，新增 `/api/scope_validation`；原评测与补采页面保留。队列和流水线都独立于 Codex 在线状态。

只有当前服务已经退出、原因排除后，才用新服务名调用本目录 `pipeline.py --run .../runs/scope_001 --resume`；恢复保留完整封存组及全部失败尝试。完整低分组不得重试。
