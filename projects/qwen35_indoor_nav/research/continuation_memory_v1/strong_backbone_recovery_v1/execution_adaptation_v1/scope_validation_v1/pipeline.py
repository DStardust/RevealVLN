...
def wait_for_predecessor(run):
    p = u.read(run / 'PROTOCOL.json')
    source = Path(p['wait_for_run'])
    began = time.time()
    with (run / 'QUEUE.lock').open('a') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        while True:
            previous = u.read(source / 'STATUS.json')
            if previous['status'] == 'COMPLETE':
                # COMPLETE is written only after releasing the predecessor lease.
                return
            if previous['status'] in ('FAILED', 'INTERRUPTED'):
                u.write(run / 'STATUS.json', dict(status='BLOCKED', phase='PREDECESSOR_FAILED',
                    predecessor=str(source), error=previous.get('error')))
                raise RuntimeError('PREDECESSOR_REQUIRES_REPAIR')
            if time.time() - began > p['queue_wait_hours'] * 3600:
                u.write(run / 'STATUS.json', dict(status='BLOCKED', phase='QUEUE_TIMEOUT'))
                raise RuntimeError('BOUNDED_QUEUE_WAIT_EXHAUSTED')
            u.write(run / 'STATUS.json', dict(status='QUEUED', phase='WAITING_FOR_CURRENT_UNSEEN',
                predecessor=str(source), predecessor_sealed_groups=previous['sealed_groups'],
                planned_groups=p['planned_groups'], planned_executions=p['planned_executions'],
                gpu_hours=0, optimizer_updates=0, unix=time.time()))
            time.sleep(30)
...
if __name__ == '__main__':
    ...
    wait_for_predecessor(args.run.resolve())
