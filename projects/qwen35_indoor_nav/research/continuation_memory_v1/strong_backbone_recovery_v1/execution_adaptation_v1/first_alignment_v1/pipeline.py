"""FIRST-aligned training now; complete-group unseen evaluation when devices are available."""
import argparse
...
def main(run, resume):
    ...
    state = dict(status='RUNNING', phase='TRAIN_SHARED', planned_models=len(p['models']), planned_optimizer_steps=18000,
    ...
    def spawn(command, gpu, output, cost_phase):
        ...
        if gpu is not None and inventory[gpu]['free_mib'] < (p['training_minimum_free_mib'] if cost_phase == 'TRAIN' else p['minimum_free_mib']):
    ...
    try:
        save()
        def registered_peer(pid):
            proc = Path(f'/proc/{pid}')
            try:
                return proc.stat().st_uid == os.getuid() and ('/system.slice/' + p['allowed_shared_service']) in (proc/'cgroup').read_text()
            except FileNotFoundError:
                return True  # A finished registered worker no longer holds this device.
        before = placeholder('status')
        if before['manual_paused'] or any(not registered_peer(pid) for pid in set(before['leases'] + before['external_pids'])):
            raise RuntimeError('UNREGISTERED_RESOURCE_OWNER')
        ...
        u.write(run / f'RESOURCE_LEASE_{time.time_ns()}.json', dict(before=before, acquired=acquired,
            sharing='Lightweight training, at most 2 GiB per process, with registered existing evaluation.'))
        inventory = devices()
        train_gpus = [g for g in p['gpu_indices'] if inventory[g]['free_mib'] >= p['training_minimum_free_mib']]
        pending_models = deque(a for a in p['models'] if a not in completed_models(train,p))
        if pending_models and not train_gpus:
            raise RuntimeError('NO_SAFE_SHARED_TRAINING_DEVICE')
        def train_next(gpu):
            if pending_models:
                arm = pending_models.popleft(); mode, seed = arm.rsplit('_s',1)
                spawn([str(u.PYTHON),'-I','-B',str(HERE/'train_worker.py'),'--run',str(train),
                    '--seed',seed,'--mode',mode,'--resume'],gpu,train/arm,'TRAIN')
        for gpu in train_gpus: train_next(gpu)
        wait(train_next)
        if set(completed_models(train,p)) != set(p['models']):
            raise ValueError('TRAINING_INCOMPLETE')
        if not (unseen/'PROTOCOL.json').exists():
            spawn([str(u.PYTHON),'-I','-B',str(HERE/'prepare.py'),'--run',str(run),'--evaluation'],None,unseen,'EVALUATE')
            wait()
        state['phase'] = 'WAIT_EVALUATION_DEVICE'; save()
        while True:
            resource = placeholder('status')
            other = [pid for pid in resource['leases'] if pid != os.getpid()]
            if any(not registered_peer(pid) for pid in other):
                raise RuntimeError('UNREGISTERED_RESOURCE_OWNER')
            if not other: break
            if wall_before + time.time() - began > p['wall_hours_limit'] * 3600:
                raise RuntimeError('WALL_BUDGET_REACHED')
            state['waiting_for_registered_service'] = p['allowed_shared_service']; save(); time.sleep(15)
        inventory = devices()
        available = [g for g in p['gpu_indices'] if inventory[g]['free_mib'] >= p['minimum_free_mib']]
        if not available: raise RuntimeError('NO_SAFE_EVALUATION_DEVICE')
        ...
        u.write(run / 'FIRST_GROUP_RESULT.json', dict(status='REAL_THIRTEEN_ARM_GROUP_SEALED', id=smoke_id,
        ...
        u.write(run / 'RESULT.json', dict(status='COMPLETE', trained_models=len(p['models']),
            optimizer_updates=18000,
