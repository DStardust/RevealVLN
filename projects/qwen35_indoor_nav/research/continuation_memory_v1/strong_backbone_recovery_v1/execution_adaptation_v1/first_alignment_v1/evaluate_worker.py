...
sys.path[:0] = [str(Path(__file__).resolve().parent), str(HERE), str(HERE.parent)]
import common as u
...
def load_registered_head(record, protocol, device):
    ...
    if config['action_scope'] != record['training_action_scope'] or record['training_action_scope'] not in ('ALL_ACTION_TOKENS_QUERY_START_MEMORY', 'FIRST_ACTION_TOKENS_QUERY_START_MEMORY') or config['base_updates'] != 0:
...
def main(run, output, ids):
    ...
    from runtime import LiveRuntime, make_logits_processors, tensor_hash
    ...
    seed_pairs = protocol['comparisons']
    if len(protocol['heads']) != protocol['expected_head_count'] or len(seed_pairs) != protocol['expected_comparison_count']:
        raise ValueError('SCOPE_REGISTRY_COUNT_CHANGED')
    for pair in seed_pairs.values():
        if set(pair) != {'CURRENT', 'DELTA'} or any(arm not in heads for arm in pair.values()):
            raise ValueError('UNREGISTERED_COMPARISON')
    if any(record['scope'] != 'FIRST' for record in protocol['heads'].values()):
        raise ValueError('FROZEN_FIRST_SCOPE_REQUIRED')
    ...
    u.write(output / 'RUNTIME_IDENTITY.json', dict(model_loaded=True, base_state_sha256=frozen,
        ...
        base_updates=0, action_scope='REGISTERED_FIRST_OR_ALL_WITH_SAME_FROZEN_HEAD',
    ...
    def logged_generate(*args, **kwargs):
        ...
        processor = ScopedProcessor(runtime, header, token_ids,
            real_actor_feature, apply_residual=active['arm'] != 'NATIVE',
            scope=protocol['heads'][active['arm']]['scope'] if active['arm'] != 'NATIVE' else 'ALL')
        ...
        for index in ids:
            ...
            for arm in order:
                ...
                u.write(output / 'PROGRESS.json', dict(status='RUNNING', complete=completed, planned=ids,
                    current_id=index, current_arm=arm, model_loaded=True, action_scope='REGISTERED_FIRST_OR_ALL_WITH_SAME_FROZEN_HEAD'))
