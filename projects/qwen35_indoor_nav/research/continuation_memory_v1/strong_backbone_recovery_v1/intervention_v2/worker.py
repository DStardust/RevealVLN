"""Frozen paired policies with real TRAIN turn prefixes and an optional gate."""
import argparse
import hashlib
import json
import os
...
from selection import forced_action, choose_once
...
def main(run, output, ids, identity_pairs):
    ...
    from intervention_gate_v1 import InterventionGate
    ...
    for arm,record in protocol['heads'].items():
        ...
        assert saved['step']==1200 and saved['arm']==record.get('source_arm',arm)
        head=ExecutionMemory(cfg.hidden_size).cuda().eval();head.load_state_dict(saved['model']);head.requires_grad_(False)
        heads[arm]=head
    gates={}
    for arm,record in protocol.get('gates',{}).items():
        assert u.sha(record['path'])==record['sha256'],'GATE_FILE_CHANGED'
        gate=InterventionGate(3592).cuda().eval()
        saved_gate=torch.load(record['path'],map_location='cpu',weights_only=True)
        assert saved_gate['arm']==protocol['heads'][arm]['source_arm'] and saved_gate['steps']==protocol['gate']['steps']
        gate.load_state_dict(saved_gate['model'])
        gate.requires_grad_(False);gates[arm]=gate
    def head_hashes():
        return {arm:{name:tensor_hash(t) for name,t in head.state_dict().items()} for arm,head in {**heads,**{'gate_'+k:v for k,v in gates.items()}}.items()}
    ...
    identity['gates']=protocol.get('gates',{})
    ...
    class LoggedEnv(real_env):
        ...
        def step(self,action,*a,**kw):
            if active['steps']<len(active['prefix']):
                assert int(action)==active['prefix'][active['steps']],'FORCED_PREFIX_NOT_EXECUTED'
            ...
            emit(dict(event='action',step=active['steps'],executed_action=int(action),before_rgb_sha256=before,
                ...
                forced_prefix=active['steps']<=len(active['prefix']),
    ...
    class Residual(transformers.LogitsProcessor):
        ...
        def __call__(self,input_ids,scores):
            ...
            forced=forced_action([token_ids[i] for i in active['prefix']],active['steps'],offset,tokenizer.eos_token_id)
            if offset!=0:
                if forced is None:return scores
                revised=torch.full_like(scores,torch.finfo(scores.dtype).min);revised[:,forced]=0;return revised
            ...
            proposed=revised.clone();gate_evidence=None
            if forced is not None:
                revised=torch.full_like(scores,torch.finfo(scores.dtype).min);revised[:,forced]=0
            elif active['arm'] in gates:
                revised,active['accepted'],gate_evidence=choose_once(gates[active['arm']],active['feature'],scores,proposed,token_ids,active['accepted'])
            if forced is None and active['arm'] in ('PRISTINE','NATIVE','ZERO_ADAPTER'):
            ...
            active['decision']=dict(native_logits=scores[0,token_ids].float().cpu().tolist(),
                ...
                proposal_logits=proposed[0,token_ids].float().cpu().tolist(),
                forced_prefix_query=forced is not None,gate_decision=gate_evidence,gate_accepted=active['accepted'],
    ...
    for index in ids:
        episode=planned[index]
        options=list(protocol['heads']);shift=index%len(options)
        arms=(['PRISTINE','ZERO_ADAPTER','NONZERO_PROBE'] if index==ids[0] else ['PRISTINE','ZERO_ADAPTER']) if identity_pairs else ['NATIVE']+options[shift:]+options[:shift]
        ...
        for arm in arms:
            ...
            with (path/'TRACE.jsonl').open('x') as log:
                active.clear();active.update(log=log,arm=arm,features=features,pending_observation=False,instruction=episode['instruction'],path=path,
                    prefix=episode.get('forced_prefix',[]),accepted=None)
