...
def main(run):
    ...
    protocol=dict(source=str(source),source_sha256=u.sha(source),compiler_sha256=u.sha(compiler_path),worker_sha256=u.sha(__file__),
        ...
        renderer='Original project Habitat 0.1.7; separate from ordinary StreamVLN Habitat0.2.4',
    ...
    try:
        for family in families:
            cfg=hs.SimulatorConfiguration();cfg.scene_id=family['scene'];cfg.gpu_device_id=0;cfg.enable_physics=False;cfg.allow_sliding=False
            ...
            for name,kind,res,hfov in [('policy_rgb',hs.SensorType.COLOR,[480,640],79),('policy_semantic',hs.SensorType.SEMANTIC,[480,640],79),
                ...
                sensor=hs.SensorSpec();sensor.uuid=name;sensor.sensor_type=kind;sensor.resolution=res
                sensor.position=[0,1.25,0];sensor.orientation=[0,0,0];sensor.parameters['hfov']=str(hfov);sensor.gpu2gpu_transfer=False;sensors.append(sensor)
            ...
            for history_id,history in family['candidate']['histories'].items():
                ...
                for continuation,suffix in family['candidate']['continuations'].items():
                    ...
                    legacy=u.read(u.ASSETS/'projects/qwen35_indoor_nav/data_pipeline/mechanism_runtime_v1/legal_history_v15/raw_run_008'/family['family_id']/(history_id+'__'+continuation+'.json'))
                    assert trace['observations'][0]['audit_semantic_hash']==legacy['observations'][0]['semantic_hash'],'AUDIT_SENSOR_INITIAL_FRAME_MISMATCH'
                    ...
                    assert [o['audit_semantic_hash'] for o in obs]==[o['semantic_hash'] for o in legacy['observations']],'AUDIT_SENSOR_FULL_TRAJECTORY_MISMATCH'
