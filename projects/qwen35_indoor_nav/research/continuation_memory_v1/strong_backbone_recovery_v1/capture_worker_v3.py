...
def main(run, output, ids, identity_pairs):
    ...
    for index in ids:
        episode=planned[index];arms=(['PRISTINE','ZERO_ADAPTER','NONZERO_PROBE'] if index==ids[0] else ['PRISTINE','ZERO_ADAPTER']) if identity_pairs else ['NATIVE']+(['BC','B2','OURS'][index%3:]+['BC','B2','OURS'][:index%3]);group=output/'episodes'/str(index)
        ...
        for arm in arms:
            ...
            with (path/'TRACE.jsonl').open('x') as log:
                active.clear();active.update(log=log,arm=arm,features=features,pending_observation=False,instruction=episode['instruction'],path=path,capture=capture,dense_features=[])
        ...
        if capture:
            ...
            torch.save(dict(memory_features=torch.stack(active['dense_features']),actor_features=torch.stack(features),
                ...
                source='ACTUAL_FROZEN_NATIVE_ROLLOUT_NO_SUCCESS_FILTER'),cache)
