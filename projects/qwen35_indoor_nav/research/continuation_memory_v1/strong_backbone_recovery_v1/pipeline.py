"""Independent, resumable bootstrap of the strong backbone and zero-residual path."""
import argparse
import fcntl
import json
import os
...
def main(run):
    ...
    status=dict(status='RUNNING',phase='ASSET_VERIFY',started_unix=started,completed_native=len(completed(run,'baseline')),
        planned_native=100,method_training_started=False,method_benefit='UNKNOWN',gpu_hours=used_hours)
    ...
    try:
        ...
        for phase,ids in [('identity',[0,1]),('baseline',list(range(100)))]:
        ...
        rows=list(completed(run,'baseline').values());outcomes=[r['outcomes']['NATIVE'] for r in rows]
        result=dict(status='NATIVE_BASELINE_COMPLETE',complete=100,planned=100,
            sr=sum(x['success'] for x in outcomes)/100,spl=sum(x['spl'] for x in outcomes)/100,
            os=sum(x['os'] for x in outcomes)/100,mean_steps=sum(x['steps'] for x in outcomes)/100,
            zero_adapter_identity_pairs=2,backbone=protocol['backbone'],exposed_internal_dev=True,
            method_training_completed=False,method_benefit='NOT_YET_MEASURED',adopted=False,gpu_hours=used_hours)
        u.write(run/'RESULT.json',result)
        status.update(status='BASELINE_COMPLETE_METHOD_DATA_PENDING',phase='BASELINE_COMPLETE',completed_native=100)
        persist()
