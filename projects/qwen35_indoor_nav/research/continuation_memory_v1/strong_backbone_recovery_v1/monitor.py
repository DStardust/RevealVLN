...
if(s.error)lines.push('错误：'+s.error);if(d.result)lines.push('本机原生SR：'+(100*d.result.sr).toFixed(1)+'%（'+d.result.complete+'/'+d.result.planned+'；不计作我们的增量）');
...
class Handler(previous.Handler):
    def do_GET(self):
        ...
        if self.path=='/api/strong':
            ...
            body=json.dumps(dict(run=run.name,status=u.read(run/'STATUS.json'),result=u.read(run/'RESULT.json') if (run/'RESULT.json').exists() else None),ensure_ascii=False).encode()
