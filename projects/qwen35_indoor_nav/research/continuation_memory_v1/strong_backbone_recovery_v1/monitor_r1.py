...
if(d.data_status){let c=d.data_status;lines.push('移动历史重验：'+c.status+' '+(c.completed_traces||0)+'/'+(c.planned_traces||72));if(c.error)lines.push('数据错误：'+c.error);}if(s.error)lines.push('错误：'+s.error);if(d.result)lines.push('本机原生SR：'+(100*d.result.sr).toFixed(1)+'%（'+d.result.complete+'/'+d.result.planned+'；不计作我们的增量）');
...
class Handler(previous.Handler):
    def do_GET(self):
        ...
        if self.path=='/api/strong':
            ...
            body=json.dumps(dict(run=run.name,status=u.read(run/'STATUS.json'),data_status=u.read(u.HERE/'data_runs/moving_002/STATUS.json') if (u.HERE/'data_runs/moving_002/STATUS.json').exists() else None,result=u.read(run/'RESULT.json') if (run/'RESULT.json').exists() else None),ensure_ascii=False).encode()
