...
def registry_value(raw_families,config):
    ...
    if len(selected)!=16:raise ValueError('DEV_FAMILY_COUNT')
    conditions=[]
    for f in selected:
        for h in ('seen','seen_sham','missing','missing_sham'):
            for task in ('task_A','task_T'):
                conditions.append(dict(family_id=f['family_id'],house=f['house'],history_id=h,task_id=task,endpoint='main' if task=='task_A' else 'control',parent_family_id=f['parent_family_id'],stratum=f['stratum']))
    ...
    return dict(scope='CONTROLLED_DEV_MECHANISM',conditions=conditions,models=models,slots=slots,
                expected_prefix_comparisons=768,models_per_condition=9,main_denominator_per_arm=192,control_denominator_per_arm=192)
...
def main(run):
    ...
    remaining=[r for r in reg['slots'] if r['condition'] in assigned and r['condition'] not in done]
    ...
    try:
        ...
        with torch.inference_mode():
            for slot in remaining:
                ...
                rows=[];alive=True
                references={other:True for other in contrast if other.split('_')[1]==tag.split('_')[1] and (other.startswith('B2_')!=tag.startswith('B2_'))}
                while alive:
                    ...
                    if len(rows)%20==0:write(run/('EVALUATION_PROGRESS_'+os.environ['B2_GPU']+'.json'),dict(rank=rank,condition=current,model=tag,step=row['decision'],completed_slots=len(done)*9+len(completed),forwards=forwards,unix=time.time()))
                ...
                if len(contrast)==9:
                    ...
                    for seed in config['seeds']:
                        for arm in ('B1','Terminal'):
                            left=f'{arm}_{seed}';right=f'B2_{seed}';audit=prefix_audit(contrast[left],contrast[right])
