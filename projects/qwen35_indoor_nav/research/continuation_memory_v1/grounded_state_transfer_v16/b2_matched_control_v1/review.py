...
def main(run):
        ...
        for endpoint in ('main','control'):
            ...
            for baseline in ('B1','Terminal'):
                ...
                for condition in sorted({r['condition'] for r in selected}):
                    ours=next(r for r in selected if r['condition']==condition and r['arm']=='B2')
    ...
    result['primary_comparison']='B2 minus Terminal, shared expanded data; main task split by takeover coverage and missing/seen history'
    ...
    lines=['# B1 / B2 / Terminal-only 匹配对照结果','',result['status'],f"完整配对组 {len(groups)}/{len(reg['conditions'])}；续接 {result['complete']}/{len(reg['slots'])}。",'',
