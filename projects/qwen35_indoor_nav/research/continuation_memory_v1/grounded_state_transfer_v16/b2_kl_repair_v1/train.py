"""Three repaired final1200 models and six sealed controls; full optimizer/RNG recovery, no TEST loss access."""
import os
import random
import sys
import time
...
def training_families(data,config):
    ...
    if len({f['parent_family_id'] for f in fit})!=32 or any(f['split'] not in ('FIT','DEV') for f in data['families']):raise ValueError('PILOT_SPLIT')
    return fit


def schedule_for_seed(families,old_schedule,fit_ordinary,seed):
    from collections import defaultdict
    grouped=defaultdict(list)
    for f in families:grouped[f['parent_family_id']].append(f['family_id'])
    ids=sorted(grouped);parents=(ids*((1200+len(ids)-1)//len(ids)))[:1200]
    random.Random(seed).shuffle(parents)
    cursors={p:seed%len(grouped[p]) for p in ids};schedule=[]
    for p in parents:
        variants=sorted(grouped[p]);schedule.append((p,variants[cursors[p]%len(variants)]));cursors[p]+=1
    natural=[old_schedule[i%len(old_schedule)] for i in range(1200)]
    if any(i not in set(fit_ordinary) for selected in natural for i in selected):raise ValueError('NONFIT_ORDINARY_SCHEDULE')
    return [dict(parent_family=p,family=f,ordinary=o) for (p,f),o in zip(schedule,natural)]
...
def main(run):
    ...
    ordinary_folder=V16.parent/'natural_transfer_v9';ordinary=read(ordinary_folder/'DATA.json')
    oldpath=Path(feature_result['ordinary_path'])
    ...
    for seed in config['seeds']:
        ...
        schedule=schedule_for_seed(fit,old_schedule,fit_ordinary,seed)
        if read(folder/f'SCHEDULE_{seed}.json')!=schedule:raise ValueError('SHARED_SCHEDULE_CHANGED')
        for arm in config['arms']:
            tag=f'{arm}_{seed}'
            if tag not in os.environ['B2_TRAIN_TAGS'].split(','):continue
            target=folder/tag;target.mkdir(exist_ok=True)
            ...
            control=read(CONTROL/'train'/f'B2_{seed}'/'RESULT.json')
            if initial!=control['initial_state_sha256']:raise ValueError('INITIAL_STATE_MISMATCH')
            ...
            for step in range(start,1200):
                ...
                special,stats,detail=o.losses(net,cache,batches[row['family']],arm,weights,retain_steps=(1,) if step==0 else ())
                ...
                if step==0:
                    write_tensor=detail['writes'][1]
                    ag=torch.autograd.grad(detail['action_loss'],write_tensor,retain_graph=True)[0]
                    ug=torch.autograd.grad(detail['auxiliary_loss'],write_tensor,retain_graph=True,allow_unused=True)[0] if arm!='B1' else None
                    write(attempt_dir/'GRADIENT_PATH.json',dict(early_write_step=1,cutoff_min=min(r['cutoff'] for r in batches[row['family']]['family']['sequences']),
                        action_gradient_norm=float(ag.norm()),auxiliary_gradient_norm=None if ug is None else float(ug.norm()),
                        truncated_backprop=False),True)
                loss.backward()
                ...
                write(run/('TRAIN_PROGRESS_'+os.environ['B2_GPU']+'.json'),dict(tag=tag,step=step+1,completed_models=len(results),seconds=time.monotonic()-began))
                if (step+1)%200==0:
    ...
    immutable(folder/('SHARD_'+os.environ['B2_GPU']+'.json'),dict(models=results,base_updates=0,final_step=1200))
