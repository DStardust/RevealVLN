        ...
        for endpoint in ('main','control'):
            for baseline in ('B1','B2'):
                ...
                for (condition,seed,arm),label in values.items():
                    if arm!='B2Fix' or registry['conditions'][condition]['endpoint']!=endpoint:continue
...
def main():
    ...
    reader = Reader(PILOT / 'runs/repair_001', PILOT / 'standalone_jobs/b2-kl-repair-20260921-01')
    if args.once:
