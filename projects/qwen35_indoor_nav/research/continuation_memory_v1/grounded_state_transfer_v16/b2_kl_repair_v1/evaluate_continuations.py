    ...
    try:
        ...
        with torch.inference_mode():
            for slot in remaining:
                ...
                rows=[];alive=True
                references={other:True for other in contrast if other.split('_')[1]==tag.split('_')[1] and (other.startswith('B2Fix_')!=tag.startswith('B2Fix_'))}
                while alive:
                ...
                if len(contrast)==9:
                    ...
                    for seed in config['seeds']:
                        for arm in ('B1','B2'):
                            left=f'{arm}_{seed}';right=f'B2Fix_{seed}';audit=prefix_audit(contrast[left],contrast[right])
