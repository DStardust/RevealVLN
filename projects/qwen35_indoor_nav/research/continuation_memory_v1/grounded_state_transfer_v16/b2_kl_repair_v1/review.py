...
def main(run):
        ...
        for endpoint in ('main','control'):
            ...
            for baseline in ('B1','B2'):
                ...
                for condition in sorted({r['condition'] for r in selected}):
                    ours=next(r for r in selected if r['condition']==condition and r['arm']=='B2Fix')
    ...
    result['primary_comparison']='B2Fix minus B2, shared expanded data; main task split by takeover coverage and missing/seen history'
    ...
    result['training_intervention']='Remove only supervised native/teacher disagreement KL terms; original denominator retained'
    result['old_rollouts_reused']=False
    immutable(run/'RESULT.json',result)
    lines=['# B1 / B2 / B2Fix 匹配对照结果','',result['status'],f"完整配对组 {len(groups)}/{len(reg['conditions'])}；续接 {result['complete']}/{len(reg['slots'])}。",'',
    ...
    lines += ['',result['interpretation'],'','STATE_DIAGNOSIS 与 MEMORY_INTERVENTIONS 是受控动作诊断，不替代闭环；未排除完整动作历史捷径。全部模型固定 final1200，没有按 DEV 选 checkpoint。']
