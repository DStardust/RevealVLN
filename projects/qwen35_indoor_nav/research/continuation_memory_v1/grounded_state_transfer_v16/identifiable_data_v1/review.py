...
def main():
    ...
    for family in families:
        compiler=legacy.Compiler(**family['compiler']);traces={};root=LINE/family['content_root']
        ...
        for h in HISTORIES:
            ...
            for task in ('task_A','task_T'):
                ...
                teacher='direct_stop' if h.startswith('seen') or task=='task_T' else 'acquire_anchor'
                for q in QUERIES:
        ...
        for a,b,sham in (('seen','missing','seen_sham'),('missing','seen','missing_sham')):
            task='task_A';q='direct_stop' if a=='seen' else 'acquire_anchor';bq='direct_stop' if b=='seen' else 'acquire_anchor'
            ids=[sequence_indices[(a,task,q)],sequence_indices[(b,task,bq)],sequence_indices[(sham,task,q)]]
            if len({records[i]['features'][cutoff] for i in ids})!=1:raise ValueError('EXPORTED_MECHANISM_INPUT_MISMATCH')
            cases.append(dict(task=task,correct=[ids[0],cutoff],wrong=[ids[1],cutoff],sham=[ids[2],cutoff],
                              target=records[ids[0]]['targets'][cutoff],interpretation='STOP admissibility and registered cost teacher contrast, not uniqueness of all valid next actions'))
            controls=[sequence_indices[(name,'task_T','direct_stop')] for name in (a,b,sham)]
            if len({records[i]['features'][cutoff] for i in controls})!=1:raise ValueError('CONTROL_INPUT_MISMATCH')
            cases.append(dict(task='task_T',correct=[controls[0],cutoff],wrong=[controls[1],cutoff],sham=[controls[2],cutoff],target=3))
        reviewed.append(dict(family_id=family['family_id'],house=house,split=split,certificate=certificate,mechanism_cases=cases))
        counts.update(families=1,physical_executions=12,physical_histories=4,cross_labels=24,mechanism_cases=2,task_T_control_cases=2)
    ...
    write(out/'RESULT.json',dict(status='VERIFIED_CONTROLLED_MECHANISM_DATA' if families else 'NO_CERTIFIED_FAMILIES',
        counts=counts,by_house=[dict(split=s,house=h,families=n) for (s,h),n in sorted(byhouse.items())],
        raw_arrays_verified=len(arrays),unique_policy_inputs=len(features),supervision_sequences=len(records),
        ...
        scope='Actual stationary histories and legally executed continuations; no empirical method gain. Task A versus task T, not dual-anchor V16 replacement.'),True)
