...
def main():
    ...
    source={str(p.relative_to(LINE)):sha(p) for p in (Path(__file__).resolve(),HERE/'certify.py',
    ...
    began=time.monotonic();last=0;backend=None
    reuse=read(HERE/cfg['reuse_manifest'])
    for path,expected in reuse['files'].items():
        if sha(Path(path))!=expected:raise ValueError('REUSED_CERTIFICATE_CHANGED')
    completed=list(reuse['families'])
    def check():
        nonlocal last
    ...
    try:
        ...
        for house in manifest['houses']:
            ...
            accepted=[f for f in reuse['families'] if f['house']==house['house']]
            fresh=[read(p) for p in sorted(house_dir.glob('*/FAMILY.json'))]
            accepted.extend(fresh);completed.extend(fresh)
            ...
            planner=load('ident_reserved_metadata',LINE/'data_pipeline/mechanism_scale_v1/planner.py')
            adapter=load('ident_role_match',LINE/'data_pipeline/mechanism_runtime_v1/habitat_backend.py')
            ...
            write(house_dir/'RESERVED_ROLE_EXCLUSIONS.json',dict(excluded=excluded,rule='Exclude whole semantic signatures, never reinterpret reserved pixels'))
