...
class MonitorTest(unittest.TestCase):
    def setUp(self):
        ...
        slots = [dict(rank=i*9+j,condition=i,model=f'{a}_{s}',arm=a,seed=s)
                 for i in range(2) for j,(s,a) in enumerate((s,a) for s in (1209,1210,1211) for a in ('B1','B2','Ours'))]
        ...
        write(self.root/'PROTOCOL.json',dict(gpu_session_hours=8))
        files = {}
        for slot in slots[:9]:
            ...
            write(self.session/name,dict(rank=slot['rank'],model=slot['model'],condition=conditions[0],
                safe_v16_label='PASS' if slot['arm']=='Ours' else 'FAIL',total_decisions=30,collisions=0,stopped=True,budget_exhausted=False))
    ...
    def test_sealed_partial_keeps_planned_unknown_denominator(self):
        ...
        ours=next(m for m in value['metrics'] if m['arm']=='Ours' and m['endpoint']=='main')
