...
def main(run):
    ...
    remaining=[r for r in reg['slots'] if r['condition'] not in done]
        ...
        with torch.inference_mode():
            for slot in remaining:
                ...
                while alive:
                    ...
                    if len(rows)%20==0:write(run/'EVALUATION_PROGRESS.json',dict(rank=rank,condition=current,model=tag,step=row['decision'],completed_slots=len(done)*9+len(completed),forwards=forwards,unix=time.time()))
