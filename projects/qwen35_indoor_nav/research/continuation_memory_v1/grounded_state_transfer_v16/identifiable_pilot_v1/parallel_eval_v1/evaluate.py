"""Dynamic nine-model complete groups, real autonomous forwards and atomic seals."""
import json
import os
...
def registry_value(raw_families,config):
    selected=sorted((f for f in raw_families if f['split']=='DEV'),key=lambda f:(f['house'],f['family_id']))
    if len(selected)!=8:raise ValueError('DEV_FAMILY_COUNT')
    conditions=[]
    for f in selected:
        for h in ('seen','seen_sham','missing','missing_sham'):
            for task in ('task_A','task_T'):
                conditions.append(dict(family_id=f['family_id'],house=f['house'],history_id=h,task_id=task,endpoint='main' if task=='task_A' else 'control'))
    models=[f'{a}_{s}' for s in config['seeds'] for a in config['arms']]
    slots=[]
    ...
    return dict(scope='CONTROLLED_DEV_MECHANISM',conditions=conditions,models=models,slots=slots,
                expected_prefix_comparisons=384,models_per_condition=9,main_denominator_per_arm=96,control_denominator_per_arm=96)
...
def main(run, assignment_path):
    import torch
    import objective as o
    import encoder
    config=read(run/'PROTOCOL.json');verify_lock(read(run/'SOURCE_LOCK.json'));reg=registry(run)
    assignment=read(assignment_path)
    if sha(RUNTIME/'evaluate.py')!=assignment['runtime_sha256']:raise ValueError('PARALLEL_RUNTIME_CHANGED')
    if sha(run/'PROTOCOL.json')!=assignment['protocol_sha256'] or sha(run/'EVALUATION_REGISTRY.json')!=assignment['registry_sha256']:raise ValueError('PARALLEL_BINDING_CHANGED')
    config=dict(config,gpu=assignment['gpu'],gpu_uuid=assignment['gpu_uuid'])
    out=run/'evaluate';out.mkdir(exist_ok=True);done=admitted(run,reg)
    remaining=[r for r in reg['slots'] if r['condition'] in assignment['conditions'] and r['condition'] not in done]
    if not remaining:return
    prefix=f"session_parallel_gpu{assignment['gpu']}_"
    ...
    p.update(execution_amendment_sha256=assignment['amendment_sha256'],parallel_runtime_sha256=assignment['runtime_sha256'],assignment_sha256=sha(assignment_path))
    write(session/'CONFIG.json',p,True);write(assignment_path.parent/'SESSION.json',dict(session=str(session)),True);policy=None;heads={};initial=None;head_initial={};completed=[];proc=None;stream=None;forward=None;log=None
    try:
        ...
        for tag in reg['models']:
            ...
            if result['updates']!=1200 or sha(path)!=result['checkpoint_sha256']:raise ValueError('FINAL_CHECKPOINT_IDENTITY')
            net=o.initialize(result['seed']);net.load_state_dict(torch.load(path,map_location='cpu',weights_only=True),strict=True)
            if c.model_identity(net)['sha256']!=result['final_state_sha256']:raise ValueError('LOADED_HEAD_IDENTITY')
        ...
        data=read(run/'DATA.json');families={f['family_id']:f for f in data['raw_families']}
        warmforward=encoder.Forward(model,policy,encoder.RawStore(data),[dict(record_idx=i,t=0,target=0,weight=1.) for i in range(len(data['features']))])
        with torch.inference_mode():warm=encoder.warmup(warmforward,data,config)
        ...
        lookup={(r['instruction'],tuple(x[7:] for x in r['rgb_refs']),tuple(r['executed'])):i for i,r in enumerate(data['features'])}
        old_inputs={}
        ...
        with torch.inference_mode():
            for slot in remaining:
                ...
                memory=net.encode(torch.cat(prefix_features).unsqueeze(0))[0][:,-1] if prefix_features else net.reset(1,'cuda')
                write(folder/'PREFILL_AUDIT.json',dict(prefix_steps=len(history),matched=True,feature_origin_rank=remaining[[r['rank'] for r in remaining].index(rank)-len(contrast)]['rank'],memory_before_cutoff=c.tensor_identity(memory)),True)
                rows=[];alive=True
                references={other:True for other in contrast if other.split('_')[1]==tag.split('_')[1] and (other.startswith('Ours')!=tag.startswith('Ours'))}
                while alive:
                    raw=c.raw_input(window);feature,logits,processed,timing=forward(0);forwards+=1
                    tick=time.perf_counter();before=memory;memory,_=net.update(feature,memory);method=net.action_logits(memory,logits,feature)
                    ...
                    row=dict(decision=len(history)+len(rows),raw=raw,processed=processed,logits=native,method_logits=values,
                             controller_seconds=time.perf_counter()-tick,**selection,**timing)
                    ...
                    if idx is not None:
                        ...
                        cm,_=net.update(cached,before);oldmethod=net.action_logits(cm,cl,cached)
                    ...
                    if len(rows)%20==0:write(assignment_path.parent/'PROGRESS.json',dict(gpu=config['gpu'],rank=rank,condition=current,model=tag,step=row['decision'],completed_slots=len(completed),forwards=forwards,unix=time.time()))
                ...
                if len(contrast)==9:
                    ...
                    for seed in config['seeds']:
                        for arm in ('B1','B2'):
                            left=f'{arm}_{seed}';right=f'Ours_{seed}';audit=prefix_audit(contrast[left],contrast[right])
                    ...
                    write(assignment_path.parent/'PROGRESS.json',dict(gpu=config['gpu'],rank=rank,condition=current,model=tag,step=row['decision'],completed_slots=len(completed),forwards=forwards,unix=time.time(),group_sealed=True))
