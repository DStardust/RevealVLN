"""Compile the complete audited real pool; preserve old CHECK and hold new houses out."""
import hashlib
...
def prepare():
    ...
    # All verified nonduplicate exports; no selection by old or new model results.
    scope = json.loads((HERE / 'POOL_PROTOCOL.json').read_text())
    audit_path = HERE.parent / 'query_semantics_v11/AVAILABLE_FAMILIES_AUDIT.json'
    assert sha(audit_path) == scope['audit_sha256']
    pool = json.loads(audit_path.read_text())
    assert not pool['failures']
    by_source = {row['source']: row for row in pool['rows']}
    sources = [by_source[source] for source in scope['selected_sources']]
    paths = [LINE / row['source'] / 'MANIFEST.json' for row in sources]
    assert all(sha(path) == row['manifest_sha256'] for path, row in zip(paths, sources))
    manifests = [json.loads(path.read_text()) for path in paths]
    split = scope['house_split']
    ...
    for path, manifest in zip(paths, manifests):
        ...
        families.append(dict(family_id=manifest['family_id'], house=house, split=split[house],
            partition=scope['house_partition'][house],
            original_cohort=str(path.parent.relative_to(LINE)) in scope['original_sources'],
            executable_content_sha256=by_source[str(path.parent.relative_to(LINE))]['executable_content_sha256'],
        ...
        audit.append(dict(export=str(path.parent.relative_to(LINE)), manifest_sha256=sha(path),
            house=house, split=split[house], partition=scope['house_partition'][house], original_training_admission=manifest['training_admission'],
    ...
    result = dict(families=families, features=features, contents=contents, query_vocabulary=vocabulary,
        audit=dict(scope='COMPLETE_EXISTING_REAL_SEE2_POOL_DEBUG_ONLY', original_training_admission=False,
            normalization_and_missing_controls_retained=True, independent_blind_test=False,
            house_split=split, house_partition=scope['house_partition'], families=audit, family_count=len(families), causal_input_count=len(features),
