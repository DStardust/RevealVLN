...
def main():
    ...
    if gpu['free_mib']<18000:
        c.write(HERE/'RESOURCE_BLOCK.json',dict(reason='INSUFFICIENT_CURRENT_FREE_MEMORY',
            selected=gpu,required_mib=18000,exclusive_reservation_claimed=False))
        return
    run=HERE/f'features_run_{len(list(HERE.glob("features_run_*")))+1:03d}'
    run.mkdir();(run/'source').mkdir()
    for path in [*HERE.glob('*.py'),HERE/'FEATURE_PROTOCOL.json',HERE/'SCREEN_PROTOCOL.json']:
    ...
    c.write(run/'PREFLIGHT.json',dict(selected=gpu,shared_allowed=True,
        expected_new_forwards=config['new_forwards'],optimizer_updates=0,
    ...
    with (run/'worker.log').open('x') as log:
        try:
            proc=subprocess.Popen([str(LINE/'.envs/q35n_qwen_g2_v1/bin/python3'),'-I','-B',
                str(HERE/'extract_screen.py'),str(run)],cwd=c.ROOT,
