"""Matched fixed actors with a separate real history-fork action stratum."""
...
sys.path.insert(0,str(HERE.parent/'cost_teacher_v13'))
import branch_metrics
c=special.c
...
def train(run,config,special_cache,special_data,natural_cache,natural_data):
    ...
    for seed in config['seeds']:
        ...
        prior=torch.load(HERE.parent/'query_semantics_v11/run_001'/f'INITIAL_{seed}.pt',map_location='cpu',weights_only=True)
        for name,value in initial.items():
            assert torch.equal(value,prior[name]),'INITIALIZATION_CHANGED:'+name
        ...
        for arm in config['arms']:
            ...
            measured['branch_actions']=branch_metrics.evaluate(net,special_cache,special_data,c.read(HERE.parent/'cost_teacher_v13/ACTOR_ADMISSION.json'))
        ...
        del net
    c.write(run/'RESULT.json',dict(status='MATCHED_FORK_BALANCED_ACTOR_TRAINING_COMPLETE',runs=result,
        base_encoder_updates=0,new_qwen_forwards=0,optimizer_updates=len(config['seeds'])*len(config['arms'])*config['steps_per_arm'],
        original_special_training_admission=False,ordinary_labels_unchanged=True,special_action_admission='V13 immutable minimum-tested-PASS masks; add equal-weight per-family mean genuine fork CE for every arm',selected_by_final_check_results=False,
