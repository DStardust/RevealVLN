...
def prepare():
    ...
    prepared=dict(prefixes=prefixes,cells=cells,queries=queries,query_vocabulary=vocabulary,audit=audit)
    # Compiler vocabulary contains tuples; persisted JSON arrays are lists. Bind
    # the complete JSON representation without changing any labels or features.
    return loader,feature_records,json.loads(json.dumps(prepared))
