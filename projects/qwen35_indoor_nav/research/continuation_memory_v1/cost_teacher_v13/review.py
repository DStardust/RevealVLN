...
def main():
    ...
    prior = c.read(HERE/'PRIOR_TEACHER_BRANCH_EVALUATION.json')
    rows = {}
    for key, measured in result['runs'].items():
        ...
        if measured['arm'] != 'N0':
            ...
            assert gradient['action_memory_gradient_norm'] > 0
        ...
        rows[key] = dict(branches=reloaded['summaries'], by_house=by_house,
            ...
            prior_equal_PASS_teacher_branches=prior['runs'][key]['branch_actions']['summaries'] if key in prior['runs'] else None)
    deltas = [rows[f'Ours_{seed}']['branches']['check']['both_correct']-
              rows[f'B2_{seed}']['branches']['check']['both_correct'] for seed in config['seeds']]
    signal = all(d >= 0 for d in deltas) and statistics.mean(deltas) > 0
    ...
    review = dict(status='MATCHED_ACTOR_PILOT_COMPLETE', all_nine_checkpoints_and_branch_actions_read_back=True,
        ...
        runs=rows, means=means, ours_minus_b2_check_both_correct=deltas,
