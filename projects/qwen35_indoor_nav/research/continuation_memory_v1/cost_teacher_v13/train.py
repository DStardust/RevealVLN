"""Matched N0/B2/Ours training with minimum-tested-PASS action teachers."""
...
def train(run,config,special_cache,special_data,natural_cache,natural_data):
    ...
    for seed in config['seeds']:
        ...
        for arm in config['arms']:
            ...
            measured['branch_actions']=branch_metrics.evaluate(net,special_cache,special_data,c.read(HERE/'ACTOR_ADMISSION.json'))
        ...
        del net
    c.write(run/'RESULT.json',dict(status='MATCHED_MINIMUM_TESTED_PASS_TEACHER_COMPLETE',runs=result,
        base_encoder_updates=0,new_qwen_forwards=0,optimizer_updates=len(config['seeds'])*len(config['arms'])*config['steps_per_arm'],
        original_special_training_admission=False,ordinary_labels_unchanged=True,special_action_admission='Minimum actual action cost among tested PASS suffixes; tied causal action conflicts abstain',selected_by_final_check_results=False,
