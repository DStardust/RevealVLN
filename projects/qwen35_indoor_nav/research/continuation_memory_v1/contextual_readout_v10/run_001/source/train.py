"""Matched contextual readout, with a short-window-only control and fixed caches."""
...
def train(run,config,special_cache,special_data,natural_cache,natural_data):
    ...
    batches=[special.tensors(f,'cuda:0') for f in special_data['families']]
    result={}
    for seed in config['seeds']:
        ...
        for arm in config['arms']:
            ...
            for step,(family_index,ordinary_indices) in enumerate(zip(schedule['family_indices'],schedule['ordinary_indices']),1):
                ...
                batch=ordinary_batch([natural_data['records'][i] for i in ordinary_indices],'cuda:0')
        ...
        del net
    c.write(run/'RESULT.json',dict(status='MATCHED_CONTEXTUAL_READOUT_COMPLETE',runs=result,
