...
import common as c
resource=c.load('v10_resource_manager',V5/'launch.py')


def main():
    config=c.read(HERE/'PROTOCOL.json')
    assert c.read(HERE/'CPU_TEST_RESULT.json')['passed']
    prior=c.read(LINE/'closed_loop_bench/ordinary_memory_transfer_v9/RESULT.json')
    assert prior['status']=='VALID_COMPLETE' and prior['complete_quartets']==300,'V9_FIXED_SEEDS_NOT_COMPLETE'
