    ...
    with (V16/'b2_kl_repair_v1/PUBLISH.lock').open('a') as guard:
        ...
        with tarfile.open(archive,'w:gz',compresslevel=4) as bundle:
            for parent,dirs,files in os.walk(run):
                dirs[:]=[d for d in dirs if d not in ('content','features','__pycache__')]
                for name in sorted(files):
                    ...
                    bundle.addfile(info,io.BytesIO(value));entries.append(dict(path=rel,sha256=sha(p),bytes=len(value)))
        parts=[]
        ...
        paths+=parts
        for name in ('PROTOCOL.json','SOURCE_LOCK.json','DATA_AUDIT.json','DATA.json','MODEL_REUSE.json','EVALUATION_REGISTRY.json','RESULT.json','REPORT_ZH.md','ROLLOUTS.csv','STATUS.json','RESOURCES.jsonl'):
            p=run/name
            if p.exists():paths.append(p)
        write(out/'INDEX.json',dict(files=entries,parts=[dict(path=p.name,sha256=sha(p)) for p in parts],
            excluded='Licensed scene/RGB/semantic arrays, environments and base checkpoint stay on server. Frozen heads remain at the already published gpu_v1 paths with MODEL_REUSE hashes.'),True)
        ...
        subprocess.run(['git','commit','-m','Register frozen MONOTONIC independent-house evaluation and evidence'],cwd=ROOT,check=True,stdout=subprocess.DEVNULL)
