...
def registry_value(raw_families, config):
    ...
    return dict(scope='FROZEN_MONOTONIC_NEW_MEMORY_HOUSES',conditions=conditions,models=models,slots=slots,
            ...
            for slot in remaining:
                ...
                if len(contrast)==len(reg['models']):
                    ...
                    for seed in config['seeds']:
                        for arm in ('DIRECT',):
                            left=f'{arm}_{seed}';right=f'MONOTONIC_{seed}';audit=prefix_audit(contrast[left],contrast[right])
