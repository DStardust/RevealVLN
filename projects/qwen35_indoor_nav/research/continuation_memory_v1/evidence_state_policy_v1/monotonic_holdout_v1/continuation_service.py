    ...
    try:
        for line in stream:
            ...
            if request.get('op')=='reset':
                ...
                ref=family['traces'][condition['history_id']+'__direct_stop']
                if sha(LINE/ref['path'])!=ref['sha256']:raise ValueError('REGISTERED_PREFIX_TRACE_CHANGED')
                reference=read(LINE/ref['path'])
