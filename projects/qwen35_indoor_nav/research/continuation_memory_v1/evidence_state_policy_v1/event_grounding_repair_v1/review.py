...
def paired(rows):
    ...
    for condition,seed in keys:
        a=indexed[condition,seed,'ORIGINAL'];b=indexed[condition,seed,'EVENT'];key=[condition,seed]
    ...
    a=metric([r for r in rows if r['arm']=='ORIGINAL']);b=metric([r for r in rows if r['arm']=='EVENT'])
...
def main(run):
    ...
    result=dict(status='VALID_COMPLETE_DEV_EVENT_GROUNDING' if complete else 'INCOMPLETE_DEV_EVENT_GROUNDING',complete=len(groups)*len(reg['models']),planned=len(rows),
        ...
        supports_dev_repair_signal=signal,method_adopted=False,new_updates=7200,event_probe_updates=3200,base_updates=0,raw_arrays_verified=len(arrays),
        ...
        limitation='Exposed DEV1 house; same MONOTONIC architecture and original trajectory pool, only unique-input house/role-uniform event BCE differs. Stationary SEE2, no independent test or natural VLN-CE claim.')
    ...
    lines=[result['status'],'',f"完整续接 {result['complete']}/{result['planned']}；完整六模型组 {len(groups)}/{len(reg['conditions'])}。本轮6模型各1200次更新，共7200次；底模更新0。",
    ...
    lines.extend(['',f"EVENT−ORIGINAL 主终点差值：{main['delta']:.2%}；未知分配识别界 [{main['delta_lower']:.2%}, {main['delta_upper']:.2%}]。这不是置信区间。",
        ...
        f"缺失历史主任务配对差：{missing['delta']:.2%}，识别界[{missing['delta_lower']:.2%},{missing['delta_upper']:.2%}]；事件留一屋诊断与完整训练事件复核另报，不将读出指标当闭环收益。",'本轮不自动采用、不追加训练；普通VLN-CE收益与算法新颖性仍需另外证据。',result['limitation']])
