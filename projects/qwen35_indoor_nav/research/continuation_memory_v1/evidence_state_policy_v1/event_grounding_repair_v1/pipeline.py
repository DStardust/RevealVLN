"""Independent event screen, matched training, full DEV evaluation and publication."""
import argparse
import fcntl
import os
...
def main():
    ...
    try:
        ...
        if not (run/'RESULT.json').exists():
            ...
            while any(monitor.gpu_snapshot(d)['free_mib']<cfg['min_start_gpu_gib']*1024 for d in cfg['devices']):
                ...
                time.sleep(2)
            if not (run/'EVENT_PREFLIGHT.json').exists():
                jobs=[f'{arm}_{fold}' for fold in range(4) for arm in cfg['arms']]
                execute(run,cfg,'event_probe',[(d,jobs[i::len(cfg['devices'])]) for i,d in enumerate(cfg['devices']) if jobs[i::len(cfg['devices'])]])
                execute(run,cfg,'probe_review',[(cfg['devices'][0],[])],gpu=False)
            if not read(run/'EVENT_PREFLIGHT.json')['advance_full_policy']:
                raise RuntimeError('EVENT_REPAIR_PREFLIGHT_NOT_SUPPORTED')
            ...
            if not (run/'EVENT_DIAGNOSIS.json').exists():execute(run,cfg,'event_diagnosis',[(cfg['devices'][0],[])],gpu=False)
            from evaluate_continuations import registry,admitted
            reg=registry(run);available=set(range(len(reg['conditions'])))
    ...
    if error:
        if error['reason']=="RuntimeError('EVENT_REPAIR_PREFLIGHT_NOT_SUPPORTED')":
            write(run/'STATUS.json',dict(status='EVENT_PREFLIGHT_NOT_SUPPORTED',reason=error['reason'],formal_updates=0,formal_rollouts=0))
            execute(run,cfg,'publish',[(cfg['devices'][0],[])],gpu=False)
            write(run/'STATUS.json',dict(status='EVENT_PREFLIGHT_NOT_SUPPORTED',formal_updates=0,formal_rollouts=0,upload_verified=True));return
