...
def registry_value(raw_families, config):
    ...
    return dict(scope='EXPOSED_DEV_EVENT_GROUNDING_V1',conditions=conditions,models=models,slots=slots,
            ...
            for slot in remaining:
                ...
                if len(contrast)==len(reg['models']):
                    ...
                    for seed in config['seeds']:
                        for arm in ('ORIGINAL',):
                            left=f'{arm}_{seed}';right=f'EVENT_{seed}';audit=prefix_audit(contrast[left],contrast[right])
