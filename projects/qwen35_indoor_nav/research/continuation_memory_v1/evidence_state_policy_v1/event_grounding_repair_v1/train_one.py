"""Matched MONOTONIC training; only the event supervision measure changes."""
import argparse
import json
import random
...
def main(args):
    ...
    immutable(output/'INITIAL.json',dict(seed=args.seed,state_sha256=initial,source='Same original EvidencePolicy(MONOTONIC) initialization'))
    ...
    event_pool=event_module.Pool(read(run/'EVENT_POOL.json')['records'])
    ...
    record = dict(status='CPU_SMOKE_COMPLETE' if args.device == 'cpu' else 'TRAINING_COMPLETE',
                  ...
                  note='CPU updates are interface checks, excluded from full study; same MONOTONIC initialization/architecture, only event BCE supervision measure differs; all original action labels retained.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', choices=('ORIGINAL','EVENT'), required=True); parser.add_argument('--seed', type=int, default=1209)
