...
def main(run):
    ...
    with (V16/'b2_kl_repair_v1/PUBLISH.lock').open('a') as guard:
        ...
        paths+=list((run/'event_probe').glob('*/FINAL.pt'))+list((run/'event_probe').glob('*/INITIAL.pt'))

        for name in ('PROTOCOL.json','SOURCE_LOCK.json','DATA_AUDIT.json','DATA.json','EVENT_POOL.json','EVENT_PREFLIGHT.json','EVENT_DIAGNOSIS.json','EVENT_AUDIT.json','TRAINING_REVIEW.json','EVALUATION_REGISTRY.json','RESULT.json','REPORT_ZH.md','ROLLOUTS.csv','STATUS.json','RESOURCES.jsonl'):
            p=run/name
            if p.exists():paths.append(p)
        write(out/'INDEX.json',dict(files=entries,parts=[dict(path=p.name,sha256=sha(p)) for p in parts],
            excluded='Licensed scene/RGB/semantic arrays, environments and base checkpoint stay on server. All completed FINAL/INITIAL policy and probe heads are included; optimizer/RNG checkpoints stay local, hashes/binding/logs are included. Raw arrays and feature caches stay local.'),True)
        ...
        subprocess.run(['git','commit','-m','Evaluate unique-input house-balanced event supervision repair'],cwd=ROOT,check=True,stdout=subprocess.DEVNULL)
