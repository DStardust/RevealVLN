    ...
    try:
        ...
        with torch.inference_mode():
            for slot in remaining:
                ...
                rows=[];alive=True
                references={other:True for other in contrast if other.split('_')[1]==tag.split('_')[1] and (other.startswith('REVISE_')!=tag.startswith('REVISE_'))}
                while alive:
                ...
                if len(contrast)==9:
                    ...
                    for seed in config['seeds']:
                        for arm in ('DIRECT','MONOTONIC'):
                            left=f'{arm}_{seed}';right=f'REVISE_{seed}';audit=prefix_audit(contrast[left],contrast[right])
