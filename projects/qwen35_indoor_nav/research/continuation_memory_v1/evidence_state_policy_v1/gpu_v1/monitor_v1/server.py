"""Read-only, localhost dashboard for the seven-GPU evidence-state study. Standard library only."""
import argparse
...
class Reader:
    ...
    def snapshot(self):
        ...
        for endpoint in ('main','control'):
            for baseline in ('DIRECT','MONOTONIC'):
                ...
                for (condition,seed,arm),label in values.items():
                    if arm!='REVISE' or registry['conditions'][condition]['endpoint']!=endpoint:continue
        ...
        live_progress={}
        for path in (run/'train').glob('*/PROGRESS.json'):
            r=dict(read(path),tag=path.parent.name,seconds=path.stat().st_mtime);live_progress[r['tag']]=r
        training_rows=[]
        for index,tag in enumerate(registry['models']):
            ...
            training_rows.append(dict(tag=tag,gpu=cfg['devices'][index%len(cfg['devices'])]['gpu'],step=step,planned=cfg['steps_per_arm'],
                complete=bool(result),loss=recent.get('loss'),auxiliary=recent.get('state_bce'),gradient_norm=sum(v*v for v in recent.get('gradients',{}).values())**.5 if recent.get('gradients') else None))
        workers=[];progresses=[]
        for w in status.get('workers',[]):
            if w['device'] is None:continue
            gpu=w['gpu'];assigned=[r for r in training_rows if r['gpu']==gpu and r['step'] and not r['complete']]
            tp=live_progress.get(assigned[0]['tag'],{}) if assigned else {};ep=optional(run/f'EVALUATION_PROGRESS_{gpu}.json',{})
...
def main():
    ...
    reader = Reader(PILOT / 'runs/gpu_001', PILOT / 'standalone_jobs/evidence-state-gpu-20260921-02')
    if args.once:
