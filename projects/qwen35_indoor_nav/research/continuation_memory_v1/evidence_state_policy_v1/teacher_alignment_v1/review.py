...
def paired(rows):
    ...
    for condition,seed in keys:
        a=indexed[condition,seed,'ORIGINAL'];b=indexed[condition,seed,'ALIGNED'];key=[condition,seed]
    ...
    a=metric([r for r in rows if r['arm']=='ORIGINAL']);b=metric([r for r in rows if r['arm']=='ALIGNED'])
...
def main(run):
    ...
    result=dict(status='VALID_COMPLETE_DEV_TEACHER_ALIGNMENT' if complete else 'INCOMPLETE_DEV_TEACHER_ALIGNMENT',complete=len(groups)*len(reg['models']),planned=len(rows),
        ...
        supports_dev_repair_signal=signal,method_adopted=False,new_updates=7200,base_updates=0,raw_arrays_verified=len(arrays),
        ...
        limitation='Exposed DEV1 house; same MONOTONIC architecture and original trajectory pool, only certified teacher action targets/masks differ. Stationary SEE2, no independent test or natural VLN-CE claim.')
    ...
    lines.extend(['',f"ALIGNED−ORIGINAL 主终点差值：{main['delta']:.2%}；未知分配识别界 [{main['delta_lower']:.2%}, {main['delta_upper']:.2%}]。这不是置信区间。",
        ...
        f"缺失历史主任务配对差：{missing['delta']:.2%}，识别界[{missing['delta_lower']:.2%},{missing['delta_upper']:.2%}]；原有12处跨变体运动教师冲突另报，不宣称已修复。",'本轮不自动采用、不追加训练；普通VLN-CE收益与算法新颖性仍需另外证据。',result['limitation']])
