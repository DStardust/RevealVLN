"""Matched MONOTONIC training with a certified teacher-action overlay only."""
import argparse
import json
import random
...
def main(args):
    ...
    data = read(run / ('DATA_ALIGNED.json' if args.mode=='ALIGNED' else 'DATA.json'))
    ...
    record = dict(status='CPU_SMOKE_COMPLETE' if args.device == 'cpu' else 'TRAINING_COMPLETE',
                  ...
                  note='CPU updates are interface checks, excluded from full study; same MONOTONIC initialization/architecture, only teacher action supervision differs.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', choices=('ORIGINAL','ALIGNED'), required=True); parser.add_argument('--seed', type=int, default=1209)
