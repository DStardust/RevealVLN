"""Independent bounded certification, matched training, full DEV evaluation and publication."""
import argparse
import fcntl
import os
...
def main():
    ...
    try:
        ...
        plans=read(run/'TEACHER_PLAN.json')['plans']
        for i,d in enumerate(cfg['devices']):
            immutable(run/'devices'/f"gpu_{d['gpu']}.json",dict(d,
                collection_indices=list(range(len(plans)))[i::len(cfg['devices'])],
        ...
        if not (run/'RESULT.json').exists():
            ...
            while any(monitor.gpu_snapshot(d)['free_mib']<cfg['min_start_gpu_gib']*1024 for d in cfg['devices']):
                ...
                time.sleep(2)
            if not (run/'BUILD_RESULT.json').exists():
                execute(run,cfg,'collect',[(d,[]) for d in cfg['devices']])
                execute(run,cfg,'build_data',[(cfg['devices'][0],[])],gpu=False)
