"""Read-only physical teacher certification, six training jobs and sealed DEV groups."""
...
def main():
    ...
    def snapshot():
        ...
        plan=read(run/'TEACHER_PLAN.json') if (run/'TEACHER_PLAN.json').exists() else {}
        value['teacher']=dict(planned=len(plan.get('plans',[])),certified=len(list((run/'certificates').glob('*/CERTIFICATE.json'))),changed_rows=len(plan.get('overlays',[])))
