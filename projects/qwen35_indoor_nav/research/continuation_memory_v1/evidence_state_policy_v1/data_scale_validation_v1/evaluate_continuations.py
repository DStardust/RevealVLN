...
def registry_value(raw_families, config):
    ...
    return dict(scope='FROZEN_DATA_SCALE_EXPOSED_HOLDOUT',conditions=conditions,models=models,slots=slots,
            ...
            for slot in remaining:
                ...
                if len(contrast)==len(reg['models']):
                    ...
                    for seed in config['seeds']:
                        for arm in ('OLD',):
                            left=f'{arm}_{seed}';right=f'EXPANDED_{seed}';audit=prefix_audit(contrast[left],contrast[right])
