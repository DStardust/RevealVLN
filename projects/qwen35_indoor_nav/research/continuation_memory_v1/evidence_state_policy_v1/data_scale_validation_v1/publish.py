...
def main(run):
    ...
    with (V16/'b2_kl_repair_v1/PUBLISH.lock').open('a') as guard:
        ...
        with tarfile.open(archive,'w:gz',compresslevel=4) as bundle:
            for parent,dirs,files in os.walk(run):
                ...
                for name in sorted(files):
                    p=Path(parent)/name
                    if p.suffix not in prior.TEXT or '.tmp' in name or name in ('TRAIN_DATA.json','WARMUP_DATA.json') or 'families' in Path(parent).parts:continue
        ...
        paths+=parts
        paths+=list((run/'train').glob('*/FINAL.pt'))+list((run/'train').glob('*/INITIAL.pt'))

        for name in ('PROTOCOL.json','SOURCE_LOCK.json','DATA_AUDIT.json','SCHEDULES.json','SHARED_FIT_WEIGHTS.json','EXPOSURE_SCHEDULE.json','PREPARED.json','TRAINING_REVIEW.json','EVALUATION_REGISTRY.json','RESULT.json','REPORT_ZH.md','ROLLOUTS.csv','STATUS.json','RESOURCES.jsonl'):
            p=run/name
            if p.exists():paths.append(p)
        write(out/'INDEX.json',dict(files=entries,parts=[dict(path=p.name,sha256=sha(p)) for p in parts],
            excluded='Licensed scene/RGB/semantic arrays, environments and base checkpoint stay on server. All six FINAL/INITIAL OLD and EXPANDED heads are included; optimizer/RNG checkpoints stay local, hashes/binding/logs are included. Raw arrays and feature caches stay local.'),True)
        ...
        subprocess.run(['git','commit','-m','Validate expanded SEE2 data with matched MONOTONIC training and heldout rollouts'],cwd=ROOT,check=True,stdout=subprocess.DEVNULL)
        ...
        for attempt in range(2):
            if check.returncode==0 and check.stdout.split() and check.stdout.split()[0]==commit:break
            check=remote(['git','ls-remote','--heads','origin','refs/heads/'+branch])
