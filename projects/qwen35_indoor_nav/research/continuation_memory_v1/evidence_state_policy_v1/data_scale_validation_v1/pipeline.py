"""Independent fixed-action-path repair, complete paired evaluation and publication."""
import argparse
import fcntl
import os
...
disk_module=load('scale_disk_watch',PARENT/'event_data_scale20_v2/disk_watch_r2.py')
...
def execute(run,cfg,stage,assignments,gpu=True):
    ...
    errors=None;disk=disk_module.DiskWatch(run,monitor,write,append,cfg['artifact_gib']*2**30)
    try:
        ...
        while True:
            ...
            if gpu and hours>cfg['gpu_session_hours']:raise RuntimeError('AGGREGATE_GPU_BUDGET')
            disk.tick()
...
def main():
    ...
    write(run/'STATUS.json',dict(status='RUNNING',stage='preflight',registered_optimizer_updates=7200))
    ...
    try:
        ...
        execute(run,cfg,'tests',[(cfg['devices'][0],[])],gpu=False)
        for i,d in enumerate(cfg['devices']):
            immutable(run/'devices'/f"gpu_{d['gpu']}.json",dict(d,
        ...
        if not (run/'RESULT.json').exists():
            ...
            while any(monitor.gpu_snapshot(d)['free_mib']<cfg['min_start_gpu_gib']*1024 for d in cfg['devices']):
                ...
                time.sleep(2)
            if not (run/'features/FEATURE_RESULT.json').exists():
                from math import ceil
                expected=ceil(read(run/'PREPARED.json')['feature_windows']/1024)
                def feature_parts():
                    values=set()
                    for p in (run/'features').glob('session_*/FEATURES_PART_*.json'):
                        seal=p.parent/'STATE_SEAL.json'
                        if seal.exists() and read(seal)['base_unchanged']:values.add(read(p)['start'])
                    return values
                while len(feature_parts())<expected:
                    before=len(feature_parts())
                    assignments=[(d,[]) for d in cfg['devices'] if any(i*1024 not in feature_parts() and i%len(cfg['devices'])==d['gpu'] for i in range(expected))]
                    execute(run,cfg,'features',assignments)
                    if len(feature_parts())==before:raise RuntimeError('NO_FEATURE_PROGRESS')
                execute(run,cfg,'merge_features',[(cfg['devices'][0],[])],gpu=False)
            if not (run/'TRAINING_REVIEW.json').exists():
            ...
            while available-set(admitted(run,reg)):
                ...
                if set(admitted(run,reg))==done:raise RuntimeError('NO_COMPLETE_EVALUATION_PROGRESS')
    ...
    write(run/'STATUS.json',dict(status=result['status'],complete=result['complete'],planned=result['planned'],registered_optimizer_updates=7200))
    ...
    write(run/'STATUS.json',dict(status=result['status'],complete=result['complete'],planned=result['planned'],upload_verified=True,registered_optimizer_updates=7200))
