...
def paired(rows):
    ...
    for condition,seed in keys:
        a=indexed[condition,seed,'OLD'];b=indexed[condition,seed,'EXPANDED'];key=[condition,seed]
    ...
    a=metric([r for r in rows if r['arm']=='OLD']);b=metric([r for r in rows if r['arm']=='EXPANDED'])
...
def main(run):
    ...
    complete=len(groups)==sum(x['available'] for x in reg['conditions']);signal=complete and main['delta_lower']>0 and positive_houses>=3 and positive_seeds>=2
    result=dict(status='VALID_COMPLETE_CONTROLLED_HOLDOUT' if complete else 'INCOMPLETE_PLANNED_HOLDOUT',complete=len(groups)*len(reg['models']),planned=len(rows),
        ...
        supports_limited_data_scale_signal=signal,method_adopted=False,new_updates=7200,available_conditions=sum(x['available'] for x in reg['conditions']),planned_conditions=len(reg['conditions']),complete_available=complete,raw_arrays_verified=len(arrays),
        ...
        limitation='Four previously exposed heldout development houses, shared instruction templates and stationary SEE2 task. Base-training exposure unresolved; no natural VLN-CE, deployment or paper acceptance claim.')
    ...
    lines=[result['status'],'',f"完整续接 {result['complete']}/{result['planned']}；完整六模型组 {len(groups)}/{len(reg['conditions'])}。新增训练更新 7200（两组×三种子×1200）；基座更新0。",
    ...
    lines.extend(['',f"EXPANDED−OLD 主终点差值：{main['delta']:.2%}；未知分配识别界 [{main['delta_lower']:.2%}, {main['delta_upper']:.2%}]。这不是置信区间。",
        f"方向为正的房屋 {positive_houses}/4、种子 {positive_seeds}/3；有限数据扩充信号：{signal}。",'',
