"""Single EXPANDED checkpoint: real autonomous forwards and atomic condition seals."""
import json
import os
...
def registry_value(raw_families, config):
    lookup={(f['house'],f['parent_slot'],f['stratum']):f for f in raw_families}
    if len(lookup)!=len(raw_families):raise ValueError('DUPLICATE_FAMILY_SLOT')
    conditions=[]
    for house in config['houses']:
        for parent in range(config['families_per_house']):
            for stratum in ('terminal_present','terminal_absent'):
                f=lookup.get((house,parent,stratum))
                for h in ('seen','seen_sham','missing','missing_sham'):
                    for task in ('task_A','task_T'):
                        conditions.append(dict(family_id=f['family_id'] if f else None,house=house,
                            history_id=h,task_id=task,endpoint='main' if task=='task_A' else 'control',
                            parent_family_id=f['parent_family_id'] if f else None,parent_slot=parent,
                            stratum=stratum,available=f is not None))
    models=[f'{a}_{seed}' for seed in config['seeds'] for a in config['arms']]
    slots=[]
    ...
    return dict(scope='EXPANDED_LONGTRAIN_EXPOSED_DEVELOPMENT_CURVE',conditions=conditions,models=models,slots=slots,
                expected_prefix_comparisons=0,models_per_condition=len(models),
...
def main(run):
    ...
    try:
        ...
        if initial['sha256']!=read(Path(config['source_run'])/'features/FEATURE_RESULT.json')['base_state_sha256']:raise ValueError('RUNTIME_BASE_DIFFERS_FROM_CACHE')
        ...
        for tag in reg['models']:
            ...
            if result['updates']!=config['evaluated_step'] or sha(path)!=result['checkpoint_sha256']:raise ValueError('FINAL_CHECKPOINT_IDENTITY')
        ...
        cache=torch.load(Path(config['source_run'])/'features/FEATURES.pt',map_location='cpu',weights_only=True)
        lookup={(r['instruction'],tuple(x[7:] for x in r['rgb_refs']),tuple(r['executed'])):i for i,r in enumerate(golden['features'])}
        old_inputs={}
        for f in (Path(config['source_run'])/'features').glob('session_*/FEATURES_INPUTS_*.jsonl'):
            if not (f.parent/'STATE_SEAL.json').exists():continue
            for row in c.records(f):
                if row['index']<len(golden['features']):old_inputs[row['index']]=row['processed']
