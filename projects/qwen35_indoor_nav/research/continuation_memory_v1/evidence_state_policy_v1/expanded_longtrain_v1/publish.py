...
def main(run):
    ...
    with (V16/'b2_kl_repair_v1/PUBLISH.lock').open('a') as guard:
        ...
        paths+=parts
        paths+=list((run/'checkpoints').glob('STEP_*/HEAD.pt'))

        for name in ('PROTOCOL.json','SOURCE_LOCK.json','CHECKPOINT_PLAN.json','LEARNING_CURVE.jsonl','SCHEDULE.json','SHARED_FIT_WEIGHTS.json','EXPOSURE_SCHEDULE.json','PREPARED.json','TRAINING_REVIEW.json','EVALUATION_REGISTRY.json','RESULT.json','REPORT_ZH.md','ROLLOUTS.csv','STATUS.json','RESOURCES.jsonl'):
            p=run/name
            if p.exists():paths.append(p)
        write(out/'INDEX.json',dict(files=entries,parts=[dict(path=p.name,sha256=sha(p)) for p in parts],
            excluded='Licensed scene/RGB/semantic arrays, environments and base checkpoint stay on server. EXPANDED milestone head weights are included; optimizer/RNG checkpoints stay local, hashes/binding/logs are included. Raw arrays and feature caches stay local.'),True)
        ...
        subprocess.run(['git','commit','-m','Run expanded-only 100k-step continuation with fixed 5k checkpoint evaluation'],cwd=ROOT,check=True,stdout=subprocess.DEVNULL)
