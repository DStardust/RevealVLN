...
def main():
 p=argparse.ArgumentParser();p.add_argument('--run',required=True,type=Path);p.add_argument('--long-run',required=True,type=Path);a=p.parse_args();reader=reader_module.Reader(a.run)
 def snapshot():
  ...
  long_status=read(a.long_run/'STATUS.json') if (a.long_run/'STATUS.json').exists() else {}
  long_progress=read(a.long_run/'train/PROGRESS.json') if (a.long_run/'train/PROGRESS.json').exists() else dict(step=1200,target=100000)
  checkpoints=[]
  for step in range(5000,100001,5000):
   folder=a.long_run/'evaluations'/f'STEP_{step:06d}';result=read(folder/'RESULT.json') if (folder/'RESULT.json').exists() else None
   groups=list((folder/'evaluate').glob('session_*/STATE_SEAL_???.json'))
   row=dict(step=step,saved=(a.long_run/'checkpoints'/f'STEP_{step:06d}'/'COMMIT.json').exists(),complete=result['complete'] if result else len(groups),planned=256,reviewed=result is not None,main=None,control=None)
   if result:
    for m in result['metrics']:
     if m['house'] is None:row[m['endpoint']]=m
   checkpoints.append(row)
  v['long_training']=dict(status=long_status,progress=long_progress,checkpoints=checkpoints)
  return v
