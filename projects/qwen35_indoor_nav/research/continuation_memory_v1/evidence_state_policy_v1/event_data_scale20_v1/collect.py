...
from shared import *
from quality import audit_family, candidate_positions, isolated_position
...
def main(run):
    ...
    store=ContentStore(out/'content',cfg['per_house_artifact_gib']*2**30);began=time.monotonic();audited={}
    old_positions=read(HERE/'HOUSE_MANIFEST.json')['old_positions'].get(house_id,[])
    def check():
    ...
    def progress(stage):
        write(out/'STATUS.json',dict(stage=stage,parents=len(list(out.glob('position_*/FAMILY.json'))),
              ...
              target_parents=cfg['families_per_house'],seconds=time.monotonic()-began,base_model_loaded=False,content_bytes=store.bytes,unix=time.time()))
    ...
    try:
        ...
        if not (out/'POSITIONS.json').exists():immutable(out/'POSITIONS.json',candidate_positions(backend,cfg,old_positions))
        for index,position in enumerate(read(out/'POSITIONS.json')):
            ...
            if not isolated_position(position,[f['initial_position'] for f in parents],cfg['parent_min_distance_m']):
                log(stage='SPATIAL_DUPLICATE',position=index);continue
            ...
            plans=transport.plans(read(folder/'PANORAMA.json'),groups,backend.eligible,delay)
            used_roles=[f['roles'] for f in parents]
            def role_count(plan):
                return sum(v[k]==roles[plan[k]] for v in used_roles for k in ('anchor','terminal'))
            plans=sorted(plans,key=lambda plan:(role_count(plan),digest(plan)))[:cfg['plans_per_position']]
            ...
            for j,plan in enumerate(plans):
                ...
                instruction=f'First see {a} in two consecutive observations, then see {t} in two consecutive observations, and stop immediately.'
                ...
                family_id='SCALE20_'+digest([house_id,position,plan])[:20]
                family=dict(family_id=family_id,parent_family_id=family_id,parent_slot=len(parents),house=house_id,split='FIT',
                    stratum='terminal_present',scene=house['scene'],initial_position=position,initial_yaw=0,compiler=spec,roles=roles2,
                    terminal_instruction=f'See {t} in two consecutive observations and stop immediately.',
                    ...
                    scope='Real stationary SEE2 training expansion; not ordinary translational VLN or independent generalization evidence')
                audit=audit_family(family,present.certify,audited);immutable(folder/'AUDIT.json',audit)
                family['training_admission']='CONTROLLED_SEE2_ARRAY_CERTIFIED_FIT';family['audit']=dict(path=str((folder/'AUDIT.json').relative_to(LINE)),sha256=sha(folder/'AUDIT.json'))
                immutable(folder/'FAMILY.json',family);parents.append(family);log(stage='CERTIFIED_PRESENT',family=family_id);progress('COLLECT_PRESENT');break
        ...
        for parent in parents:
            ...
            for j,plan in enumerate(plans):
                ...
                family=dict(parent,family_id='SCALE20_ABSENT_'+digest([parent['family_id'],plan])[:20],stratum='terminal_absent',
                    histories=plan['histories'],suffixes=plan['suffixes'],certificate=certificate,
                    traces={k:dict(path=str((trial/(k+'.json')).relative_to(LINE)),sha256=sha(trial/(k+'.json'))) for k in traces})
                audit=audit_family(family,absent.certify,audited);immutable(folder/'AUDIT.json',audit)
                family['audit']=dict(path=str((folder/'AUDIT.json').relative_to(LINE)),sha256=sha(folder/'AUDIT.json'))
