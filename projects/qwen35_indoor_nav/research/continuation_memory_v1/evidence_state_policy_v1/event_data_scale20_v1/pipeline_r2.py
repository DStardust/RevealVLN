...
def main():
    ...
    amendment=read(HERE/'RUNTIME_AMENDMENT_R2.json');verify_lock(amendment)
    if amendment['original_source_lock_sha256']!=sha(run/'SOURCE_LOCK.json'):raise ValueError('ORIGINAL_LOCK_CHANGED')
    immutable(run/'RUNTIME_AMENDMENT_R2.json',amendment)
    if (run/'RESULT.json').exists():raise ValueError('ALREADY_COMPLETE_NO_REPLAY')
    if not (run/'STOPPED_BEFORE_R2.json').exists():immutable(run/'STOPPED_BEFORE_R2.json',read(run/'STATUS.json'))
    if not (run/'REPORT_BEFORE_R2.md').exists() and (run/'REPORT_ZH.md').exists():(run/'REPORT_BEFORE_R2.md').write_bytes((run/'REPORT_ZH.md').read_bytes())
    ...
    try:
        ...
        immutable(run/f'PLACEHOLDER_BEFORE_{time.time_ns()}.json',resource.verify_placeholders())
        ...
        while any(monitor.gpu_snapshot(d)['free_mib']<cfg['min_start_gpu_gib']*1024 for d in cfg['devices']):
            ...
            time.sleep(2)
        pending=[h for h in cfg['houses'] if not (run/'collect'/h/'DATASET.json').exists()]
        for d in cfg['devices']:
            if pending:launch(pending.pop(0),d)
        while any(w['ended'] is None for w in workers):
            now=time.monotonic();views=[];hours=previous;free_devices=[]
            ...
            for device in free_devices:
                if pending:launch(pending.pop(0),device)
            if now-last_counts>15:
                stats,_=counts(run);write(run/'COUNTS.json',stats);last_counts=now
            write(run/'STATUS.json',dict(status='RUNNING',runtime_revision='R2_NONBLOCKING_DISK',stage='real_physical_collection',unix=time.time(),workers=views,
                 pending_houses=pending,gpu_hours=hours,counts=stats,training=False))
    ...
    if not error:
        result=dict(status='DATA_SCALE_TARGET_COMPLETE' if stats['new_parents']>=800 else 'PHYSICAL_SHORTFALL',
