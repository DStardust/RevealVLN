...
def main():
    ...
    try:
        ...
        while any(w['ended'] is None for w in workers):
            ...
            write(run/'STATUS.json',dict(status='RUNNING',stage='real_physical_collection',unix=time.time(),workers=views,
            ...
            if now-last_disk>300:
                scan=subprocess.run(['du','-sb',str(run)],text=True,capture_output=True,timeout=120)
                if not scan.stdout.split():raise RuntimeError('DISK_SCAN_FAILED')
                size=int(scan.stdout.split()[0]);last_disk=now
                write(run/'DISK.json',dict(bytes=size,cap_bytes=cfg['artifact_gib']*2**30,unix=time.time()))
