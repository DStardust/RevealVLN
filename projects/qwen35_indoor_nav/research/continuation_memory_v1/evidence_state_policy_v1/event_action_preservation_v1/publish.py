...
def main(run):
    ...
    with (V16/'b2_kl_repair_v1/PUBLISH.lock').open('a') as guard:
        ...
        for name in ('PROTOCOL.json','SOURCE_LOCK.json','DATA_AUDIT.json','DATA.json','PRESERVATION_POOL.json','PRESERVATION_DIAGNOSIS.json','PRESERVATION_AUDIT.json','TRAINING_REVIEW.json','EVALUATION_REGISTRY.json','RESULT.json','REPORT_ZH.md','ROLLOUTS.csv','STATUS.json','RESOURCES.jsonl'):
            p=run/name
            if p.exists():paths.append(p)
        write(out/'INDEX.json',dict(files=entries,parts=[dict(path=p.name,sha256=sha(p)) for p in parts],
            excluded='Licensed scene/RGB/semantic arrays, environments and base checkpoint stay on server. All six FINAL/INITIAL reference and repaired heads are included; optimizer/RNG checkpoints stay local, hashes/binding/logs are included. Raw arrays and feature caches stay local.'),True)
        ...
        subprocess.run(['git','commit','-m','Validate event-only repair with frozen ORIGINAL action and memory weights'],cwd=ROOT,check=True,stdout=subprocess.DEVNULL)
