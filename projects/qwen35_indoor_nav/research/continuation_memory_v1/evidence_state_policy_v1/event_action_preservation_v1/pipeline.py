...
def main():
    ...
    write(run/'STATUS.json',dict(status='RUNNING',stage='preflight',registered_optimizer_updates=600))
    ...
    try:
        ...
        if not (run/'RESULT.json').exists():
            ...
            if not (run/'TRAINING_REVIEW.json').exists():
                tags=[f'REPAIR_{s}' for s in cfg['seeds']]
            ...
            if not (run/'PRESERVATION_DIAGNOSIS.json').exists():execute(run,cfg,'event_diagnosis',[(cfg['devices'][0],[])],gpu=False)
    ...
    write(run/'STATUS.json',dict(status=result['status'],complete=result['complete'],planned=result['planned'],registered_optimizer_updates=600))
    ...
    write(run/'STATUS.json',dict(status=result['status'],complete=result['complete'],planned=result['planned'],upload_verified=True,registered_optimizer_updates=600))
