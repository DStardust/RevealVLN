...
def main(run):
    ...
    if index['source_data_sha256'] != sha(CPU/'DATA.json'):
        raise ValueError('PRESERVATION_INDEX_SOURCE_CHANGED')
                ...
                for role in range(2):
                    if masks[role]:
                        ...
                        if key in expected and expected[key] != value:
                            raise ValueError('INCONSISTENT_INPUT_PRESERVATION_LABEL')
                        expected[key] = value
    ...
    if actual != expected:
        raise ValueError('AUDITED_PRESERVATION_POOL_DIFFERS_FROM_ORIGINAL_LABELS')
    for row in records:
    ...
    immutable(run/'PRESERVATION_POOL.json', dict(records=records, source=str(source), source_sha256=sha(source),
    ...
    immutable(run/'PRESERVATION_AUDIT.json', dict(fold_houses=houses, unique_fit_input_role_labels=len(records),
    ...
    for name in ('PRESERVATION_POOL.json', 'PRESERVATION_AUDIT.json', 'PROTOCOL.json', 'SOURCE_LOCK.json'):
        files[str(run/name)] = sha(run/name)
    for seed in cfg['seeds']:
        source = Path(cfg['source_original_run'])/'train'/f'ORIGINAL_{seed}'
        record = read(source/'RESULT.json')
        if sha(source/'FINAL.pt') != record['checkpoint_sha256'] or record['updates'] != 1200:
            raise ValueError('SOURCE_ORIGINAL_IDENTITY')
        files[str(source/'FINAL.pt')] = record['checkpoint_sha256']
        files[str(source/'RESULT.json')] = sha(source/'RESULT.json')
        target = run/'train'/f'ORIGINAL_{seed}'
        copy_bound(source/'FINAL.pt', target/'FINAL.pt')
        copy_bound(source/'FINAL.pt', target/'INITIAL.pt')
        immutable(target/'RESULT.json', dict(status='FROZEN_REFERENCE', mode='ORIGINAL', seed=seed,
            updates=0, source_updates=1200, initial=record['final'], final=record['final'], parameters_changed=False,
            base_updates=0, base_loaded=False, seconds=0, checkpoint_sha256=record['checkpoint_sha256'],
            source=str(source), reference_copied=True))
        immutable(target/'PROGRESS.json', dict(step=0, target=0, mode='ORIGINAL', seed=seed, frozen=True))
