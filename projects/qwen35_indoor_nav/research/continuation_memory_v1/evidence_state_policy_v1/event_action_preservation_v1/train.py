...
def main(run):
    ...
    for tag in os.environ['B2_TRAIN_TAGS'].split(','):
        ...
        if (target/'RESULT.json').exists():
            ...
            if result['updates']!=cfg['model_updates'][mode] or sha(target/'FINAL.pt')!=result['checkpoint_sha256']:raise ValueError('FINAL_HEAD_CHANGED')
            continue
        args=[cfg['torch_python'],'-I','-B',str(HERE/'train_one.py'),'--run',str(run),'--output',str(target),
