...
def registry_value(raw_families, config):
    ...
    return dict(scope='EXPOSED_DEV_PRESERVATION_GROUNDING_V1',conditions=conditions,models=models,slots=slots,
...
def main(run):
    ...
    try:
        ...
        for tag in reg['models']:
            ...
            if result['updates']!=config['model_updates'][tag.split('_')[0]] or sha(path)!=result['checkpoint_sha256']:raise ValueError('FINAL_CHECKPOINT_IDENTITY')
            ...
            for slot in remaining:
                ...
                if len(contrast)==len(reg['models']):
                    ...
                    for seed in config['seeds']:
                        for arm in ('ORIGINAL',):
                            left=f'{arm}_{seed}';right=f'REPAIR_{seed}';audit=prefix_audit(contrast[left],contrast[right])
