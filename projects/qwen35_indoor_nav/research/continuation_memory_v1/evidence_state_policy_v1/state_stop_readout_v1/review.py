...
def paired(rows):
    ...
    for condition,seed in keys:
        a=indexed[condition,seed,'MONOTONIC'];b=indexed[condition,seed,'STOPFIX'];key=[condition,seed]
    ...
    a=metric([r for r in rows if r['arm']=='MONOTONIC']);b=metric([r for r in rows if r['arm']=='STOPFIX'])
...
def main(run):
    ...
    result=dict(status='VALID_COMPLETE_DEV_REPAIR' if complete else 'INCOMPLETE_DEV_REPAIR',complete=len(groups)*len(reg['models']),planned=len(rows),
        ...
        supports_dev_repair_signal=signal,method_adopted=False,new_updates=cfg['repair_updates']*len(cfg['seeds']),raw_arrays_verified=len(arrays),
        ...
        limitation='Exposed original DEV house, frozen MONOTONIC vs a trained STOP-only residual. No independent holdout or natural VLN-CE claim.')
    ...
    lines=[result['status'],'',f"完整续接 {result['complete']}/{result['planned']}；完整六模型组 {len(groups)}/{len(reg['conditions'])}。新增STOP读出更新 {cfg['repair_updates']*len(cfg['seeds'])}，原模型参数更新0。",
    ...
    lines.extend(['',f"STOPFIX−MONOTONIC 主终点差值：{main['delta']:.2%}；未知分配识别界 [{main['delta_lower']:.2%}, {main['delta_upper']:.2%}]。这不是置信区间。",
        f"方向为正的房屋 {positive_houses}/1、种子 {positive_seeds}/3；原DEV局部修复信号：{signal}。",'',
        '既有开发屋结果用于选择本次候选，未重命名为盲测。本轮只用原FIT训练、原DEV诊断，不选择新屋样本。',
