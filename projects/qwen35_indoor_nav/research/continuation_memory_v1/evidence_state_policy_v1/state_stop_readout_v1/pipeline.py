"""Independent STOP readout training, local gate and complete-group DEV evaluation."""
import argparse
import fcntl
import os
...
def main():
    ...
    write(run/'STATUS.json',dict(status='RUNNING',stage='preflight',new_optimizer_updates=sum(read(p)['updates'] for p in (run/'train').glob('STOPFIX_*/RESULT.json'))))
    ...
    try:
        ...
        if not (run/'RESULT.json').exists():
            ...
            while any(monitor.gpu_snapshot(d)['free_mib']<cfg['min_start_gpu_gib']*1024 for d in cfg['devices']):
                ...
                time.sleep(2)
            assignments=[(cfg['devices'][i],[f'STOPFIX_{seed}']) for i,seed in enumerate(cfg['seeds'])
                if not (run/'train'/f'STOPFIX_{seed}'/'RESULT.json').exists()]
            if assignments:execute(run,cfg,'train',assignments)
            if not (run/'LOCAL_GATE.json').exists():execute(run,cfg,'gate',[(cfg['devices'][0],[])],gpu=False)
            if read(run/'LOCAL_GATE.json')['admit_closed_loop']:
    ...
    write(run/'STATUS.json',dict(status=result['status'],complete=result['complete'],planned=result['planned'],new_optimizer_updates=sum(read(p)['updates'] for p in (run/'train').glob('STOPFIX_*/RESULT.json'))))
    ...
    write(run/'STATUS.json',dict(status=result['status'],complete=result['complete'],planned=result['planned'],upload_verified=True,new_optimizer_updates=sum(read(p)['updates'] for p in (run/'train').glob('STOPFIX_*/RESULT.json'))))
