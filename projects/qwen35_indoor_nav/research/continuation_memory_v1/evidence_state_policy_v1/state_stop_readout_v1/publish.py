...
def main(run):
    ...
    with (V16/'b2_kl_repair_v1/PUBLISH.lock').open('a') as guard:
        ...
        paths+=parts
        paths+=list((run/'train').glob('STOPFIX_*/FINAL.pt'))+list((run/'train').glob('STOPFIX_*/RESULT.json'))+list((run/'train').glob('STOPFIX_*/LOCAL_DIAGNOSIS.json'))
        ...
        write(out/'INDEX.json',dict(files=entries,parts=[dict(path=p.name,sha256=sha(p)) for p in parts],
            excluded='Licensed scene/RGB/semantic arrays, environments and base checkpoint stay on server. Original frozen heads remain at gpu_v1 with MODEL_REUSE hashes; new STOPFIX finals are published. Causal feature caches and optimizer checkpoints remain local, bound by SHA.'),True)
        ...
        subprocess.run(['git','commit','-m','Register bounded state-to-STOP readout repair and measured evidence'],cwd=ROOT,check=True,stdout=subprocess.DEVNULL)
