...
完整结果与逐种子解读：[MONOTONIC 候选报告]({base}/gpu_runtime_r1/RESULT_READOUT_ZH.md)。
...
def main(run):
    ...
    with (V16/'b2_kl_repair_v1/PUBLISH.lock').open('a') as guard:
        ...
        def remote(args):
            return subprocess.run(['bash','-lc','proxyon >/dev/null 2>&1 && exec '+shlex.join(args)],cwd=ROOT,env=env,capture_output=True,text=True,timeout=1800)
