"""Seven-GPU independent coordinator; full model/group recovery and owned cleanup."""
import argparse
import fcntl
import os
...
def execute(run,cfg,stage,assignments,gpu=True):
    ...
    try:
        ...
        while True:
            ...
            if stage=='train' and hours-prior_hours>cfg['gpu_training_session_hours_cap']:raise RuntimeError('TRAIN_GPU_BUDGET')
...
def main():
    ...
    for i,d in enumerate(cfg['devices']):immutable(run/'devices'/f"gpu_{d['gpu']}.json",dict(d,conditions=list(range(128))[i::len(cfg['devices'])]))
    ...
    try:
        ...
        if not (run/'RESULT.json').exists():
            ...
            while not all((run/'train'/tag/'RESULT.json').exists() for tag in tags):
                assignments=[(d,[t for t in tags[i::len(cfg['devices'])] if not (run/'train'/t/'RESULT.json').exists()]) for i,d in enumerate(cfg['devices'])]
            ...
            immutable(run/'train/RESULT.json',dict(models=results,total_updates_this_run=10800,reused_control_models=0,base_updates=0,final_step=1200))
            if not (run/'DIAGNOSTICS_COMPLETE.json').exists():
                execute(run,cfg,'diagnose',[(d,tags[i::len(cfg['devices'])]) for i,d in enumerate(cfg['devices'])])
                immutable(run/'DIAGNOSTICS_COMPLETE.json',dict(models=tags,scope='cached teacher paths, not navigation'))
            from evaluate_continuations import registry,admitted
            reg=registry(run)
            while len(admitted(run,reg))<len(reg['conditions']):
                complete=set(admitted(run,reg));assignments=[(d,[]) for i,d in enumerate(cfg['devices']) if set(range(128)[i::len(cfg['devices'])])-complete]
    ...
    try:
        ...
        if not (run/'CALIBRATION.json').exists():execute(run,cfg,'calibration',[(cfg['devices'][0],[])],gpu=False)
