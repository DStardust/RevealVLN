...
def event_label(value):
    # Compiler.atoms returns witnessed instance IDs, or None for unknown evidence.
    if value is None:return None
    if not isinstance(value,list):raise ValueError('UNEXPECTED_EVENT_CERTIFICATE')
    ...
    for slot in reg['slots']:
        ...
        for row in c.records(folder/'POLICY_STEPS.jsonl'):
            ...
            for bit,name in enumerate(('anchor','terminal')):
                ...
                truth=event_label(atoms[t][key])
                if truth is not None:buckets[(slot['arm'],'event_'+name,scope)].append((row['event_probabilities'][bit],truth))
    immutable(run/'CALIBRATION.json',dict(status='READ_ONLY_DIAGNOSTIC',postprocessor='calibration_r2.py',source_sha256=sha(Path(__file__)),groups=len(groups),rows=[dict(arm=a,target=t,scope=s,**summarize(v)) for (a,t,s),v in sorted(buckets.items())],
