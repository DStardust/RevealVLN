...
def registry_value(raw_families, config):
    ...
    return dict(scope='EXPOSED_DEV_NONNEGATIVE_STOP_RESIDUAL_V3',conditions=conditions,models=models,slots=slots,
...
def main(run):
    ...
    try:
        ...
        for tag in reg['models']:
            ...
            if result['updates']!=(config['repair_updates'] if tag.startswith('STOPPLUS_') else 1200) or sha(path)!=result['checkpoint_sha256']:raise ValueError('FINAL_CHECKPOINT_IDENTITY')
            ...
            for slot in remaining:
                ...
                if len(contrast)==len(reg['models']):
                    ...
                    for seed in config['seeds']:
                        for arm in ('MONOTONIC',):
                            left=f'{arm}_{seed}';right=f'STOPPLUS_{seed}';audit=prefix_audit(contrast[left],contrast[right])
