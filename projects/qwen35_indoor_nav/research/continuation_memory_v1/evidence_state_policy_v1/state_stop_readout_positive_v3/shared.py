...
def make_head(tag):
    ...
    if 'evidence_gpu_architecture' not in sys.modules:
        common=load('stop_frozen_common',PARENT/'common.py')
        sys.modules['common']=common
        load('evidence_gpu_architecture',PARENT/'model.py')
    original=sys.modules['evidence_gpu_architecture'].EvidencePolicy(int(seed),'MONOTONIC')
    sys.path.insert(0,str(HERE))
    if mode=='MONOTONIC':return original
    if mode!='STOPPLUS':raise ValueError('UNREGISTERED_HEAD')
    repair=load('state_stop_model',HERE/'model.py')
    return repair.StopPolicy(original,int(seed))
