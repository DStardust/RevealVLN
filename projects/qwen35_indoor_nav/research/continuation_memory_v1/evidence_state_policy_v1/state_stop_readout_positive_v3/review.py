...
def paired(rows):
    ...
    for condition,seed in keys:
        a=indexed[condition,seed,'MONOTONIC'];b=indexed[condition,seed,'STOPPLUS'];key=[condition,seed]
    ...
    a=metric([r for r in rows if r['arm']=='MONOTONIC']);b=metric([r for r in rows if r['arm']=='STOPPLUS'])
...
def main(run):
    ...
    result=dict(status='VALID_COMPLETE_DEV_STOPPLUS' if complete else 'INCOMPLETE_DEV_STOPPLUS',complete=len(groups)*len(reg['models']),planned=len(rows),
        ...
        supports_dev_repair_signal=signal,method_adopted=False,new_updates=0,reused_readout_updates=3600,raw_arrays_verified=len(arrays),
    ...
    lines=[result['status'],'',f"完整续接 {result['complete']}/{result['planned']}；完整六模型组 {len(groups)}/{len(reg['conditions'])}。本轮更新0；复用初版的3600次STOP读出更新，原模型参数更新0。",
    ...
    lines.extend(['',f"STOPPLUS−MONOTONIC 主终点差值：{main['delta']:.2%}；未知分配识别界 [{main['delta_lower']:.2%}, {main['delta_upper']:.2%}]。这不是置信区间。",
