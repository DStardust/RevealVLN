...
def main():
    ...
    p=copy.deepcopy(old);p.update(id='Q35N_ORDINARY_ONPOLICY_RECOVERY_DATA_V2',lanes=1,
    ...
    predecessor=HERE.parent/'ordinary_onpolicy_recovery_v1'
    failed=read(predecessor/'run_001/COLLECTION_FAILURE.json')
    assert '_shortest_path_cache' in failed['error'] and failed['counts'].get('explicit_primitives',0)==0
    assert not read(predecessor/'run_001/LAUNCH_RESULT.json')['cleanup']['remaining']
    c.write(HERE/'PREDECESSOR_FAILURE.json',dict(path=str(predecessor),error=failed['error'],explicit_primitives=0,
        wall_seconds=read(predecessor/'run_001/LAUNCH_RESULT.json')['wall_seconds'],preserved=True,
        change='typed mutable Episode cache adapter; original distance function and tolerances unchanged'),True)
    sourcefiles=[FIT/'SOURCE_LOCK.json',predecessor/'SOURCE_LOCK.json',predecessor/'run_001/COLLECTION_FAILURE.json',
        predecessor/'run_001/LAUNCH_RESULT.json',HERE/'PREDECESSOR_FAILURE.json',FIT/'run_001/RESULT.json',FIT/'run_001/LAUNCH_RESULT.json',
