...
class DistanceTests(unittest.TestCase):
    def test_mutable_episode_cache_and_goal_isolation(self):
        s=importlib.util.spec_from_file_location('distance_adapter_test',HERE/'distance_adapter.py')
        d=importlib.util.module_from_spec(s);s.loader.exec_module(d)
        class Metric:
            @staticmethod
            def measure(pf,pos,goals,episode):
                self.assertFalse(isinstance(episode,dict))
                episode._shortest_path_cache=goals
                return 2.5
        a=d.GoalDistance(Metric,[1,2,3]);b=d.GoalDistance(Metric,[4,5,6])
        self.assertEqual(a.measure(None,[0,0,0]),2.5)
        self.assertEqual(a.episode._shortest_path_cache,[[1,2,3]])
        self.assertIsNone(b.episode._shortest_path_cache)
    def test_worker_uses_adapter(self):
        text=(HERE/'worker.py').read_text()
        self.assertIn("meter=D.GoalDistance(distance_module,e['goals'][0]['position'])",text)
        self.assertIn('value=meter.measure(sim.pathfinder,pos)',text)
        self.assertNotIn("distance_module.measure(sim.pathfinder,pos,[e['goals'][0]['position']],e)",text)
