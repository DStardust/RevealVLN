...
import collections,importlib.util,json,math,os,subprocess,time
...
c=load('common');k=load('contracts');route=load('route')
...
def main():
    ...
    route_logs=[lines(OUT/n) for n in ('ROUTE_PROGRESS_FIRST.jsonl','ROUTE_PROGRESS_SECOND.jsonl')]
    assert len(route_logs[0])==len(route_logs[1])==7225
    verified_route={}
    for left,right in zip(*route_logs):
        assert left==right,'INDEPENDENT_ROUTE_PROGRESS_REPLAY_MISMATCH'
    for log in route_logs:
        cursors=collections.defaultdict(int);last_t=collections.defaultdict(lambda:-1);seen_route=set()
        for x in log:
            i,t=x['index'],x['decision_step'];assert (i,t) not in seen_route;seen_route.add((i,t))
            assert t==last_t[i]+1;last_t[i]=t
            targets=episodes[i]['reference_path']+[episodes[i]['goals'][0]['position']]
            cursor=cursors[i];assert x['cursor_before']==cursor
            last=None
            for number,test in enumerate(x['tested']):
                assert cursor<len(targets) and test['index']==cursor
                d=test['distance'];last=d
                if d is None:
                    assert number==len(x['tested'])-1
                    break
                ...
                if d>.35:
                    ...
                    break
                cursor+=1
            assert x['cursor_after']==cursor and x['selected_index']==min(cursor,len(targets)-1)
            assert x['route_complete']==(cursor==len(targets))
            if cursor<len(targets):
                assert x['tested'] and x['tested'][-1]['index']==cursor and last==x['selected_distance']
            else:assert x['selected_distance'] is None or math.isfinite(x['selected_distance']) and x['selected_distance']>=0
            cursors[i]=cursor;verified_route[i,t]=x
        assert len(seen_route)==7225
    ...
    for x in rows:
        ...
        vr=verified_route[tag];rt=x['route_target']
        assert rt=={key:value for key,value in vr.items() if key not in ('index','decision_step','cursor_before')}
        targets=episodes[x['source_index']]['reference_path']+[episodes[x['source_index']]['goals'][0]['position']]
        assert x['teacher_target_position']==targets[rt['selected_index']]
        if route.stop_at(rt,e['distance']):
            ...
            assert b['physical_valid'] and abs(b['distance_before']-rt['selected_distance'])<1e-5
    ...
    files=[OUT/'POLICY_INPUTS.jsonl',OUT/'TRAINING_TARGETS.json',OUT/'SUPERVISION_ONLY.jsonl',
           OUT/'REPLAY_AUDIT.jsonl',OUT/'ROUTE_PROGRESS_FIRST.jsonl',OUT/'ROUTE_PROGRESS_SECOND.jsonl',OUT/'PIXEL_AUDIT.json',OUT/'COLLECTION_RESULT.json',OUT/'QUARANTINED_GROUPS.json']
    ...
    result=dict(status='AUDITED_ONPOLICY_TEACHER_DATA_READY' if passed else 'DATA_GATE_NOT_MET',unix=time.time(),
        ...
        teacher_one_step_only_not_full_instruction_semantics=True,source_lock_verified=True,ordered_route_teacher=True,
        route_progress_replay_rows=14450,route_geodesic_queries=r['counts']['route_geodesic_queries'],
