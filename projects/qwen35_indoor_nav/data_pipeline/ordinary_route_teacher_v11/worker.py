...
D=load('recovery_distance_adapter',HERE/'distance_adapter.py')
route=load('route_teacher_cursor',HERE/'route.py')
...
def main():
    ...
    def init(e):
        ...
        return pixels(obs)
    def geom(e,pos):
        value=meter.measure(sim.pathfinder,pos)
        assert math.isfinite(value) and value>=0;return value
    def route_distance(target,pos):
        counts['route_geodesic_queries']+=1
        key=tuple(target)
        if key not in route_meters:route_meters[key]=D.GoalDistance(distance_module,target)
        d=route_meters[key].measure(sim.pathfinder,pos)
        assert not math.isnan(d) and d>=0
        return d
    def route_info(cursor,pos,index,j,pass_id):
        value=route.advance(targets,cursor,lambda target:route_distance(target,pos))
        c.append(OUT/('ROUTE_PROGRESS_FIRST.jsonl' if pass_id==1 else 'ROUTE_PROGRESS_SECOND.jsonl'),
            dict(index=index,decision_step=j,cursor_before=cursor,**value))
        return value
    try:
        for idx,e in enumerate(episodes):
            ...
            meter=D.GoalDistance(distance_module,e['goals'][0]['position'])
            targets=e['reference_path']+[e['goals'][0]['position']];route_meters={};cursor=0
            ...
            for j,(decision,observed) in enumerate(zip(pol,steps)):
                ...
                rt=route_info(cursor,before['position'],idx,j,1);cursor=rt['cursor_after']
                teacher_goal=targets[rt['selected_index']];route_d=rt['selected_distance']
                if route.stop_at(rt,d):
                    target=3;teacher='STOP';counts['teacher_stop_labels']+=1
                elif route_d is None:reason='ROUTE_TARGET_UNREACHABLE'
                else:
                    counts['teacher_planner_queries']+=1
                    try:
                        follower.reset()
                        teacher=follower.next_action_along(np.array(teacher_goal,dtype=np.float32))
                    ...
                    if teacher in k.ACTIONS[:3]:
                        ...
                        try:
                            ...
                            after=pose(agent);after_distance=route_distance(teacher_goal,after['position'])
                            valid=math.isfinite(after_distance) and k.movement_valid(teacher,before,after,bool(obs['collided']),route_d,after_distance)
                            branch=dict(action=teacher,before=before,after=after,collided=bool(obs['collided']),
                                distance_before=route_d,distance_after=after_distance if math.isfinite(after_distance) else None,physical_valid=valid)
                ...
                row=dict(record_id=ident,source_index=idx,episode_id=e['episode_id'],scene_group=house,split='FIT',
                    ...
                    distance_to_goal=d,route_target=rt,teacher_target_position=teacher_goal,reason=reason,teacher_branch=branch,input_pixel_hash=digest,
            ...
            rgb=init(e);cursor=0;route_meters={}
            ...
            for j,(decision,observed) in enumerate(zip(pol,steps)):
                ...
                rt=route_info(cursor,pose(agent)['position'],idx,j,2);cursor=rt['cursor_after']
