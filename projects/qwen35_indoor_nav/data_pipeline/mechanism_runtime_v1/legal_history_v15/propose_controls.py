...
def main():
    ...
    for distance in (1,2,3,4):
        for turn,back in [('L','R'),('R','L')]:
            ...
            histories={}
            common_tail=(['L','R']*4+['F']*sync_forward if sync_forward else [])+['L','R']*4
            for name in ('H_A','H_B'):
                histories[name]=old[name]+common_tail
                histories[name+'_N']=old[name]+loop+common_tail
            ...
            for delta in [0]+[s*i*1e-5 for i in range(1,501) for s in (1,-1)]:
                ...
                for name,actions in histories.items():
                    vectors[name],rot=components(actions,rotation)
                    tails[name]=tuple(rot[-2:]+([rot[-9-sync_forward]] if sync_forward else []))
                tested+=1
    ...
    if sync_forward:filename=filename.replace('.json',f'_SYNC{sync_forward}_ALIGNED.json')
