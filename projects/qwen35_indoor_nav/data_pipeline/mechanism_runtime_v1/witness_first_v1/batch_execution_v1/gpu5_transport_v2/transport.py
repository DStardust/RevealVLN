...
class RealOps:
    ...
    def status(self,pid):
        """Exit-safe identity only; cwd/cmdline may vanish before /proc/stat."""
        try:raw=(Path('/proc')/str(pid)/'stat').read_text()
        except FileNotFoundError:return None
        fields=raw.rsplit(')',1)[1].split()
        return {'starttime_ticks':int(fields[19]),'state':fields[0]}
...
def holder_status(ops,identity):
    status=ops.status(identity['pid'])
    if status is not None:
        require(status['starttime_ticks']==identity['starttime_ticks'],'HOLDER_PID_REUSED')
    return status

def wait_terminal(ops,identity):
    for _ in range(80):
        status=holder_status(ops,identity)
        if status is None or status['state'] in ('Z','X'):return
        ops.sleep(.25)
    status=holder_status(ops,identity)
    require(status is None or status['state'] in ('Z','X'),'HOLDER_EXIT_FAILED_NO_ESCALATION')

def wait_dead_pane(ops,identity):
    for _ in range(80):
        require(ops.pane('#{pane_id}')==PANE_ID,'PANE_IDENTITY_CHANGED')
        require(int(ops.pane('#{pane_pid}'))==HOLDER_PID,'PANE_PROCESS_IDENTITY_CHANGED')
        status=holder_status(ops,identity)
        require(status is None or status['state'] in ('Z','X'),'HOLDER_NOT_TERMINAL')
        if ops.pane('#{pane_dead}')=='1':return
        ops.sleep(.25)
    require(ops.pane('#{pane_dead}')=='1','PANE_DEATH_NOT_OBSERVED_WITHIN_20_SECONDS')

def wait_gpu_drain(out,identity,module,ops,name):
    """A terminated holder's GPU context may lag /proc termination; never alter samples."""
    for index in range(81):
        ...
        with (out/name).open('a') as f:
            f.write(json.dumps({'sample':index,'gpu':snapshot})+'\n');f.flush();os.fsync(f.fileno())
        external=[v['mib'] for p,v in snapshot['processes'].items() if p!=HOLDER_PID]
        require(all(0<=m<=768 for m in external) and sum(external)<1024,'GPU_DRAIN_EXTERNAL_RESOURCE_LOAD')
        if HOLDER_PID not in snapshot['processes'] and snapshot['memory_mib']<1024:
            module.check_gpu(snapshot);return snapshot
        if index<80:ops.sleep(.25)
...
def run_lease(out,identity,module,execute,*,ops=None,signals=signal):
    ...
    try:
        ...
        wait_terminal(ops,identity)
        wait_gpu_drain(out,identity,module,ops,'GPU_LEASE_DRAIN.jsonl')
        ...
        try:
            ...
            if not attempted:
                ...
                status=holder_status(ops,identity);retained=False
                if status is not None and status['state'] not in ('Z','X'):
                    # During SIGTERM exit, cwd/cmdline can disappear first. That
                    # is not proof of PID reuse and must not abort restoration.
                    try:retained=identity_equal(ops.identity(),identity)
                    except FileNotFoundError:retained=False
                    if not retained:wait_terminal(ops,identity)
                if retained:
                    snapshot=ops.gpu(module)
                    require(snapshot['processes'].get(HOLDER_PID,{}).get('mib',0)>20000,'ORIGINAL_HOLDER_RETENTION_NOT_OBSERVED')
                    restoration={'restored':True,'original_holder_retained':True,'pid':HOLDER_PID}
                else:
                    restoration=restore_dead_holder(out,identity,module,ops)
        except BaseException as exc:
            ...
            if remain_changed:
                try:
                    ...
                    # Never delete the only recoverable dead pane after a failed
                    # restore. V1 incorrectly reset 'off' in this failure path.
                    if restoration.get('restored') and ops.pane('#{pane_dead}')=='0':
...
def restore_dead_holder(out,identity,module,ops):
    ...
    wait_dead_pane(ops,identity)
    snapshot=wait_gpu_drain(out,identity,module,ops,'GPU_RESTORE_DRAIN.jsonl')
    ...
    for _ in range(60):
        ...
        if snapshot['processes'].get(restored_pid,{}).get('mib',0)>20000:
            passed=True;break
    return {'restored':passed,'pid':restored_pid,'process_identity':observed,'gpu':snapshot}
