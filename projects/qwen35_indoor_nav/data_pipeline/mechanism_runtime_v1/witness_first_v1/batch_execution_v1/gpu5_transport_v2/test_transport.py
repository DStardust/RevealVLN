...
IDENTITY={'pid':t.HOLDER_PID,'starttime_ticks':131709588,'command':t.COMMAND,'cwd':str(t.ROOT),
...
class Ops:
    def __init__(self):
        ...
        self.exit_window=False;self.exit_reads=0;self.pane_delay=0;self.signal_during_exit=False;self.status_reused=False
        self.gpu_delay=0
    def identity(self):
        if self.state=='exiting':raise FileNotFoundError('/proc/exiting/cwd')
        x=dict(IDENTITY)
    ...
    def gpu(self,module):
        ...
        if self.state=='dead' and self.gpu_delay:
            self.gpu_delay-=1;procs[t.HOLDER_PID]={'mib':22000,'type':'C'}
    ...
    def terminate(self,pid):
        ...
        if not self.stuck:self.state='exiting' if self.exit_window else 'dead'
    def exists(self,pid):
        if pid==t.HOLDER_PID:return self.state in ('held','exiting')
        if pid==999001:return self.state=='restored'
        if pid==123456:return self.worker_alive
        return False
    def proc(self,pid):
        if self.state=='exiting':raise FileNotFoundError('/proc/exiting/cwd')
        return dict(IDENTITY,pid=pid)
    def status(self,pid):
        if self.status_reused:return {'starttime_ticks':IDENTITY['starttime_ticks']+1,'state':'S'}
        if self.state=='exiting':
            self.exit_reads+=1
            if self.exit_reads>3:self.state='dead'
        if self.state in ('held','exiting'):return {'starttime_ticks':IDENTITY['starttime_ticks'],'state':'S'}
        return None
    def pane(self,fmt):
        if fmt=='#{pane_id}':return t.PANE_ID
        if fmt=='#{pane_pid}':return str(999001 if self.state=='restored' else t.HOLDER_PID)
        if fmt=='#{pane_dead}':
            if self.state=='dead' and self.pane_delay:
                self.pane_delay-=1;return '0'
            return '0' if self.unknown_pane or self.state!='dead' else '1'
    ...
    def sleep(self,seconds):
        self.start+=seconds
        if self.state=='exiting' and self.signal_during_exit:
            self.signal_during_exit=False;raise InterruptedError('simulated_during_exit_window')
...
class Tests(unittest.TestCase):
    ...
    def test_actual_identity_schema_and_source_hashes(self):
        doc=t.read(t.LINE/'authorizations/WITNESS_BATCH02R1_GPU5_HOLDER_IDENTITY_V1.json')
    ...
    def test_actual_authorization_scope_and_no_widening(self):
        auth=t.read(t.LINE/'authorizations/WITNESS_CROSS_HOUSE_BATCH02R1_GPU5_V1.json')
        paths=(t.BE/'batch_02r1',t.BE/'batch_02_preparation_v1/composite',[0,1,2],
               t.LINE/'authorizations/WITNESS_BATCH02R1_GPU5_HOLDER_IDENTITY_V1.json')
    ...
    def test_exit_stat_alive_cwd_absent_does_not_abort(self):
        self.ops.exit_window=True
        self.runlease()
        self.assertTrue(t.read(self.out/'RESTORATION.json')['restored'])
        self.assertGreaterEqual(self.ops.exit_reads,4)
    def test_interrupt_during_cwd_absent_exit_still_restores(self):
        self.ops.exit_window=True;self.ops.signal_during_exit=True
        with self.assertRaises(RuntimeError):self.runlease()
        self.assertTrue(t.read(self.out/'RESTORATION.json')['restored'])
    def test_delayed_pane_death_waits_then_restores(self):
        self.ops.pane_delay=5
        self.runlease()
        self.assertTrue(t.read(self.out/'RESTORATION.json')['restored'])
        self.assertGreaterEqual(self.ops.start,1.25)
    def test_restore_failure_never_resets_remain_off(self):
        self.ops.restored_memory=100
        with self.assertRaises(RuntimeError):self.runlease()
        self.assertNotIn(('remain','off'),self.ops.events)
        self.assertTrue(t.read(self.out/'RESTORATION.json')['remain_on_exit_preserved_on_for_recovery'])
    def test_pane_death_timeout_preserves_recoverable_pane(self):
        self.ops.unknown_pane=True
        with self.assertRaises(RuntimeError):self.runlease()
        self.assertNotIn(('remain','off'),self.ops.events)
        self.assertFalse(any(x[0]=='restore' for x in self.ops.events))
    def test_reused_pid_never_signalled_again_or_respawned(self):
        def reused():self.ops.status_reused=True
        with self.assertRaises(RuntimeError):self.runlease(reused)
        self.assertEqual(sum(x[0]=='signal' for x in self.ops.events),1)
        self.assertFalse(any(x[0]=='restore' for x in self.ops.events))
        self.assertNotIn(('remain','off'),self.ops.events)
    def test_terminated_holder_gpu_context_lag_is_recorded_then_drained(self):
        self.ops.gpu_delay=3
        self.runlease()
        rows=[t.json.loads(x) for x in (self.out/'GPU_LEASE_DRAIN.jsonl').read_text().splitlines()]
        self.assertEqual(len(rows),4)
        self.assertIn(str(t.HOLDER_PID),rows[0]['gpu']['processes'])
        self.assertNotIn(str(t.HOLDER_PID),rows[-1]['gpu']['processes'])
