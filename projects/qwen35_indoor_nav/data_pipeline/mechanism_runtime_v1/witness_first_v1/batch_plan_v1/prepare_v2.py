...
from balanced_v2 import solve,matched,closed,FamilyFactory,CONTROL_TYPE
...
def snapshot(output):
    ...
    source=WF/'scout_v1/run_v1';locks={};cache={}
        ...
        for index,hub in enumerate(frozen['hubs']):
            ...
            for proposal in offered['proposals']:
                ...
                for name,old in [('a','anchor_A'),('b','anchor_B'),('i','irrelevant_loop'),('terminal','terminal_only')]:
                    ...
                    if str(path) not in cache:cache[str(path)]=read(path)
                    trace=cache[str(path)]
                    if digest(trace)!=component['trace_digest'] or not compiler.complete(trace):raise ValueError('COMPONENT_TRACE_MISMATCH')
                ...
                candidate={'candidate_id':'WF_BATCH_'+ident,'house_id':house,'hub_index':index,'hub_key':proposal['hub_key'],
                    ...
                    'expected_history_action_counts':plan['action_counts'],'history_actions_before_public_tail':plan['history_actions'],
                    ...
                    'component_provenance':{'source_proposal_id':proposal['proposal_id'],
                        ...
                        'selection_rule':'per_completed_hub_min_history_actions_then_max_continuation_actions_then_proposal_id; k2..16_j1..16; i>=2F',
    ...
    for path in [HERE/'balanced_v2.py',HERE/'prepare_v2.py',WF/'bank_cpu/bank.py',WF/'compatibility_cpu/audit.py',
    ...
    products={'CONFIG_DRAFT.json':{'node':'Q35N_WITNESS_BALANCED_BATCH_V2','runtime_allowed':False,'executable':False,
