...
def gpu():
    return parse_gpu(subprocess.check_output(['nvidia-smi','-i','1','-q','-x'], text=True, timeout=15))
...
def main():
    ...
    try:
        with (OUT/'worker.log').open('x') as log:
            proc=subprocess.Popen([str(ENV/'bin/python3'),'-I','-B','/mnt/data_nas/deeprobotics/daiyang/vla/projects/qwen35_indoor_nav/data_pipeline/mechanism_runtime_v1/witness_first_v1/bulk_source_v1/worker.py','--shard','0'],
            ...
            while proc.poll() is None:
                ...
                assert sample['elapsed'] < 4500, 'WALL_BUDGET'
                ...
                if count%12==0:
                    sample['disk_bytes']=disk_size(); assert sample['disk_bytes'] < 8*1024**3, 'DISK_BUDGET'
