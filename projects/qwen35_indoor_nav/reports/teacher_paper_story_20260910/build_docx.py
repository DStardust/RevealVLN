...
def main():
    ...
    footer = XML+f'<w:ftr xmlns:w="{W}"><w:p><w:pPr><w:pStyle w:val="Header"/><w:jc w:val="right"/></w:pPr>'+run('教师沟通稿  ·  2026.09.10    |    ')+ '<w:fldSimple w:instr=" PAGE "><w:r><w:t>1</w:t></w:r></w:fldSimple></w:p></w:ftr>'
    ...
    parts = {
        ...
        'docProps/core.xml': XML+'<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:title>从有证据的理解，到记得任务的导航</dc:title><dc:subject>面向居家医疗养老机器人的论文研究思路</dc:subject><dc:description>教师沟通稿；拟贡献与实际进展分别说明。</dc:description></cp:coreProperties>',
    }
    ct = XML+'<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/>'
    ...
    with ZipFile(OUTPUT) as z:
        ...
        assert 'EgoCoT-Bench' in text and '核心贡献' in text
        assert text.count('UAD') == 0
        assert len(root.findall('.//w:br[@w:type="page"]',ns)) == 5
        assert len(root.findall('.//w:tbl',ns)) == 7
        assert len(links) == 7
    report = {
        ...
        'explicit_page_sections': 6, 'editable_tables': 7, 'external_reference_links': 7,
