...
LINE = HERE.parents[1]
NEW_RUN = Path('/mnt/data_nas/deeprobotics/daiyang/vla/projects/qwen35_indoor_nav/data_pipeline/mechanism_runtime_v1/witness_first_v1/diversity_v1r2')
...
def main():
    audit=load(NEW_RUN/'audit_v1/RESULT.json')
    assert audit['small_batch_acceptance'] and not audit['training_admission']
    accepted=[r for r in audit['attempts'] if r.get('quality_pass')]
    sources={r['attempt_id']:dict(path=str(NEW_RUN/'run_v1/bundles'/r['attempt_id']/'export_v4/MANIFEST.json'),house=r['house_id']) for r in accepted}
    out = HERE/'new_image_review'
    out.mkdir(parents=True, exist_ok=False)
    records = []
    for aid in sources:
        source = sources[aid]
        mp = Path(source['path'])
        assert mp.is_relative_to(NEW_RUN/'run_v1')
        assert load(NEW_RUN/'audit_v1'/aid/'DIVERSITY_REPORT.json')['quality_pass']
        folder = mp.parent
        m = load(mp)
        ...
        assert a_step<=k-8 and b_step<=k-8,'HISTORY_EVENT_NOT_OUTSIDE_PUBLIC_TAIL'
        frames=[]
        for label,h,c,step,role in [
            ...
            frames.append(dict(label=label,history=h,continuation=c,step=step,role=role,
                ...
                target_bbox=([int(z) for z in (np.where(np.isin(sem,target_ids))[1].min(),np.where(np.isin(sem,target_ids))[0].min(),np.where(np.isin(sem,target_ids))[1].max(),np.where(np.isin(sem,target_ids))[0].max())] if role else None)))
        ...
        records.append(dict(attempt_id=aid,source=str(folder),manifest_sha256=sha(mp),
            ...
            new_run=str(NEW_RUN),strong_audit_sha256=sha(NEW_RUN/'audit_v1'/aid/'DIVERSITY_REPORT.json'),
