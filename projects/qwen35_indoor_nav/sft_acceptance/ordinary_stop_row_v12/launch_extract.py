...
def main():
    ...
    env=os.environ.copy();env.update(CUDA_VISIBLE_DEVICES='1',TMPDIR=str(c.LINE/'.t12'),TORCHINDUCTOR_CACHE_DIR=str(HERE/'inductor_cache'),TRITON_CACHE_DIR=str(HERE/'triton_cache'),HF_HUB_OFFLINE='1',TRANSFORMERS_OFFLINE='1',TOKENIZERS_PARALLELISM='false')
    ...
    try:
        with (HERE/'extract.log').open('x') as log:
            ...
            while proc.poll() is None:
                ...
                assert time.monotonic()-began<=1200 and info['memory_mib']<=25*1024 and rss<=16*1024**3 and size<=1024**3,'RESOURCE_BUDGET'
                time.sleep(5)
