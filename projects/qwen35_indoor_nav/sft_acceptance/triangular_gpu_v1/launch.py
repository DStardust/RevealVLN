"""Single GPU7 diagnostic lease; preserve ongoing GPU5/6 training."""
import hashlib
import json
import os
...
ROOT=LINE.parents[1]
GPUS=[7]
PANES={7:'vla_idle_occupancy_20260904:4.0'}
UUIDS={7:'GPU-3f8830f6-45f2-6c4d-776b-2b348159ccb3'}
...
def env_for(rank):
    ...
    env.update(PATH=str(LINE/'.envs/q35n_qwen_g2_v1/bin')+':/usr/bin:/bin',PYTHONNOUSERSITE='1',PYTHONDONTWRITEBYTECODE='1',
        CUDA_VISIBLE_DEVICES=str(GPUS[rank]),RANK=str(rank),WORLD_SIZE='1',LOCAL_RANK='0',
    ...
    try:
        ...
        with (OUT/'RESOURCES.jsonl').open('x') as f:
            while any(p.poll() is None for p in procs):
                ...
                assert elapsed<=270 and rss<=48*1024**3 and disk<=10*1024**3,'Resource cap'
                time.sleep(5)
