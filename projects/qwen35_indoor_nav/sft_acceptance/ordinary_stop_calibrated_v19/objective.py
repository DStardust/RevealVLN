...
def losses(theta,x,reference,negative_indices,negative_mask,positive_indices,positive_mask,trajectory_weights):
    ...
    # Log-mean-exp negative bag: focus high margins without a log(length) prior shift.
    count=negative_mask.sum(1).clamp(min=1).to(values.dtype)
    negative=torch.nn.functional.softplus(torch.logsumexp(values,dim=1)-count.log())
    ...
    return neg+pos+reg,dict(negative_bag=neg,verified_terminal_stop=pos,regularization=reg)
