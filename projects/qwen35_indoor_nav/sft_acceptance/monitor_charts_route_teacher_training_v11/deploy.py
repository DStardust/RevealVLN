...
OLD=HERE.parent/'monitor_charts_route_teacher_v11/server.py';NEW=HERE/'server.py'
PANE='%294';OLD_PID=3577769
...
def check(port):
    ...
    t=d['onpolicy_training'];assert t['build']['planned_decisions']==98979
    if t['eval_progress']:assert t['training_result'] and t['eval_progress']['unix']>t['training_result']['unix']
...
def protected_pids():
    owner=json.loads((LINE/'reviews/Q35N_ORDINARY_ROUTE_TEACHER_V11/SEAL.json').read_text())['owned_training_lease']
