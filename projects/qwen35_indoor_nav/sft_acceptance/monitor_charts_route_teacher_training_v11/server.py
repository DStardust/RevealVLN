...
TRAIN=HERE.parent/'ordinary_route_teacher_v11'
...
def collect(include_gpu=True):
    d=m.collect(include_gpu=include_gpu);d['monitor_version']='ordinary_route_teacher_v11'
    history=[]
    try:
    ...
    d['onpolicy_training']=dict(transport=read(TRAIN/'IPC_TEST_RESULT.json'),prior_transport_failure=read(LINE/'reviews/Q35N_ORDINARY_FP32_MASTER_V8/WORKFLOW_RESULT.json'),precision_bridge=read(HERE.parent/'ordinary_onpolicy_fp32_master_v8/PRECISION_BRIDGE_AUDIT.json'),progress=read(TRAIN/'formal/attempt_001/PROGRESS.json'),
      ...
      route_data=read(LINE/'data_pipeline/ordinary_route_teacher_v11/run_001/RESULT.json'),frame_stride=1,history=history,result=result,eval_progress=read(CASE/'run_001/PROGRESS.json'),
