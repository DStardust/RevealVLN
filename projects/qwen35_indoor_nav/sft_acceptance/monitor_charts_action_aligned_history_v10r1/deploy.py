...
OLD=HERE.parent/'monitor_charts_action_aligned_history_v10/server.py';NEW=HERE/'server.py'
PANE='%294';OLD_PID=3529399
...
def check(port):
    ...
    assert d['monitor_version']=='ordinary_action_aligned_history_v10'
    t=d['onpolicy_training'];assert t['eval_progress']['unix']>t['training_result']['unix']
    assert 'policy_preservation_dev_v9' not in NEW.read_text()
...
def protected_pids():
    state=json.loads((LINE/'reviews/Q35N_ORDINARY_ACTION_ALIGNED_HISTORY_V10/WORKFLOW_STATUS.json').read_text())
    owner=ident(state['own_launcher_pid'])
    assert owner['argv']==[str(PY),'-I','-S','-B',str(LINE/'closed_loop_bench/ordinary_action_aligned_history_dev_v10/launch.py')] and owner['cwd']==str(ROOT)
