...
TRAIN=HERE.parent/'ordinary_history8_paired_train_v1'
...
def collect(include_gpu=True):
    d=parent.collect(include_gpu=include_gpu);d['monitor_version']='ordinary_history8_pair_v1'
    arms={}
    ...
    d['paired_history']=dict(preparation=read(TRAIN/'PREPARATION_RESULT.json'),protocol_present=(TRAIN/'PROTOCOL.json').is_file(),
...
class Handler(parent.Handler):
    def do_GET(self):
        try:
            ...
            elif self.path=='/healthz':self.respond(200,b'{"status":"ok","version":"ordinary_history8_pair_v1"}','application/json')
