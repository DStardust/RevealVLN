...
async function refresh(){try{
 ...
 const d=await res.json();if(d.monitor_version!=='ordinary_history8_pair_v1')throw Error('监控版本不匹配');
 ...
 rows('training',Object.entries(arms).map(([key,a])=>{const r=a.progress;return [labels[key],r?num(r.updates)+' / 4000':'未开始',r?num(r.decisions)+' / 384,000':'未开始',r?r.metrics.mean_ce.toFixed(4):'—',a.live_rank_pids.join(', ')||'无',a.acceptance?'完整训练已验收':a.launch?.status||(a.live_rank_pids.length?'运行中':'等待')];}));
