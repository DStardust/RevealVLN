...
def event(name,**kw):
    obj={'event':name,'unix':time.time(),**kw};append('EVENTS.jsonl',obj);print(json.dumps(obj),flush=True)

def check_lock():
    for name,h in json.loads((SOURCE/'PREREGISTRATION_LOCK.json').read_text()).items():
        if isinstance(h,str):assert hashlib.sha256((SOURCE/name).read_bytes()).hexdigest()==h
...
def checkpoint(policy,name,row,opt=None,updates=0):
    ...
    torch.save(dict(trainable=state,optimizer=opt.state_dict() if opt else None,updates=updates,spec_sha256=hashlib.sha256((SOURCE/'EXPERIMENT_SPEC.json').read_bytes()).hexdigest()),path)
...
def main():
    check_lock();spec=json.loads((SOURCE/'EXPERIMENT_SPEC.json').read_text());split=json.loads((SOURCE/'SPLIT.json').read_text())
    amendment=json.loads((OUT/'AMENDMENT.json').read_text())
    assert amendment['prior_optimizer_updates']==1 and amendment['remaining_updates']==399
    assert len((SOURCE/'UPDATE_LEDGER.jsonl').read_text().splitlines())==1
    event('model_load_start');policy=build();event('model_loaded')
    loaded=torch.load(SOURCE/'checkpoints/initial.pt',map_location='cpu',weights_only=True)
    load_trainable(policy,loaded['trainable'])
    expected=torch.tensor(json.loads((SOURCE/'RELOAD_CHECKS.jsonl').read_text().splitlines()[0])['logits_after'])
    actual=fixed_logits(policy,split['train'][0]);delta=float((actual-expected).abs().max())
    save('INITIAL_CHECKPOINT_IDENTITY.json',dict(max_abs_delta=delta,pass_gate=delta<=1e-5,reused_before_offline=True,reused_before_10_episodes=True))
    assert delta<=1e-5
    initial=trainable_state(policy)
    before=json.loads((SOURCE/'OFFLINE_BEFORE.json').read_text())
    before_episodes=[json.loads(x) for x in (SOURCE/'EPISODE_LEDGER.jsonl').read_text().splitlines()]
    assert sum(x['event']=='complete' and x['stage']=='before' for x in before_episodes)==10
    spec['training']['max_optimizer_updates']=399
    spec['training']['token_budget']-=6906
    event('training_start',prior_updates_consumed=1,remaining_updates=399)
    ...
    closed_after=closed_loop(policy,split['closed_loop'],'after')
    save('WORKER_RESULT.json',dict(training_interface_pass=True,offline_learning_signal=after['decision_micro_CE']<before['decision_micro_CE'],closed_loop_interface_pass=closed_after,optimizer_updates=updates,cumulative_optimizer_updates=updates+1,training_tokens=tokens,all_forward_tokens=policy.forward_tokens,peak_cuda_allocated_bytes=torch.cuda.max_memory_allocated(),peak_cuda_reserved_bytes=torch.cuda.max_memory_reserved(),scientific_pass=False))
    event('worker_complete',updates=updates,cumulative_updates=updates+1)
