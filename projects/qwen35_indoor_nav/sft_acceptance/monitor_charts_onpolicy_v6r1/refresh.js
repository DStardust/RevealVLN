...
async function refresh(){
 ...
 const q=d.onpolicy_recovery,p=q.progress||{},r=q.result||{},cnt=p.counts||{};
 el('recovery-status').textContent=r.status||p.status||(q.prepared?'来源已冻结，等待启动':'CPU准备中');
 el('recovery-bar').value=p.completed||0;
 el('recovery-counts').textContent='完整回放：'+fmt(p.completed||0)+' / 64 条\n已处理决策：'+fmt(cnt.visited_decisions||0)+' / 7,225\n显式动作（含复核）：'+fmt(cnt.explicit_primitives||0)+' / 32,000\n去重合格建议：'+fmt(r.valid_unique_advice)+'\n教师/模型分歧：'+fmt(r.unique_disagreements)+(q.failure&&Object.keys(q.failure).length?'\n失败记录：'+JSON.stringify(q.failure):'');
