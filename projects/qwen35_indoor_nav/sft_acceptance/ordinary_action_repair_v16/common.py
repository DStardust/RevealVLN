...
c=load('motion16_frozen_io',V5/'common.py')
...
def validate_head(original,candidate):
    ...
    for name,t in original.items():
        if name in ('action_head.weight','action_head.bias'):assert t.shape==candidate[name].shape,'HEAD_SHAPE_CHANGED'
        else:assert torch.equal(t,candidate[name]),'NONHEAD_PARAMETER_CHANGED'
...
def audit_episode(folder,index,gt_path,collection=False):
    ...
    aggregate=load('motion16_frozen_metric_audit',LINE/'closed_loop_bench/ordinary_cycle_pair_gpu1_v3/aggregate.py')
