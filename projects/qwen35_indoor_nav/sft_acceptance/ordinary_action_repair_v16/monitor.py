...
def snapshot():
    run=u.HERE.parent/'ordinary_stop_coverage_v15/runs/coverage_001';values=review.completed_collect(run/'collect');collection=list(values.values());checkpoints=[]
    ...
    return dict(motion=motion_snapshot(),unix=time.time(),status=read(run/'STATUS.json',{}),collection=dict(complete=len(collection),planned=640,decisions=sum(v['steps'] for v in collection),stop_positive_observations=sum(v['positive'] for v in collection),teacher_unavailable=sum(v['termination']=='TEACHER_UNAVAILABLE' for v in collection)),
        sessions=[dict(session=str(p.parent.relative_to(run)),**u.read(p)) for p in run.glob('*/sessions/*/PROGRESS.json') if time.time()-p.stat().st_mtime<60],
...
def motion_snapshot():
    ...
    manifest=read(run/'recovery/MANIFEST.json',{})
    result=read(run/'recovery/RESULT.json')
    n=len(list((run/'recovery').glob('sessions/*/pairs/*/COLLECT.json')))
    return dict(status=read(run/'STATUS.json',{}),training=read(run/'TRAIN_PROGRESS.json',{}),training_result=read(run/'TRAIN_RESULT.json'),
        DEV=read(run/'DEV/LIVE_RESULT.json'),UNSEEN=read(run/'UNSEEN/LIVE_RESULT.json'),
        recovery=dict(complete=n,planned=manifest.get('selected'),result=result),
        sessions=[dict(session=str(p.parent.relative_to(run)),**u.read(p)) for p in run.glob('*/sessions/*/PROGRESS.json') if time.time()-p.stat().st_mtime<60])
