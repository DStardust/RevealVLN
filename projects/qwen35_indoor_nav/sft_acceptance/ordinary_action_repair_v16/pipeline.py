...
import common as u
review=u.load('motion16_review',u.HERE/'review.py')
resources=u.load('motion16_resources',u.V16/'pipeline.py')
disk=u.load('motion16_disk',u.HERE.parent/'ordinary_stop_coverage_v15/runtime_r2/disk.py')
...
def main():
    ...
    cfg=u.read(run/'PROTOCOL.json');up=Path(cfg['upstream_run']);workers=[];phase='WAIT_V15_TRAIN';leased=False;began=time.monotonic();gpu_began=None;last_disk=0;artifact=0
    ...
    def recover():
        stage=run/'recovery';manifest=u.read(stage/'MANIFEST.json');n=manifest['selected'];remaining=sorted(set(range(n))-set(review.completed_collect(stage)));active=[];ref=u.read(u.HERE.parent/'ordinary_stop_coverage_v15/PROTOCOL.json');houses=u.read(Path(cfg['manifests'])/'SPLIT.json')['all_training_houses']
        for i,gpu in enumerate(cfg['devices']):
            rr=remaining[i::len(cfg['devices'])]
            if not rr:continue
            assert resources.gpu_snapshot(gpu)['free_mib']>=cfg['start_free_mib'],'SHARED_GPU_HEADROOM'
            session=stage/'sessions'/f'gpu{gpu["gpu"]}_{time.time_ns()}';session.mkdir(parents=True);(session/'frames').mkdir();(session/'pairs').mkdir()
            p=dict(cfg,**gpu,phase='RECOVERY',episodes_path=str(stage/'EPISODES.json'),order_path=str(stage/'ORDER.json'),geometry_path=str(stage/'GEOMETRY.json'),gt_path=cfg['train_gt'],scheduled_ranks=rr,run=str(run),training_houses_list=houses,reference_checkpoint=ref['reference_checkpoint'],reference_checkpoint_sha256=ref['reference_checkpoint_sha256'],runtime_executor=str(u.HERE/'recovery_executor.py'),v16_protocol_sha256=u.sha(run/'PROTOCOL.json'))
            u.write(session/'CONFIG.json',p,True);active.append(spawn('recover',session,gpu))
        poll(active);seals=review.completed_collect(stage);assert len(seals)==n
        u.write(stage/'RESULT.json',dict(status='COLLECTION_COMPLETE',planned=n,sealed=n,real_decisions=sum(v['steps'] for v in seals.values()),stop_positive_observations=sum(v['positive'] for v in seals.values()),scope='FIT recovery teacher collection, not model efficacy; no automatic training on this data'),True)
    try:
        status();wait_for('TRAIN_RESULT.json');phase='TRAIN'
        ...
        phase='REGISTER_RECOVERY'
        if not (run/'recovery/MANIFEST.json').exists():poll([spawn('prepare_recovery',run)])
        phase='WAIT_V15_COMPLETE';wait_for('RESULT.json');dependency=u.read(up/'RESULT.json');assert dependency['status']=='COMPLETE'
        u.write(run/'DEPENDENCY.json',dict(result_sha256=u.sha(up/'RESULT.json'),reference_checkpoint_sha256=u.sha(Path(cfg['reference_checkpoint']))))
        ...
        for phase in cfg['eval_phases']:evaluate(phase)
        phase='RECOVERY';recover();results={tag:u.read(run/tag/'REVIEW.json') for tag in cfg['eval_phases']}
        u.write(run/'RESULT.json',dict(status='COMPLETE',evaluations=results,recovery=u.read(run/'recovery/RESULT.json'),gpu_hours=hours(),automatic_adoption=False),True)
        report='# 动作监督对照与独立恢复采集\n\n'
        for tag,r in results.items():report+=f"{tag}: {r['complete_pairs']}/{r['planned_pairs']} 对，V15 SR={r['arms']['A']['sr']}，V16 SR={r['arms']['B']['sr']}，ΔSR={r['delta_sr']}。\n\n"
        report+='同一640条训练池、同底模/输入/动作/预算；V16额外使用实际教师运动监督并训练四个输出行。共同数据收益不归因于新监督。恢复轨迹单独在评测之后采集，未用于此次训练；没有自动部署，没有新架构或论文贡献结论。\n';(run/'REPORT_ZH.md').write_text(report);u.write(run/'STATUS.json',dict(status='COMPLETE',phase='DONE',unix=time.time(),gpu_hours=hours()))
