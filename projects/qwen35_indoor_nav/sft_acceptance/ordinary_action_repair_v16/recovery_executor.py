...
import common as u
rgb_check=u.load("motion16_rgb_r3",Path(__file__).resolve().parent/"rgb.py")
...
old=u.load('motion16_frozen_sim',source)
route=u.load('motion16_route',u.LINE/'data_pipeline/ordinary_route_teacher_v11/route.py')
    ...
    try:
        for line in stream:
            ...
            if req.get('op')=='reset':
                ...
                assert mode=='RECOVERY' and p['phase']=='RECOVERY' and house in p['training_houses_list'],'RECOVERY_SCOPE'
                assert u.sha(Path(row['source_seal']))==row['source_seal_sha256'],'REPLAY_SEAL_CHANGED'
                source_folder=Path(row['source_folder']);assert u.sha(source_folder/'POLICY_STEPS.jsonl')==row['policy_sha256'] and u.sha(source_folder/'STEPS_PRIVILEGED.jsonl')==row['private_sha256'],'REPLAY_LOG_CHANGED'
                source_policy=u.c.records(source_folder/'POLICY_STEPS.jsonl');source_private=u.c.records(source_folder/'STEPS_PRIVILEGED.jsonl');cutoff=row['cutoff']
                ...
                follower=hs.GreedyGeodesicFollower(sim.pathfinder,sim.get_agent(0),goal_radius=.35,fix_thrashing=False) if mode=='RECOVERY' else None
                digest=picture(rgb)
            ...
            if mode=='RECOVERY':
                ...
                if steps<cutoff:
                    assert digest==source_policy[steps]['raw']['rgb_sha256'][-1],'REPLAY_RGB_CHANGED'
                    expected=source_private[steps-1]['position'] if steps else e['start_position'];assert np.allclose(before_pos,expected,rtol=0,atol=1e-6),'REPLAY_POSE_CHANGED'
                    action=source_policy[steps]['executed_action'];assert action!='STOP','PREFIX_CONTAINS_STOP'
                elif route.stop_at(rt,values['distance_to_goal']):action='STOP'
                else:
                    try:
            ...
            if p['phase']=='RECOVERY':u.append(folder/'SUPERVISION_ONLY.jsonl',dict(step=steps,house=house,episode_id=e['episode_id'],trajectory_id=e['trajectory_id'],mode=mode,target=int(before_distance<3),distance_before=before_distance,executed_action=action,proposed_action=proposed))
            if done:
