...
def main():
    assert os.environ.get('CUDA_VISIBLE_DEVICES')=='' and os.environ['TMPDIR']==str(LINE/'.t10')
    import torch
    from torch.utils.data import TensorDataset,DataLoader
    assert not torch.cuda.is_initialized()
    assert tempfile.gettempdir()==str(LINE/'.t10')
    address=str(LINE/'.t10'/'listener_probe')
    ...
    result=dict(status='PASS',unix=time.time(),actual_unix_socket_bind=True,torch_loader_workers=2,
      ...
      max_expected_worker_socket_path_bytes=len(str(LINE/'.t10').encode())+len('/pymp-abcdefgh/listener-abcdefgh'),
