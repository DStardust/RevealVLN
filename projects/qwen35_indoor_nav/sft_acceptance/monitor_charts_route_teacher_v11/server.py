...
TRAIN=HERE.parent/'ordinary_action_aligned_history_v10'
...
def collect(include_gpu=True):
    d=m.collect(include_gpu=include_gpu);d['monitor_version']='ordinary_route_teacher_data_v11'
    data=LINE/'data_pipeline/ordinary_route_teacher_v11/run_001'
    d['route_teacher_data']=dict(progress=read(data/'PROGRESS.json'),result=read(data/'RESULT.json'),collection=read(data/'COLLECTION_RESULT.json'),launch=read(data/'LAUNCH_RESULT.json'),source_dir=str(data),training_started=False)
    history=[]
    try:
        for line in (TRAIN/'formal/attempt_001/PROGRESS.jsonl').read_text().splitlines():
            try:x=json.loads(line)
            except ValueError:continue
            history.append(dict(updates=x['cursor']['updates'],ce=x['metrics']['mean_ce'],kl=x['metrics'].get('mean_kl'),loss=x['metrics'].get('mean_loss'),unix=x['unix']))
    except FileNotFoundError:pass
    result=read(REVIEW/'RESULT.json')
    d['onpolicy_training']=dict(transport=read(TRAIN/'IPC_TEST_RESULT.json'),prior_transport_failure=read(LINE/'reviews/Q35N_ORDINARY_FP32_MASTER_V8/WORKFLOW_RESULT.json'),precision_bridge=read(HERE.parent/'ordinary_onpolicy_fp32_master_v8/PRECISION_BRIDGE_AUDIT.json'),progress=read(TRAIN/'formal/attempt_001/PROGRESS.json'),
      ...
      view_audit=read(TRAIN/'VIEW_AUDIT.json'),frame_stride=8,history=history,result=result,eval_progress=read(CASE/'run_001/PROGRESS.json'),
