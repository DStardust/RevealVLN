...
async function refresh(){
 try{
  ...
  if(fin){const v=fin.after.result;rows.push(['上一轮V10：两帧对齐（未采用）',v.sr,v.spl,v.ndtw]);}else rows.push(['本次：结果未完成',null,null,null]);
  ...
  const data=d.route_teacher_data||{},dp=data.progress||{},dr=data.result||{};
  el('status').textContent=dr.status||(dp.status||'数据审核准备中');
  el('summary').textContent='当前任务：按原参考路点顺序生成纠错建议；同64条FIT旧轨迹，每条双重真实回放，建议独立一步验证后恢复。未启动新训练。';
  el('transport').textContent='上一轮V10完整100条SR19%，未过门槛；当前最佳仍21%。本轮只检验新监督是否合法，不把审核通过当导航收益。';
  el('progress').max=64;el('progress').value=dp.completed||0;
  el('counters').textContent='已完成 '+(dp.completed||0)+' / 64条；显式原语 '+(dp.total_actions||0)+' / 32000；合法去重建议 '+(dr.valid_unique_advice??data.collection?.valid_unique_advice??'待去重审核');
  el('resources').textContent='GPU1空闲卡有界数据回放；GPU3/4/5训练占位已恢复。原生产、特殊池和输入锁不变。';
