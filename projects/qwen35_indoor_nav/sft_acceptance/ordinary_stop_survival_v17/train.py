...
obj=u.load('survival17_objective',u.HERE/'objective.py')
...
def trajectory_metrics(m,groups):
    ...
    return dict(any_false_stop=float((w*(neg>0)).sum()),first_legal_stop_recall=float((w*pm*(pos>0)).sum()/(w*pm).sum()),has_positive_weight=float((w*pm).sum()),scope='fixed observed training trajectories; not counterfactual navigation success')
def main(run):
    ...
    old=u.load('survival17_shared_builder',u.HERE.parent/'ordinary_action_repair_v16/train.py')
    ...
    for p in sorted(source.glob('collect/sessions/*/pairs/*/COLLECT.json'),key=lambda p:u.read(p)['rank']):
        ...
        for t,(r,s,priv) in enumerate(zip(records,supervision,private)):
            ...
            if first is None:
                if s['target']:positive=mapping[key];first=t
                elif key not in seen:negative.append(mapping[key]);seen.add(key)
            elif s['target']:quality['positive_observations_after_first_opportunity']+=1
    ...
    u.write(run/'DATA_AUDIT.json',dict(physical_trajectories=640,unique_features=len(h),negative_opportunities=sum(len(t['negative']) for t in trajectories),first_legal_stop_opportunities=sum(t['positive'] is not None for t in trajectories),quality=dict(quality),source_seals=bindings,recovery64_used=False,observed_history_only=True),True)
    ...
    fit=u.load('survival17_old_weights',u.LINE/'sft_acceptance/ordinary_stop_row_v12/fit.py');split=u.read(Path(cfg['manifests'])/'SPLIT.json');fits=[]
    ...
    u.write(run/'TRAIN_RESULT.json',dict(status='TRAINED_PENDING_CLOSED_LOOP',fits=fits,checkpoint_sha256=u.sha(run/'CANDIDATE.pt'),learned_parameters=2049,base_updates=0,wall_seconds=time.time()-start,navigation_benefit=None),True);u.write(run/'TRAIN_PROGRESS.json',dict(status='COMPLETE',unix=time.time()))
