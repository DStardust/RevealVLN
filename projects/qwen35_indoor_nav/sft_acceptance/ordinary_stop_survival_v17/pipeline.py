...
import common as u
review=u.load('survival17_review',u.HERE/'review.py')
resources=u.load('survival17_resources',u.V16/'pipeline.py')
disk=u.load('survival17_disk',u.HERE.parent/'ordinary_stop_coverage_v15/runtime_r2/disk.py')
...
def main():
    ...
    try:
        ...
        report='# 整段路径STOP风险修复\n\n'
        for tag,r in results.items():report+=f"{tag}: {r['complete_pairs']}/{r['planned_pairs']} 对，V13 SR={r['arms']['A']['sr']}，V17 SR={r['arms']['B']['sr']}，ΔSR={r['delta_sr']}。\n\n"
        report+='同一640条训练池、原底模/输入/动作/预算；保护V13运动输出，仅训练STOP残差。逐轨迹负例集合损失和首次合法STOP监督是本次共同修复，未单独分解初始化/损失的因果作用。已有64条恢复数据未用于此次训练；无自动部署或论文贡献结论。\n'
