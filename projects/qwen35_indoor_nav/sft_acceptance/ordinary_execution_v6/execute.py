...
def probe(backend, rows, data, out, binding):
    ...
    before_first = copy.deepcopy(backend.state())
    _, first_metrics = backend.train_chunk([short.decision(0)], backend.zero_memory(), finalize=True)
    ...
    before_second = copy.deepcopy(backend.state())
    _, second_metrics = backend.train_chunk([short.decision(0)], backend.zero_memory(), finalize=True)
    second_update = backend.state()
    differences = {}
    for name, first_value in first_update['trainable'].items():
        delta=(first_value.float()-second_update['trainable'][name].float())
        if bool(delta.abs().max()):
            differences[name]=dict(max_abs=float(delta.abs().max()), l2=float(delta.norm()))
    diagnostics=dict(first_metrics=first_metrics, second_metrics=second_metrics,
        before_trainable_exact=tensor_equal(before_first['trainable'],before_second['trainable'],torch),
        before_optimizer_exact=tensor_equal(before_first['optimizer'],before_second['optimizer'],torch),
        after_optimizer_exact=tensor_equal(first_update['optimizer'],second_update['optimizer'],torch),
        parameter_differences=differences, deterministic_algorithms=torch.are_deterministic_algorithms_enabled())
    write(out/'REPEATABILITY_DIAGNOSTIC.json',diagnostics)
    for name, value in [('repeat_before_a',before_first),('repeat_before_b',before_second),('repeat_after_a',first_update),('repeat_after_b',second_update)]:
        with (out/(name+'.pt')).open('xb') as stream:
            torch.save(value,stream);stream.flush();os.fsync(stream.fileno())
    assert diagnostics['before_trainable_exact'] and diagnostics['before_optimizer_exact'], 'RESTORE_STATE_NOT_EXACT'
...
def train(backend, rows, data, out, binding, protocol, resume_state=None):
    ...
    if resume_state is not None:
        cursor = copy.deepcopy(resume_state['cursor'])
        memory = resume_state['memory']
        if memory is not None:
            memory = memory.to('cuda:0')
        stats = copy.deepcopy(resume_state['stats'])
    segment_initial_decisions = cursor['decisions']
    started = time.monotonic()
    stop_requested = []
    ...
    while cursor['epoch'] < config['epochs']:
        ...
        while cursor['step'] < row['decisions']:
            ...
            if metrics['optimizer_step'] and (cursor['updates'] % protocol['checkpoint_updates'] == 0 or cursor['updates'] in protocol.get('startup_checkpoint_updates',[]) or stop or ending_epoch):
                latest_checkpoint = checkpoint(backend, out, binding, cursor, memory, stats)
            if time.monotonic()-last_report >= protocol['progress_seconds'] or stop:
                ...
                event = dict(status='SEGMENT_COMPLETED' if stop else 'TRAINING', unix=time.time(), cursor=cursor,
                    ...
                    throughput=(cursor['decisions']-segment_initial_decisions)/max(.001,time.monotonic()-started),
...
def main():
    ...
    try:
        ...
        resume = protocol['resume_from']
        path = Path(resume['path']).resolve()
        assert path.is_relative_to(LINE) and ref.sha256(path)==resume['sha256']
        assert ref.sha256(Path(str(path)+'.json'))==resume['receipt_sha256']
        assert not Path('/proc',str(resume['old_worker_pid'])).exists(), 'OLD_WORKER_STILL_PRESENT'
        parent=path.parents[1]
        assert ref.sha256(parent/'PROBE.json')==resume['parent_probe_sha256']
        assert ref.sha256(parent/'SUPERVISOR_RESULT.json')==resume['parent_supervisor_result_sha256']
        resumed = backend.torch.load(path,map_location='cpu',weights_only=True)
        assert resumed['binding']==read(Path(str(path)+'.json'))['binding']
        assert resumed['binding']['reference']==binding['reference'], 'RESUME_DATA_OR_REFERENCE_CHANGED'
        new_probe_compute=dict(backend.total_compute)
        backend.restore(resumed['backend'])
        actual=backend.state()
        assert tensor_equal(actual['trainable'],resumed['backend']['trainable'],backend.torch), 'REAL_RESUME_TRAINABLE'
        assert tensor_equal(actual['optimizer'],resumed['backend']['optimizer'],backend.torch), 'REAL_RESUME_OPTIMIZER'
        backend.total_compute={k:resumed['cumulative_compute'][k]+new_probe_compute[k] for k in new_probe_compute}
        write(out/'RESUME_RECEIPT.json',dict(source=resume,cursor=resumed['cursor'],
            trainable_optimizer_exact=True,cumulative_compute=backend.total_compute,unix=time.time()),exclusive=True)
        train(backend, rows, data, out, binding, protocol, resume_state=resumed)
