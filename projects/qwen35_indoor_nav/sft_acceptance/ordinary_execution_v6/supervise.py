...
def parse_gpu(xml, uuid):
    ...
    return dict(uuid=uuid, total_mib=mib(gpu.findtext('./fb_memory_usage/total')), used_mib=mib(gpu.findtext('./fb_memory_usage/used')), processes=processes)
...
def guard(snap, own_pid, budget):
    assert budget['max_gpu_memory_bytes']//1024**2 + budget['external_gpu_mib'] <= snap['total_mib']-512, 'PARTITIONS_EXCEED_PHYSICAL_CAPACITY'
