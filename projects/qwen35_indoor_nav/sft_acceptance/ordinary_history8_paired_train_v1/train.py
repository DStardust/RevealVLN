...
def main():
    ...
    assert protocol['runtime_allowed'] and protocol['id']=='ORDINARY_HISTORY8_PAIRED_TRAIN_V1'
    for path,digest in protocol['code_sha256'].items():assert r.sha(Path(path))==digest,'SOURCE_CHANGED:'+path
    assert r.sha(HERE/'SELECTED_SAMPLES.jsonl')==protocol['sample_index_sha256']
    ...
    selected=[json.loads(line) for line in (HERE/'SELECTED_SAMPLES.jsonl').read_text().splitlines()]
    ...
    micro=protocol['microbatch'];assert micro==8 and 32%micro==0
    ...
    packing=r.load('paired_packing',HERE/'batching.py')
    ...
    loader=torch.utils.data.DataLoader(dataset,batch_sampler=batches,num_workers=2,prefetch_factor=2,
        pin_memory=True,collate_fn=collate,timeout=90,generator=torch.Generator().manual_seed(1209+rank))
    ...
    worker_ids=[own.inspect(p.pid) for p in iterator._workers]
    r.save(out/f'WORKERS_RANK{rank}.json',dict(rank=rank,rank_pid=os.getpid(),workers=worker_ids),True)
    ...
    try:
        ...
        # Stop and join only this rank's actual DataLoader workers.
        iterator._shutdown_workers()
        assert all(not p.is_alive() for p in iterator._workers)
        ...
        r.save(out/f'FINAL_RANK{rank}.json',dict(rank=rank,updates=completed,decisions=completed*32,
            ...
            data_workers_joined=True,peak_reserved_bytes=torch.cuda.max_memory_reserved(device)),True)
        if rank==0:
