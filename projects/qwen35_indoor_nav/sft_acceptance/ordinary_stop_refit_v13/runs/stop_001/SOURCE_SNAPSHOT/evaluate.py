...
def main(session):
    ...
    enc=u.load('stop13_encoder',u.V16/'encoder.py')
    import torch
    ...
    try:
        ...
        for arm in ('A','B'):
            ...
            proc=subprocess.Popen([p['habitat_python'],'-I','-B',str(u.HERE/'executor.py'),str(child.fileno()),str(session),arm],pass_fds=(child.fileno(),),env=env,stdin=subprocess.DEVNULL,stdout=log,stderr=subprocess.STDOUT)
            child.close();processes.append(proc);sockets.append(parent);logs.append(log);streams[arm]=parent.makefile('rw')
        order=u.read(u.V5/'PAIR_ORDER.json');began=time.time()
        with torch.inference_mode():
            for rank in p['scheduled_ranks']:
                ...
                while any(alive.values()):
                    ...
                    for arm in row['inference_order']:
                        ...
                        final=base if arm=='A' else torch.nn.functional.linear(h,weight,bias)[0]
                        assert torch.isfinite(final).all(),'NONFINITE_METHOD_LOGITS'
                        assert torch.equal(final[:3],base[:3]),'MOTION_LOGITS_CHANGED'
                ...
                result={a:c.audit_episode(folder/a,row['index']) for a in ('A','B')}
                if not diverged:
                ...
                pair=dict(rank=rank,episode_id=row['episode_id'],house=row['house'],A=result['A'],B=result['B'],
                    ...
                    base_state_sha256=initial['sha256'],candidate_sha256=p['candidate_sha256'],state_unchanged=True,
