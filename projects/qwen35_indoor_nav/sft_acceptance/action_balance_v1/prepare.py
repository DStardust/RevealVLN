...
def main():
    ...
    control_sealed=0
    for entry in (control/'SHA256SUMS').read_text().splitlines():
        expected,rel=entry.split('  ',1);p=(control/rel).resolve()
        assert p.is_relative_to(control.resolve()) and digest(p)==expected,rel
        control_sealed+=1
    ...
    sources = [OLD/'SHA256SUMS', OLD/'SPLIT.json', OLD/'EXPERIMENT_SPEC.json', OLD/'result.json',
               OLD/'recovery_r1/policy.py', OUT.parent/'d1_v1/worker.py', control/'SHA256SUMS', control/'SUBSET.json', control/'rank0_EVAL_after.json', OLD/'checkpoints/initial.pt', OLD/'recovery_r1/checkpoints/terminal.pt',
    ...
    assert digest(OUT/'SUBSET.json')==digest(control/'SUBSET.json'),'Matched schedule changed'
    with (OUT/'WEIGHTS.json').open('x') as f:json.dump(w.make_weights(dict(counts)),f,indent=2)
    lock = dict(control_seal_files_verified=control_sealed,weights_sha256=digest(OUT/'WEIGHTS.json'),original_seal_files_verified=sealed, sources={str(p.relative_to(LINE)):digest(p) for p in set(sources)},
