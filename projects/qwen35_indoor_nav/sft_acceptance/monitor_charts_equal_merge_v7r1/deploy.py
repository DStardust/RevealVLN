...
OLD=HERE.parent/'monitor_charts_onpolicy_eval_v6r3/server.py';NEW=HERE/'server.py'
PANE='%294';OLD_PID=2949175
...
def protected_pids():
    ...
    for row in rows:
        ...
        pids.append(old['pid'])
    owner=int(tmux('display-message','-p','-t','q35n_equal_merge_v7r1','#{pane_pid}'))
    assert ident(owner)['argv']==[str(PY),'-I','-S','-B',str(LINE/'reviews/Q35N_ORDINARY_EQUAL_MERGE_V7R1/workflow.py'),'run']
    pids.append(owner)
    return tuple(pids)
