...
OLD=HERE.parent/'monitor_charts_onpolicy_resume_v6r1/server.py';NEW=HERE/'server.py'
PANE='%294';OLD_PID=2918444
...
def check(port):
    ...
    assert d['monitor_version']=='ordinary_onpolicy_eval_v6r3'
...
def protected_pids():
    rows=json.loads((LINE/'sft_acceptance/ordinary_onpolicy_adapt_v6r1/lease_v1/RESTORATION.json').read_text())['holders']
    pids=[3996270,112240,1563749,1353421]
    for row in rows:
        old=row['process_identity'];now=ident(old['pid'])
        assert now['start']==old['starttime_ticks'] and now['cwd']==old['cwd']
        assert ' '.join(now['argv'])==old['command']
        pids.append(old['pid'])
    owner=int(tmux('display-message','-p','-t','q35n_onpolicy_v6r3_eval','#{pane_pid}'))
    assert ident(owner)['argv']==[str(PY),'-I','-S','-B',str(LINE/'reviews/Q35N_ORDINARY_ONPOLICY_ADAPT_V6_EVAL_R3/workflow.py'),'run']
    pids.append(owner)
    return tuple(pids)
