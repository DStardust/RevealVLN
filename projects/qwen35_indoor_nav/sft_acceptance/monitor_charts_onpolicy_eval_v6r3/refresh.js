...
async function refresh(){
 try{
  ...
  el('summary').textContent=n<1000?'正在学习实际偏离时该如何纠正；尚无新导航结论。':'固定训练段已结束，查看下方完整复测结论；未出齐时不判定收益。';
  ...
  const rows=[['旧51301步',.14,.126726765,.367404866],['当前最佳：扩产4k',.21,.18016983923442284,.3658797007353029],['继续到8k（未采用）',.14,.1232098103,.332880856],['低LR8k（未采用）',.13,.1143599416,.380144997],['完整一轮（未采用）',.14,.1298740061,.3537311656],['R2R适配8k（未采用）',.15,.1339120518,.4041111783]];
  if(fin){const v=fin.after.result;rows.push(['本次：4k＋纠错1k',v.sr,v.spl,v.ndtw]);}else rows.push(['本次：结果未完成',null,null,null]);
  ...
  el('resources').textContent='训练占位恢复：'+(t.lease_result?.holders_restored?'已核实':'训练中 / 等待恢复验收')+'；评测状态：'+(t.eval_result?.status||t.eval_progress?.status||'正在载入模型')+'；已完成 '+(t.eval_progress?.completed??0)+' / 100 条';
