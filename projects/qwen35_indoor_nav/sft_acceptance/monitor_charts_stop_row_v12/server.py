...
def collect(include_gpu=True):
    d=m.collect(include_gpu=include_gpu);d['monitor_version']='ordinary_stop_row_v12'
    d['stop_row']=dict(build=read(TRAIN/'BUILD_RESULT.json'),extraction=read(TRAIN/'EXTRACTION_PROGRESS.json'),
      extraction_result=read(TRAIN/'EXTRACTION_RESULT.json'),resource=read(TRAIN/'RESOURCE.json'),launch=read(TRAIN/'LAUNCH_RESULT.json'),
