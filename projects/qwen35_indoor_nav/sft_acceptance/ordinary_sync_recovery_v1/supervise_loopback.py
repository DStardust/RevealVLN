...
def main():
    ...
    protocol = read(HERE / 'PROTOCOL_LOOPBACK.json')
    ...
    for number in range(1, 4):
        ...
        argv = [str(PY), '-B', '-m', 'torch.distributed.run', '--nnodes=1', '--nproc_per_node=3', '--master_addr=127.0.0.1', '--master_port=29572',
                str(HERE / 'train.py'), '--protocol', str(HERE / 'PROTOCOL_LOOPBACK.json'),
