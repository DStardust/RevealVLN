...
def main():
    ...
    for key in ('HF_HOME','XDG_CACHE_HOME','TORCH_HOME','CUDA_CACHE_PATH','TORCHINDUCTOR_CACHE_DIR','TRITON_CACHE_DIR'):
        Path(env[key]).mkdir(parents=True,exist_ok=True)
    (Path(env['XDG_CACHE_HOME'])/'torch/kernels').mkdir(parents=True,exist_ok=True)
    ...
    try:
        ...
        while True:
            ...
            for pid,identity in seen.items():
                current=own.inspect(pid)
                if current and current['state'] not in ('Z','X') and own.stable(current)==identity:
                    memory.append(dict(pid=pid,rss_bytes=rss(pid),start=identity['start']))
            total_rss=sum(x['rss_bytes'] for x in memory)
            ...
            r.save(root/'RESOURCE.json',dict(unix=time.time(),arm=args.arm,gpu=gpu,total_rss_bytes=total_rss,
                output_bytes=size,tree_scan_transient_misses=transient_misses,wall_seconds=time.monotonic()-started,
                owned_pids=sorted(allowed),live_cpu_memory=memory))
