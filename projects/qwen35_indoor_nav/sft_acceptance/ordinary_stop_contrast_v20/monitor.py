...
s=importlib.util.spec_from_file_location('contrast20_previous_monitor',OLD/'ops_recovery_r1/monitor.py');base=importlib.util.module_from_spec(s);s.loader.exec_module(base)
...
def snapshot():
    ...
    long=HERE.parents[1]/'closed_loop_bench/ordinary_long100k_transfer_v1/runs/transfer_001'
    result['long100k']=dict(status=read(long/'STATUS.json',{}),UNSEEN=read(long/'UNSEEN/REVIEW.json',read(long/'UNSEEN/LIVE_RESULT.json')),live=base.live(long))
    result['survival']['data']=read(run/'DATA_AUDIT.json',{}).get('counts',{})
    return result
