...
import common as u
review=u.load('contrast20_review',u.HERE/'review.py')
resources=u.load('contrast20_resources',u.V16/'pipeline.py')
OPS=Path(__file__).resolve().parent
disk=u.load('contrast20_async_disk',OPS/'disk_monitor.py')
...
def main():
    ...
    prior_gpu_hours=sum(u.read(p).get('gpu_hours',0) for p in run.glob('*/sessions/*/attempts/*/EXIT.json'))
    ...
    try:
        ...
        for failure in run.glob('*/sessions/*/FAILURE.json'):raise RuntimeError('UNRESOLVED_WORKER_FAILURE:'+str(failure))
        ...
        sanity=dict(reference=last['reference'],candidate=last['candidate'],rule='Finite real update, frozen motion/base, nonzero actual terminal recall; diagnostic gains not required before one DEV evaluation',admitted=last['candidate']['verified_terminal_stop_recall']>0)
        if (run/'TRAIN_SANITY.json').exists():assert u.read(run/'TRAIN_SANITY.json')==sanity,'TRAIN_SANITY_CHANGED'
        else:u.write(run/'TRAIN_SANITY.json',sanity,True)
        if not sanity['admitted']:raise RuntimeError('ALL_TERMINAL_STOP_COLLAPSED')
        ...
        phase='DEV'
        if (run/'DEV/REVIEW.json').exists():
            assert u.read(run/'DEV/REVIEW.json')==review.summarize(run/'DEV',True),'COMPLETED_DEV_CHANGED'
        else:evaluate(phase)
        dev=u.read(run/'DEV/REVIEW.json')
        ...
        if (run/'DEV_GATE.json').exists():assert u.read(run/'DEV_GATE.json')==gate,'DEV_GATE_CHANGED'
        else:u.write(run/'DEV_GATE.json',gate,True)
        results={'DEV':dev}
        if qualifies:
        ...
        report='# V20 同指令终点/困难负例修复\n\n'
        for tag,r in results.items():report+=f"{tag}: {r['complete_pairs']}/{r['planned_pairs']} 对，V13 SR={r['arms']['A']['sr']}，V20 SR={r['arms']['B']['sr']}，ΔSR={r['delta_sr']}。\n\n"
        report+='同一640条训练池、原底模/输入/动作/预算；保护V13运动输出，仅训练STOP残差。训练屋内真实终点与困难负例配对，保护逐输入V13继续决策；一次固定目标拟合，未做截距校准或选点。已有64条恢复数据未用于此次训练；无自动部署或论文贡献结论。\n'
