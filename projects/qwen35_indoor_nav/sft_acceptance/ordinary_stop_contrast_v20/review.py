...
def summarize(stage,verify=False):
    ...
    return dict(status='VALID_COMPLETE' if complete==n else 'PARTIAL',phase=config['phase'],complete_pairs=complete,planned_pairs=n,missing_ranks=sorted(set(range(n))-set(found)),arms=arms,wins=wins,losses=losses,delta_sr=(len(wins)-len(losses))/n if complete==n else None,houses=houses,max_base_logit_delta=max((r['max_base_logit_delta'] for r in rows),default=None),logits_bitwise_equal=all(r['logits_bitwise_equal'] for r in rows) if rows else None,reference='V13_STOP_ONLY',candidate='V20_TRAJECTORY_STOP',adoption='NOT_AUTOMATICALLY_ADOPTED')
def main(stage):
