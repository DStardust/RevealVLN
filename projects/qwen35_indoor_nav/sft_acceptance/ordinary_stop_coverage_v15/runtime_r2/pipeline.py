...
import common as u
disk=u.load("coverage15_concurrent_disk",Path(__file__).resolve().parent/"disk.py")
...
def main():
    ...
    if (run/'INFRASTRUCTURE_REVISION_R2.json').exists():assert u.read(run/'INFRASTRUCTURE_REVISION_R2.json')==revision
    else:u.write(run/'INFRASTRUCTURE_REVISION_R2.json',revision,True)
