...
def main(session):
    u.verify();p=u.read(session/'CONFIG.json');sys.path.insert(0,str(u.V16));enc=u.load('coverage15_encoder',u.V16/'encoder.py')
    import torch
    ...
    proc=subprocess.Popen([p['habitat_python'],'-I','-B',str(u.HERE/'executor.py'),str(child.fileno()),str(session),'C'],pass_fds=(child.fileno(),),env=env,stdin=subprocess.DEVNULL,stdout=log,stderr=subprocess.STDOUT)
