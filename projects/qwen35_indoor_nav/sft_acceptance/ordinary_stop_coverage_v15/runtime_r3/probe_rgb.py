...
OUT=HERE/'rgb_probe_001';OUT.mkdir(exist_ok=False)
...
try:collect(sim,139,'fresh')
finally:sim.close()
sim=old.build_sim(house,p)
try:
    collect(sim,136,'control');collect(sim,139,'warm')
finally:sim.close()
u.write(OUT/'RESULT.json',dict(rows=results,wall_seconds=time.monotonic()-start,model_forwards=0,navigation_actions=0,scene=house,gpu=1,semantics='same frozen RGB sensor and registered initial pose; repeat sensor reads only'),True)
