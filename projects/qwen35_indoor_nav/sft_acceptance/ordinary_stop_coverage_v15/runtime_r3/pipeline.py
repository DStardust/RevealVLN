...
import common as u
disk=u.load("coverage15_concurrent_disk",u.HERE/"runtime_r2/disk.py")
...
def main():
    ...
    revision=u.read(Path(__file__).resolve().parent/'SOURCE_LOCK.json')
    for path,h in revision['files'].items():assert u.sha(Path(path))==h,'INFRASTRUCTURE_SOURCE_CHANGED'
    if (run/'INFRASTRUCTURE_REVISION_R3.json').exists():assert u.read(run/'INFRASTRUCTURE_REVISION_R3.json')==revision
    else:u.write(run/'INFRASTRUCTURE_REVISION_R3.json',revision,True)
    ...
    def spawn(script,folder,device=None,python=None):
        ...
        command=[python or cfg['torch_python'],'-I','-B',str((Path(__file__).resolve().parent if script in ('collect','evaluate') else u.HERE)/(script+'.py')),str(folder)];log=(attempt/'stdout.log').open('x');proc=subprocess.Popen(command,cwd=u.ROOT,env=env(device),stdin=subprocess.DEVNULL,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
    ...
    def poll(active):
        ...
        while any(not w['done'] for w in active):
            ...
            if time.monotonic()-last_disk>300:
                measurement=disk.measure(run);disk_bytes=measurement['bytes'];u.append(run/'DISK_MEASUREMENTS.jsonl',measurement);last_disk=time.monotonic()
    ...
    def batch(tag,script):
        ...
        for i,device in enumerate(cfg['devices']):
            ...
            p.update(runtime_executor=str(Path(__file__).resolve().parent/'executor.py'),runtime_revision_sha256=u.sha(Path(__file__).resolve().parent/'SOURCE_LOCK.json'))
