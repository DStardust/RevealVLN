...
old=u.load('coverage15_frozen_sim',source)
route=u.load('coverage15_route',u.LINE/'data_pipeline/ordinary_route_teacher_v11/route.py')
    ...
    try:
        for line in stream:
            ...
            if req.get('op')=='reset':
                ...
                rgb=np.ascontiguousarray(obs['rgb'][:,:,:3]);assert rgb.shape==(224,224,3) and np.std(rgb)>0
