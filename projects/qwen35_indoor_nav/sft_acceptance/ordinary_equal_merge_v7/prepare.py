...
REVIEW=LINE/'reviews/Q35N_ORDINARY_EQUAL_MERGE_V7'
...
def build():
    ...
    spec=dict(id='Q35N_EQUAL_PARAMETER_MERGE_V7',alpha=.5,allowed_alphas=[.5],inference_only=True,
      parameter_updates=0,optimizer_resumption_allowed=False,merge='same_named_trainable_parameter_arithmetic_mean',
    ...
    for k,x in a['trainable'].items():
        y=b['trainable'][k];assert x.shape==y.shape and x.dtype==y.dtype==torch.float32
        assert torch.isfinite(x).all() and torch.isfinite(y).all()
        z=(x+y)*.5;assert torch.isfinite(z).all()
        assert torch.equal(x*(1.-0.)+y*0.,x) and torch.equal(x*(1.-1.)+y*1.,y)
        assert torch.equal(z,y*.5+x*.5)
        weights[k]=z
        stats[k]=dict(shape=list(x.shape),source_delta_l2=float(torch.linalg.vector_norm(y.double()-x.double())),
    ...
    common_files={str(f):sha(f) for f in list(HERE.glob('*.py'))+list(HERE.glob('*.json'))+
      [HERE/'PLAN_ZH.md',ckpt]+list(REVIEW.glob('*.py'))+[A,B]}
