...
class Tests(unittest.TestCase):
    ...
    def test_external_guard(self):
        ...
        s=dict(used_mib=522,processes=[dict(pid=1,mib=246),dict(pid=2,mib=246)])
        m.guard(s,None,b)
    ...
    def test_owned_memory(self):
        m=load('supervise');b=dict(external_per_process_mib=768,external_gpu_mib=2048,max_gpu_memory_bytes=28*1024**3)
        s=dict(used_mib=31000,processes=[dict(pid=1,mib=30000)])
        with self.assertRaisesRegex(AssertionError,'OWN_GPU'):m.guard(s,1,b)
    def test_graphics_included(self):
        m=load('supervise')
        s=m.parse_gpu('<nvidia_smi_log><gpu><uuid>GPU-X</uuid><fb_memory_usage><used>500 MiB</used></fb_memory_usage><processes><process_info><pid>2</pid><used_memory>400 MiB</used_memory><type>G</type></process_info></processes></gpu></nvidia_smi_log>','GPU-X')
        self.assertEqual(s['processes'][0]['type'],'G')
    def test_unknown_memory_rejected(self):
        m=load('supervise')
        with self.assertRaises(AssertionError):m.parse_gpu('<nvidia_smi_log><gpu><uuid>GPU-X</uuid><fb_memory_usage><used>N/A</used></fb_memory_usage></gpu></nvidia_smi_log>','GPU-X')
