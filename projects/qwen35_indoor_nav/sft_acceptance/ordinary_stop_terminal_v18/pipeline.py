...
import common as u
review=u.load('terminal18_review',u.HERE/'review.py')
resources=u.load('terminal18_resources',u.V16/'pipeline.py')
disk=u.load('terminal18_disk',u.HERE.parent/'ordinary_stop_coverage_v15/runtime_r2/disk.py')
...
def main():
    ...
    try:
        ...
        sanity=dict(reference=last['reference'],candidate=last['candidate'],rule='terminal recall >=90% of reference and observed-trajectory false STOP rate not increased; FIT only')
        ...
        report='# 整段路径STOP风险修复\n\n'
        for tag,r in results.items():report+=f"{tag}: {r['complete_pairs']}/{r['planned_pairs']} 对，V13 SR={r['arms']['A']['sr']}，V18 SR={r['arms']['B']['sr']}，ΔSR={r['delta_sr']}。\n\n"
