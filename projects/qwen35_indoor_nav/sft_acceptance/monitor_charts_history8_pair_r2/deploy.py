...
OLD=HERE.parent/'monitor_charts_history8_pair_r1/server.py';NEW=HERE/'server.py'
PANE='%294';OLD_PID=3756415;OLD_START=167114618
...
def check(port):
    d=json.loads(fetch(port));assert d['monitor_version']=='ordinary_history8_pair_r2'
    assert d['sr40']['official_episode_count']==1839
    assert d['sr40']['minimum_successes_for_40']==736 and not d['sr40']['pass_goal']
    assert d['sr40']['historical_full']['selected_batch_size']==8
    ...
    assert set(d['paired_history']['evaluation'])=={'control_recent2','treatment_prefix8'}
    assert d['paired_history']['workflow']['live']
...
def protected_pids():
    return (3757402,3757403,3757404,3996270,112240,1563749,1353421,3770736)


def main():
    old=ident(OLD_PID)
    assert old['start']==OLD_START
