...
def collect(include_gpu=True):
    d=parent.collect(include_gpu=include_gpu);d['monitor_version']='ordinary_history8_pair_r2'
    arms={}
    ...
    workflow=read(shared/'WORKFLOW_PROCESS.json')
    evaluation={}
    for arm,dirname in (('control_recent2','ordinary_history2_dev_r1'),('treatment_prefix8','ordinary_history8_dev_r1')):
        case=shared.parent/dirname;out=case/'run_001'
        process=read(out/'PROCESS.json')
        ident=dict(pid=process['pid'],start=process['start_ticks'],
            argv=[str(LINE/'.envs/q35n_qwen_g2_v1/bin/python3'),'-I','-B',str(case/'evaluate.py')],
            cwd=str(LINE.parents[1])) if process else None
        evaluation[arm]=dict(progress=read(out/'PROGRESS.json'),result=read(out/'RESULT.json'),
            launch=read(out/'LAUNCH_RESULT.json'),worker_live=live(ident),worker_pid=process['pid'] if process else None,
            checkpoint_bound=(case/'SOURCE_LOCK.json').is_file(),review=read(case/'MAIN_REVIEW.json'),
            failure=read(out/'INFERENCE_FAILURE.json') or read(out/'LAUNCH_FAILURE.json') or read(out/'AUDIT_FAILURE.json'))
    d['paired_history'].update(evaluation=evaluation,workflow=dict(status=read(shared/'WORKFLOW_STATUS.json'),
        live=live(workflow),pid=workflow['pid'] if workflow else None,failure=read(shared/'WORKFLOW_FAILURE.json')),
        comparison=read(shared/'COMPARISON_RESULT.json'),collected_unix=time.time(),
        first200={arm:read(LINE/'reviews/Q35N_HISTORY_PAIR_R1_CHECKPOINTS'/f'{arm}_FIRST200.json')
            for arm in arms})
    return d
class Handler(parent.Handler):
    def do_GET(self):
        try:
            ...
            elif self.path=='/healthz':self.respond(200,b'{"status":"ok","version":"ordinary_history8_pair_r2"}','application/json')
