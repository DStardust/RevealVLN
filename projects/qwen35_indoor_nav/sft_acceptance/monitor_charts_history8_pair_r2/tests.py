...
assert d['monitor_version']=='ordinary_history8_pair_r2'
...
assert d['paired_history']['first200']['control_recent2']['status']=='PASS_FIRST200_NUMERICAL_ONLY'
json.dumps(d,allow_nan=False)
for name in ('phase','diagnostic','training','training-time','training-resource','loss-chart','navigation','resource','updated','error','eval-phase','eval-live','evaluation','comparison'):assert 'id="'+name+'"' in (h/'index.html').read_text()
...
subprocess.run([shutil.which('node'),str(h/'test_render.js')],check=True,capture_output=True)
