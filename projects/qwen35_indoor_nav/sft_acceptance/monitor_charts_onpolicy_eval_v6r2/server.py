...
TRAIN=HERE.parent/'ordinary_onpolicy_adapt_v6r1'
OLDTRAIN=HERE.parent/'ordinary_onpolicy_adapt_v6'
REVIEW=LINE/'reviews/Q35N_ORDINARY_ONPOLICY_ADAPT_V6_EVAL_R2'
ACCEPT=LINE/'reviews/Q35N_ORDINARY_ONPOLICY_ADAPT_V6_TRANSPORT_R1'
CASE=LINE/'closed_loop_bench/ordinary_onpolicy_adapt_dev_v6r2'
def read(p):
    try:return json.loads(p.read_text())
    except FileNotFoundError:return None
def collect(include_gpu=True):
    d=m.collect(include_gpu=include_gpu);d['monitor_version']='ordinary_onpolicy_eval_v6r2'
    history=[]
    for line in (OLDTRAIN/'formal/attempt_001/PROGRESS.jsonl').read_text().splitlines():
        x=json.loads(line);history.append(dict(updates=x['cursor']['updates'],ce=x['metrics']['mean_ce'],unix=x['unix']))
    try:
    ...
    d['onpolicy_training']=dict(transport_predecessor=read(OLDTRAIN/'formal/attempt_001/RESULT.json'),progress=read(TRAIN/'formal/attempt_001/PROGRESS.json'),
      ...
      workflow_result=read(REVIEW/'WORKFLOW_RESULT.json'),first_acceptance=read(ACCEPT/'FIRST_CHECKPOINT_ACCEPTANCE.json'),
      final_acceptance=read(ACCEPT/'FINAL_CHECKPOINT_ACCEPTANCE.json'),build=read(TRAIN/'BUILD_AUDIT.json'),
