...
def protected_pids():
    ...
    for row in rows:
        ...
        pids.append(old['pid'])
    owner=int(tmux('display-message','-p','-t','q35n_onpolicy_v6r2_eval','#{pane_pid}'))
    assert ident(owner)['argv']==[str(PY),'-I','-S','-B',str(LINE/'reviews/Q35N_ORDINARY_ONPOLICY_ADAPT_V6_EVAL_R2/workflow.py'),'run']
    pids.append(owner)
    return tuple(pids)
