...
TRAIN=HERE.parent/'ordinary_policy_preservation_v9'
...
def collect(include_gpu=True):
    d=m.collect(include_gpu=include_gpu);d['monitor_version']='ordinary_policy_preservation_v9'
    history=[]
    try:
    ...
    d['onpolicy_training']=dict(transport=read(TRAIN/'TRANSPORT_AUDIT.json'),prior_transport_failure=read(LINE/'reviews/Q35N_ORDINARY_FP32_MASTER_V8/WORKFLOW_RESULT.json'),precision_bridge=read(HERE.parent/'ordinary_onpolicy_fp32_master_v8/PRECISION_BRIDGE_AUDIT.json'),progress=read(TRAIN/'formal/attempt_001/PROGRESS.json'),
