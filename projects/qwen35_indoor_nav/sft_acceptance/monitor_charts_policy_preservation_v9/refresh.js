...
async function refresh(){
 try{
  ...
  el('transport').textContent='V9：固定最佳4k作参考，只加入输出KL约束；此前FP32主权重版SR15%未采用。当前训练 '+(n?('已完成 '+n+' 次更新'):'等待首批更新')+'。';
  el('status').textContent=fin?(fin.positive_development_signal?'完整复测达到事前正向门槛':'完整复测未达到正向门槛'):(w.status||'训练准备中');
  el('summary').textContent=n<1000?'正在检验固定源策略约束能否减少旧能力退化；尚无新导航结论。':'固定训练段已结束，查看下方完整复测结论；未出齐时不判定收益。';
  ...
  el('anchor').textContent='额外教师前向：'+(r.teacher_forward_decisions||p.teacher_forward_decisions||0)+' / 99047次；总policy-forward决策：'+(r.total_policy_forward_decisions||p.total_policy_forward_decisions||0)+' / 198094次。最近KL：'+(t.history.at(-1)?.kl?.toFixed(6)||'等待')+'；总loss：'+(t.history.at(-1)?.loss?.toFixed(4)||'等待')+'。';
  const rows=[['旧51301步',.14,.126726765,.367404866],['当前最佳：扩产4k',.21,.18016983923442284,.3658797007353029],['继续到8k（未采用）',.14,.1232098103,.332880856],['低LR8k（未采用）',.13,.1143599416,.380144997],['完整一轮（未采用）',.14,.1298740061,.3537311656],['R2R适配8k（未采用）',.15,.1339120518,.4041111783],['原精度纠错1k（未采用）',.19,.17527383450573364,.3933786870665479],['FP32主权重1k（未采用）',.15,.1342932342183672,.3143712169898085]];
  if(fin){const v=fin.after.result;rows.push(['本次：4k＋固定输出约束1k',v.sr,v.spl,v.ndtw]);}else rows.push(['本次：结果未完成',null,null,null]);
