...
def main():
    assert os.environ.get('CUDA_VISIBLE_DEVICES')=='' and os.environ['TMPDIR']==str(LINE/'.t11')
    import torch
    from torch.utils.data import TensorDataset,DataLoader
    assert not torch.cuda.is_initialized()
    assert tempfile.gettempdir()==str(LINE/'.t11')
    address=str(LINE/'.t11'/'listener_probe')
    ...
    result=dict(status='PASS',unix=time.time(),actual_unix_socket_bind=True,torch_loader_workers=2,
      ...
      max_expected_worker_socket_path_bytes=len(str(LINE/'.t11').encode())+len('/pymp-abcdefgh/listener-abcdefgh'),
