...
def freeze():
    ...
    for name in ('common.py','evaluate.py','executor.py','path_metrics.py','official_distance.py','aggregate.py','launch.py'):
        ...
        if name not in ('common.py','evaluate.py','aggregate.py'):assert text==r.parent.source(name)
        checks.append(name)
    ...
    template.update(id='Q35N_ORDINARY_ACTION_ALIGNED_HISTORY_DEV_V10',checkpoint_role='base4000_plus_action_aligned1000_optimizer5000_pure_model',
         ...
         sample_index_sha256=read(TRAIN/'PROTOCOL_FILESTORE.json')['sample_index_sha256'],
         main_criterion='SR delta >0, SPL delta >=0, nDTW delta >=-0.01 vs pure4000',
         frame_stride=8,selected_input_audit_required=True,controller_enabled=False,checkpoint_selection='fixed correction1000, optimizer5000; no intermediate selection')
    save(HERE/'PROTOCOL_TEMPLATE.json',template)
    save(HERE/'CPU_TEST_RESULT.json',dict(passed=True,checks=checks,simulator_metrics_unchanged=True,policy_window_changed=True,aligned_cpu_tests=read(TRAIN/'CPU_TEST_RESULT.json'),
         batch_size=1,frame_stride=8,controller_enabled=False))
    import runpy
...
def bind():
    ...
    import runpy
    launched=runpy.run_path(str(HERE/'launch.py'),run_name='CPU_FULL_LAUNCHER_PREFIX')
    source=runpy.run_path(str(HERE/'reuse.py'),run_name='CPU_SOURCE')
    tree=ast.parse(source['source']('launch.py'));fn=next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='main')
    prefix=[]
    for n in fn.body:
        if isinstance(n,ast.Assign) and any(isinstance(t,ast.Name) and t.id=='lease' for t in n.targets):break
        prefix.append(n)
    assert len(prefix)==3
    exec(compile(ast.fix_missing_locations(ast.Module(body=prefix,type_ignores=[])),'ACTUAL_LAUNCHER_PREFIX','exec'),dict(launched))
    assert not (HERE/'run_001').exists()
    print('BOUND_FIXED_END_AND_ACTUAL_LAUNCHER_PREFIX_PASS')
