...
def main(session):
    ...
    enc=u.load('long100k_encoder',u.V16/'encoder.py')
    import torch
    ...
    Candidate=u.load('long100k_runtime_adapter',u.HERE/'adapter.py').Candidate
    candidate=Candidate(p,'cuda')
    original=model.trainable_state(policy)
    assert initial['sha256']==p['memory_base_state_sha256'],'MEMORY_ENCODER_BASE_CHANGED'
    ...
    write('HEAD_IDENTITY.json',dict(A=p['reference_checkpoint_sha256'],B=p['candidate_sha256'],reference=reference_stamp,candidate=candidate.identity()),True)
    candidate_stamp=candidate.identity()
    ...
    try:
        ...
        with torch.inference_mode():
            for j,i in enumerate((0,1829,3658)):
                h,z,processed,timing=gold(j);old=cache['logits'][i];cand=candidate.first(h,z).cpu();current=z[0].cpu()
                cached_cand=candidate.first(cache['features'][i].float().unsqueeze(0).cuda(),cache['logits'][i].float().unsqueeze(0).cuda()).cpu()
        ...
        with torch.inference_mode():
            for rank in p['scheduled_ranks']:
                ...
                candidate.reset()
                reset_identity=candidate.state_identity()
                ...
                while any(alive.values()):
                    ...
                    for arm in row['inference_order']:
                        ...
                        detail={}
                        if arm=='A':final=torch.nn.functional.linear(h,ref_weight,ref_bias)[0]
                        else:final,detail=candidate.step(h,z)
                        ...
                        record=dict(step=step,raw=raw,processed=processed,base_logits=base.cpu().tolist(),logits=final.cpu().tolist(),
                            ...
                            repeated_input=key in seen[arm],feature_sha256=c.tensor_identity(h),**detail,**timing)
                ...
                assert candidate.updates==result['B']['steps'],'MEMORY_UPDATE_COUNT'
                now=c.model_identity(policy);unchanged=now['sha256']==initial['sha256']
                assert unchanged and candidate_stamp==candidate.identity(),'MODEL_STATE_CHANGED'
                ...
                pair=dict(rank=rank,episode_id=row['episode_id'],house=row['house'],A=result['A'],B=result['B'],
                    memory_reset=reset_identity,memory_updates=candidate.updates,first_action_divergence=first,input_prefix_matched=True,base_action_prefix_matched=True,
