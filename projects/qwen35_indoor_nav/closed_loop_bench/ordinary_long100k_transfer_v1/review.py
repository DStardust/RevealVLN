...
def summarize(stage,verify=False):
    ...
    return dict(status='VALID_COMPLETE' if complete==n else 'PARTIAL',phase=config['phase'],complete_pairs=complete,planned_pairs=n,missing_ranks=sorted(set(range(n))-set(found)),arms=arms,wins=wins,losses=losses,delta_sr=(len(wins)-len(losses))/n if complete==n else None,houses=houses,max_base_logit_delta=max((r['max_base_logit_delta'] for r in rows),default=None),logits_bitwise_equal=all(r['logits_bitwise_equal'] for r in rows) if rows else None,reference='V13_STOP_ONLY',candidate='EXPANDED_MONOTONIC_100000',adoption='NOT_AUTOMATICALLY_ADOPTED')
def main(stage):
    r=summarize(stage,True)
    exposure=u.read(u.HERE/'EXPOSURE_AUDIT.json');order=u.read(Path(u.read(stage/'STAGE.json')['order_path']))
    rows=[u.read(p) for p in stage.glob('sessions/*/pairs/*/PAIR.json')]
    for row in rows:
        assert row['memory_updates']==row['B']['steps'],'MEMORY_UPDATE_COUNT'
        for name,value in row['memory_reset'].items():
            assert value['sha256'],'MISSING_MEMORY_RESET_FINGERPRINT'
    r['exposure_strata']={}
    for group in sorted({x['stratum'] for x in exposure['public_unseen_houses'].values()}):
        houses={h for h,x in exposure['public_unseen_houses'].items() if x['stratum']==group}
        planned=sum(x['house'] in houses for x in order);rr=[x for x in rows if x['house'] in houses]
        arms={}
        for arm in ('A','B'):
            passed=sum(x[arm]['success'] for x in rr)
            arms[arm]=dict(successes=passed,complete=len(rr),planned=planned,sr=passed/planned if len(rr)==planned else None,
                           identification_bounds=[passed/planned,(passed+planned-len(rr))/planned])
        r['exposure_strata'][group]=dict(houses=sorted(houses),arms=arms,delta_sr=(arms['B']['successes']-arms['A']['successes'])/planned if len(rr)==planned else None)
    r['exposure_note']=exposure['scope'];u.write(stage/'REVIEW.json',r);return r
