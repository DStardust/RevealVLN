...
REVIEW=LINE/'reviews/Q35N_ORDINARY_R2R_ADAPT_V5'
...
def freeze():
    ...
    template.update(id='Q35N_ORDINARY_R2R_ADAPT_DEV_V5',checkpoint_role='r2r_adapt_8000_pure_model',
         ...
         controller_enabled=False,checkpoint_selection='fixed final 8000 R2R source continuation')
    ...
    save(HERE/'PENDING_SEAL.json',dict(status='AWAIT_FIXED_R2R_8000',files={str(p):sha(p) for p in paths}))
...
def bind():
    ...
    assert final['status']=='STOPPED' and final['stop']==['BUDGET:max_updates'] and final['cursor']==tp['expected_final_cursor']
    ...
    save(HERE/'SOURCE_LOCK.json',dict(files=files,scope='same 100, fixed pure R2R-adapted model at8000',training_updates=0))
    save(HERE/'MAIN_REVIEW.json',dict(status='PASS_FOR_FIXED_R2R_8000_DEV',source_lock_sha256=sha(HERE/'SOURCE_LOCK.json'),
