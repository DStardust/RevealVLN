...
def main():
    ...
    previous_common=c.load('previous_full_common',HERE.parent/'r2r_ce_full_v1/common.py')
    previous_common.verify_lock()
    ...
    previous=json.loads((HERE.parent/'r2r_ce_full_v1/PROTOCOL.json').read_text())
    observed_unix=previous['checkpoint_selected_unix'];receipts=[Path(previous['checkpoint']+'.json')]
    ...
    p.update(id='Q35N_R2R_CE_FULL_V2',checkpoint=str(checkpoint),checkpoint_sha256=c.sha(checkpoint),checkpoint_updates=updates,
        ...
        checkpoint_selection='unchanged checkpoint 41800 from original full-v1 latest selection; versioned geometry-guard repair, no weight reselection',
    ...
    original_selection=json.loads((HERE.parent/'r2r_ce_full_v1/CHECKPOINT_SELECTION.json').read_text())
    c.write(HERE/'CHECKPOINT_SELECTION.json',dict(unix=observed_unix,selected=str(checkpoint),receipt=receipt,
        latest_observed_receipt=str(receipt_path),total_completed_receipts=original_selection['total_completed_receipts'],
        reselected=False,recovery_version_created_unix=time.time(),original_selection=str(HERE.parent/'r2r_ce_full_v1/CHECKPOINT_SELECTION.json')),True)
    fixtures=[[],[]]
    for i in range(8):
    ...
    geometry=json.loads((HERE/'GEOMETRY_PREFLIGHT.json').read_text());assert geometry['all_count']==1839 and geometry['all_reachable']
    assert all(r['index']==i and r['episode_id']==e['episode_id'] and len(e['goals'])==1 for i,(r,e) in enumerate(zip(geometry['rows'],episodes)))
    navsource=c.ROOT/'third_party/habitat-lab/habitat/sims/habitat_simulator/habitat_simulator.py'
    files[str(navsource)]=c.sha(navsource)
    previous_lock=HERE.parent/'r2r_ce_full_v1/SOURCE_LOCK.json';files[str(previous_lock)]=c.sha(previous_lock)
    auth=c.LINE/'authorizations/ORDINARY_NAVBENCH_FULL_V2_20260911.json';files[str(auth)]=c.sha(auth)
