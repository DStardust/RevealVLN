...
def main():
    ...
    result=dict(status='COMPLETE' if full else 'INCOMPLETE',unix=time.time(),checkpoint_updates=p['checkpoint_updates'],
        ...
        **{k:overall[k] if full else None for k in ('sr','spl','ne_m','osr','ndtw','sdtw')},
        ...
        recovery_note='Full restart after v1 invalid cached-distance assertion; original checkpoint and all episodes retained',
        predecessor_completed=13,predecessor_environment_actions=3279,
        total_environment_actions_including_failed_predecessor=3279+sum(x['steps'] for x in rows),
        distance_backend='unmodified Habitat-Lab 0.1.7 geodesic_distance body; full frozen CPU preflight gate',
