...
c=load('common');metric_module=c.load('official_metrics',c.TINY/'metrics.py');path_metrics=load('path_metrics')
LANE=int(sys.argv[2])
OUT=HERE/'run_001'/'lanes'/f'lane_{LANE:02d}'
...
def main():
    ...
    schedule=c.schedules(p['episode_count'],p['lanes'])[LANE]
    ...
    try:
        for line in stream:
            ...
            if reset:
                ...
                if steps%100==0 or done:picture(rgb,index,steps)
                if done:
