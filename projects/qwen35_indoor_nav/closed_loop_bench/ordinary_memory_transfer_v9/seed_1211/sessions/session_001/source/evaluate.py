...
MEMORY=LINE/'research/continuation_memory_v1/natural_transfer_v9'
sys.path.insert(0,str(V5))
import common as c
base_evaluate=c.load('v9_base_evaluate',V5/'evaluate.py')
ARMS=('A','B','C','D')
...
def main(session):
    ...
    try:
        ...
        assert initial['sha256']==c.read(MEMORY/'run_001/FEATURE_RESULT.json')['base_state_sha256'],'FEATURE_ENCODER_DIFFERS_FROM_DEPLOYED_BASE'
        import torch
        mem_model=c.load('v9_navigation_memory',MEMORY.parent/'query_reader_repair_v2/model.py')
        ...
        with torch.inference_mode():
            for rank in p['scheduled_ranks']:
                ...
                while any(alive.values()):
                    ...
                    for arm in inference_order:
                        ...
                        if arm in heads:
                            ...
                            method=heads[arm].action_logits(memories[arm],logits.float())
                ...
                c.write(folder/'QUARTET.json',dict(rank=rank,method_seed=seed,index=row['index'],episode_id=episodes['A']['episode_id'],
                ...
                if len(completed) in (5,20,100):
                    seal()
                    review=c.load('v9_milestone_review',HERE/'review.py')
