...
MEMORY=LINE/'research/continuation_memory_v1/cold_start_repair_v8'
sys.path.insert(0,str(V5))
import common as c
base_evaluate=c.load('v8_base_evaluate',V5/'evaluate.py')
ARMS=('A','B','C','D')
...
def main(session):
    ...
    try:
        ...
        import torch
        mem_model=c.load('v8_navigation_memory',MEMORY.parent/'query_reader_repair_v2/model.py')
        ...
        with torch.inference_mode():
            for rank in p['scheduled_ranks']:
                ...
                if len(completed) in (5,20,100):
                    seal()
                    review=c.load('v8_milestone_review',HERE/'review.py')
                    c.write(session/f'MILESTONE_{len(completed):03d}.json',review.summarize(),True)
