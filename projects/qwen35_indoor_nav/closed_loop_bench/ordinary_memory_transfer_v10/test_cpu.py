...
class Tests(unittest.TestCase):
    def test_all_declared_runtime_controls(self):
        self.assertEqual(evaluate.ARMS,('A','B','C','D','E'))
        self.assertEqual(review.ARMS,evaluate.ARMS)
        self.assertIn(('D','C'),evaluate.CONTRASTS)
        self.assertIn(('E','C'),evaluate.CONTRASTS)
        self.assertEqual(review.CONTRASTS,evaluate.CONTRASTS)
        self.assertEqual(len(evaluate.CONTRASTS),10)
    ...
    def test_channel_memory_reset_and_budget(self):
        module=evaluate.c.load('v10_test_mem',evaluate.MEMORY/'model.py')
    ...
    def test_incomplete_seeds_cannot_produce_complete_benefit(self):
        with patch.object(review.c,'read',return_value={'method_seeds':[1209,1210,1211]}), \
             patch.object(review,'summarize',side_effect=[{'complete_groups':100},{'complete_groups':20},{'complete_groups':0}]):
            result=review.aggregate()
        self.assertEqual(result['complete_groups'],120)
        self.assertEqual(result['episode_executions'],600)
    ...
    def test_wrong_seed_receipt_rejected(self):
        with tempfile.TemporaryDirectory(dir=HERE) as folder:
            ...
            (session/'STATE_SEAL_001.json').write_text(json.dumps(dict(unchanged=True,group_ranks=[0])))
            (pair/'GROUP.json').write_text(json.dumps(dict(rank=0,method_seed=1210,valid_behavioral_group=True)))
