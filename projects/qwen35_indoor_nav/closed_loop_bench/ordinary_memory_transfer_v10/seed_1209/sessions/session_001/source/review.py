"""Whole-group denominators and paired SR effects; partial progress is explicit."""
import json
import math
...
def committed(seed=1209):
    ...
    for session in sorted((HERE/f'seed_{seed}'/'sessions').glob('session_*')):
        ...
        for path in session.glob('STATE_SEAL_*.json'):
            seal=c.read(path)
            if seal['unchanged']:covered.update(seal['group_ranks'])
        for path in session.glob('pairs/pair_*/GROUP.json'):
            row=c.read(path)
            assert row['method_seed']==seed, 'SEED_BINDING_MISMATCH'
            if row['rank'] not in covered:continue
            assert row['valid_behavioral_group']
            assert row['rank'] not in found,'DUPLICATE_COMPLETE_GROUP'
...
def summarize(seed=1209,verify=False):
    ...
    if not rows:return dict(status='NO_COMPLETE_GROUP',complete_groups=0,expected_groups=100)
    arms={}
    for arm in ARMS:
    ...
    return dict(status='VALID_COMPLETE' if len(rows)==100 else 'VALID_PARTIAL_DEVELOPMENT',
        complete_groups=len(rows),expected_groups=100,episode_executions=len(rows)*len(ARMS),
        missing_ranks=sorted(set(range(100))-set(found)),arms=arms,
        comparisons={a+b:contrast(rows,a,b) for a,b in CONTRASTS},
...
def aggregate(verify=False):
    ...
    complete=all(r['complete_groups']==100 for r in reports.values())
    output=dict(status='VALID_COMPLETE' if complete else 'PARTIAL_FIXED_SEED_REPLICATION',
        seeds=seeds,per_seed=reports,complete_groups=sum(r['complete_groups'] for r in reports.values()),
        expected_groups=100*len(seeds),episode_executions=len(ARMS)*sum(r['complete_groups'] for r in reports.values()),
    ...
    output.update(comparisons=comparisons,unique_navigation_episodes=100,
        ...
        method_increment_supported=comparisons['BC']['development_support'] and comparisons['DC']['development_support'] and comparisons['EC']['development_support'])
    return output
