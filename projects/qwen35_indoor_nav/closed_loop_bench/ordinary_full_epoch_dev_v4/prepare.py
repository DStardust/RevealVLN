...
def freeze():
    ...
    template.update(id='Q35N_ORDINARY_FULL_EPOCH_DEV_V4',checkpoint_role='full_epoch_31059_pure_model',
         checkpoint=str(TRAIN/'formal/attempt_001/checkpoint_000031059.pt'),checkpoint_sha256=None,
         checkpoint_updates=31059,training_protocol_sha256=sha(TRAIN/'PROTOCOL_FILESTORE.json'),
         main_criterion='SR delta >0, SPL delta >=0, nDTW delta >=-0.01 vs pure4000',
         controller_enabled=False,checkpoint_selection='fixed final 31059 at epoch=1, position=0')
    ...
    save(HERE/'PENDING_SEAL.json',dict(status='AWAIT_FIXED_EPOCH_END',files={str(p):sha(p) for p in paths}))
...
def bind():
    ...
    assert final['global_decisions']==final['charged_compute_decisions']==tp['expected_epoch_unique_decisions']
    ...
    assert str(ckpt)==final['latest_checkpoint'] and receipt['cursor']==tp['expected_final_cursor']
    ...
    save(HERE/'SOURCE_LOCK.json',dict(files=files,scope='same 100, fixed pure model at one full epoch',training_updates=0))
    save(HERE/'MAIN_REVIEW.json',dict(status='PASS_FOR_FIXED_FULL_EPOCH_DEV',source_lock_sha256=sha(HERE/'SOURCE_LOCK.json'),
