...
def main():
    ...
    for session in sorted((HERE/'sessions').glob('session_*')):
        ...
        if (session/'RUNTIME_IDENTITY.json').exists():
            ...
            block=identity_blocks[session.name]
            block['A']=metrics([r['episodes']['A'] for r in rows if r['session']==session.name])
            block['B']=metrics([r['episodes']['B'] for r in rows if r['session']==session.name])
            block['delta_sr']=block['B']['sr']-block['A']['sr'] if block['completed_pairs'] else None
    if len({r['protocol_sha256'] for r in rows})>1:
        raise c.PairError('MIXED_PROTOCOLS_CANNOT_BE_POOLED')
    bit_checks={}
    for r in rows:
        folder=(HERE/r['artifact']).parent
        a=c.records(folder/'A/POLICY_STEPS.jsonl')[:r['audit']['prefix_decisions']]
        b=c.records(folder/'B/POLICY_STEPS.jsonl')[:r['audit']['prefix_decisions']]
        bit_checks[r['rank']]=all(struct.pack('<4f',*x['logits'])==struct.pack('<4f',*y['logits']) for x,y in zip(a,b))
    arm_stats={}
    for arm in ('A','B'):
    ...
    result=dict(protocol='V5 behavioral pair protocol, not V3/V4 bitwise acceptance',planned_pairs=100,
        aggregation_source_sha256=c.sha(HERE/'aggregate.py'),
        ...
        arms=arm_stats,audit=dict(input_prefix_matched=all(r['audit']['input_prefix_matched'] for r in rows) if rows else None,
            ...
            logits_bitwise_equal=all(bit_checks.values()) if rows else None,
            bitwise_measurement='IEEE754 float32 bytes, independently checked from saved logits including signed zero',
