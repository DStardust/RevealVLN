#!/usr/bin/env python3
"""Open the sealed MF3M ranks-18--23 shadow exactly once."""
...
from scripts.select_rxr_uad_robust_top2_mf3m import (  # noqa: E402
    OUT as DEVELOPMENT,
    collect,
    summarize,
)
...
SELECTION = DEVELOPMENT / "MF3M_DEVELOPMENT_SELECTION.json"
OUT = ROOT / "artifacts/evaluation/mf3m_robust_top2_shadow_gate_v1"
...
def main() -> int:
    ...
    if not (
        ...
        and selection.get("ranks18_23_payload_read") is False
    ):
        raise RuntimeError("MF3M development does not authorize fresh shadow")
    manifest = json.loads(DATA.read_text())
    counts = manifest.get("counts", {})
    if not (
        manifest.get("status") == "PASS"
        and counts.get("fit") == 519
        and counts.get("calibration") == 112
        and counts.get("diagnostic", 0) >= 330
        and counts.get("shadow", 0) >= 330
        and all(
        ...
        raise RuntimeError("MF3M ranks18-23 manifest gate failed")
    ...
    shadow = summarize(
        rows, float(rule["mad_weight"]),
        float(rule["robust_advantage_threshold"]),
    )
    ...
    atomic_json(OUT / "MF3M_SHADOW_GATE.json", {
        "schema_version": "revealnav-mf3m-shadow-gate/1",
        ...
        "ranks18_23_payload_read": True,
        "fresh_data": {
            "path": str(DATA.relative_to(ROOT)),
            "bytes": DATA.stat().st_size,
            "sha256": sha256_file(DATA),
            "shadow_episodes": counts["shadow"],
        },
