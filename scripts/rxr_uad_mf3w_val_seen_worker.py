#!/usr/bin/env python3
"""Run one val_seen episode with the train-gated MF3V lower-tail candidate."""
...
def main() -> int:
    ...
    if args.mode == "uncertainty":
        ...
        base._CONTROLLER = base.MF3KTop2Controller(
            torch.device("cuda:0"), controller_trace, revision="mf3v"
        )
        # MF3W is deliberately a train-calibrated lower-tail-only candidate:
        # retain the frozen lower threshold and remove only the MF3V upper cap.
        base._CONTROLLER.parameters["score_upper_threshold"] = 1.0e9
    ...
    sys.argv = [
        "run.py", "--exp_name", f"mf3w_{args.mode}_{args.episode_id}",
    ...
    summary = {
        "schema_version": "revealnav-mf3w-rxr-worker/1",
        "status": "RUNNING", "episode_id": args.episode_id,
        "mode": args.mode, "revision": "mf3w", "split": "val_seen",
        "public_unseen_accessed": False, "threshold_tuned_on_val_seen": False,
        "method_scope": "mf3v_train_gated_lower_tail_candidate",
