#!/usr/bin/env python3
"""Seal the train-shadow gate for MF3ZB temporal-maturity UAD."""
...
OUT = ROOT / "artifacts/evaluation/mf3zb_temporal_maturity_shadow_gate_v1"
DESIGN = ROOT / "artifacts/design/METHOD_FREEZE_3ZB_TEMPORAL_MATURITY.md"
MINIMUM_DECISION_STEP = 3
LOW_QUANTILE = 0.985
...
def main() -> int:
    ...
    for sequence in shadow:
        for step, row in enumerate(sequence):
            if (
                row is not None
                and step >= MINIMUM_DECISION_STEP
                and lower < score(row) <= upper
            ):
                selected.append(row)
                break
    ...
    gates = {
        ...
        "rescue_vs_harm_wilson_lower_above_half": (
            summary["rescue_vs_harm_wilson95_lower"] > 0.5
        ),
    }
    ...
    rule = {
        ...
        "final_training_threshold": lower,
        "score_upper_quantile": UPPER_QUANTILE,
        "score_upper_threshold": upper,
        "persistence_steps": 1,
        "minimum_decision_step": MINIMUM_DECISION_STEP,
    }
    atomic_json(OUT / "MF3ZB_SHADOW_GATE.json", {
        "schema_version": "revealnav-mf3zb-shadow-gate/1",
        ...
        "prior_unseen_development_result": {
            ...
            "used_only_to_identify_early_override_failure_mode": True,
        },
    })
    print(json.dumps({"status": "SHADOW_GATE_PASS" if passed else "SHADOW_GATE_FAIL", "rule": rule, "shadow": summary, "control": control, "gates": gates}, indent=2))
