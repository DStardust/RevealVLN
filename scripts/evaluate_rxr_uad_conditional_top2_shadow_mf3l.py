#!/usr/bin/env python3
"""Open the sealed MF3L ranks-18--23 shadow exactly once."""
...
from scripts.select_rxr_uad_conditional_top2_mf3l import (  # noqa: E402
    OUT as DEVELOPMENT,
    collect,
    load_models,
    summarize,
...
SELECTION = DEVELOPMENT / "MF3L_DEVELOPMENT_SELECTION.json"
OUT = ROOT / "artifacts/evaluation/mf3l_conditional_top2_shadow_gate_v1"
...
def main() -> int:
    ...
    if not (
        ...
        raise RuntimeError("MF3L development does not authorize fresh shadow")
    ...
    if not (
        ...
        raise RuntimeError("MF3L ranks18-23 manifest gate failed")
    ...
    shadow = summarize(
        rows, float(rule["conditional_advantage_threshold"])
    )
    ...
    atomic_json(OUT / "MF3L_SHADOW_GATE.json", {
        "schema_version": "revealnav-mf3l-shadow-gate/1",
