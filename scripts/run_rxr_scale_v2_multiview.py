#!/usr/bin/env python3
"""Run scale-v2 multi-view rendering over 24 isolated GPU shards."""
...
SCRIPT = ROOT / "scripts/build_rxr_scale_v2_multiview.py"
BASE = ROOT / "artifacts/phase1/rxr_train_expansion/scale_v2"
STATUS = BASE / "RXR_SCALE_V2_MULTIVIEW_STATUS.json"
SHARDS = 24
GPUS = tuple(range(8))
...
def run_one(shard: int) -> dict:
    ...
    log_dir = BASE / "automatic/multiview/worker_logs"
...
def main() -> int:
    state = {
        "schema_version": "revealnav-rxr-scale-v2-multiview-status/1",
        "status": "RUNNING",
        "pid": os.getpid(),
        "started": time.time(),
    ...
    state["status"] = "SCALE_V2_MULTIVIEW_PASS" if result.returncode == 0 else "FAIL"
