#!/usr/bin/env python3
"""Run one frozen MF3V RxR val_unseen episode without changing the model path."""
...
def main() -> int:
    ...
    # The imported MF3V controller is frozen; only the evaluation-scope audit
    # field changes for this explicitly authorized unseen harness.
    ...
    sys.argv = [
        "run.py", "--exp_name", f"mf3v_{args.mode}_{args.episode_id}",
    ...
    summary = {
        "schema_version": "revealnav-mf3v-rxr-unseen-worker/1",
        "status": "RUNNING", "episode_id": args.episode_id,
        "mode": args.mode, "revision": "mf3v", "split": "val_unseen",
        "public_unseen_accessed": True, "public_unseen_authorized": True,
        "threshold_tuned_on_val_unseen": False,
        "checkpoint": None if base._CONTROLLER is None else base._CONTROLLER.checkpoint,
        "method_scope": "frozen_mf3v_engineering_unseen_pilot",
    }
