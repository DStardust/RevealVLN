#!/usr/bin/env python3
"""Train or aggregate the fixed primary-only structured U/A/D revision."""
...
from revealnav_mf2r1 import StructuredUADLoss  # noqa: E402
...
OUT = ROOT / "artifacts/evaluation/mf2_structured_uad_v1"
PROTOCOL = OUT / "RXR_STRUCTURED_UAD_PROTOCOL_V1.json"
AGGREGATE = OUT / "RXR_STRUCTURED_UAD_COMPARISON_V1.json"
...
def preconditions() -> tuple[dict, dict]:
    ...
    if not (
        ...
        and baseline["selected"]["balanced_full_ree_hidden_dim"] == HIDDEN_DIM
        and baseline["selected"]["balanced_history_direct_uad_hidden_dim"]
        == HIDDEN_DIM
    ):
        raise RuntimeError("structured U/A/D precondition failed")
...
def train(seed: int, device: torch.device) -> int:
    ...
    state_weights, _, binary_counts, uad_counts = training_weights(train_set)
    random.seed(seed)
    np.random.seed(seed)
    ...
    model = RevealOptionHeads(768, HIDDEN_DIM, 4).to(device)
    objective = StructuredUADLoss(
        RevealOptionLossConfig(
            state_pos_weights=tuple(float(value) for value in state_weights)
        ),
        uad_weight=protocol["uad_loss_weight"],
    )
    ...
    for epoch in range(1, 21):
        ...
        with torch.no_grad():
            for cpu_batch in development_loader:
                ...
                development_uad += float(losses["uad"]) * size
        ...
        history.append({
            ...
            "train_uad": train_uad / train_count,
            "development_total": native_loss,
            "development_uad": development_uad / development_count,
    ...
    checkpoint = run_dir / "structured_uad_full_ree.pt"
    best = max(
        history,
        key=lambda row: (
    ...
    torch.save({
        "schema_version": "revealnav-mf2-structured-uad-checkpoint/1",
    ...
    result = {
        "schema_version": "revealnav-mf2-structured-uad-run/1",
        "status": "STRUCTURED_UAD_PRIMARY_RUN_COMPLETE",
        "seed": seed,
        "hidden_dim": HIDDEN_DIM,
        "uad_loss_weight": 1.0,
        "results": {"structured_uad_full_ree": best_metrics},
...
def aggregate() -> int:
    ...
    history = []
    for seed in SEEDS:
        row = json.loads((OUT / f"seed_{seed}/result.json").read_text())
        baseline = json.loads(
            (BASELINE / f"h128_seed_{seed}/result.json").read_text()
        )
        if row.get("status") != "STRUCTURED_UAD_PRIMARY_RUN_COMPLETE":
            raise RuntimeError(f"incomplete structured run: {seed}")
        structured.append(row["results"]["structured_uad_full_ree"])
        frozen_full.append(baseline["results"]["balanced_full_ree"])
        history.append(baseline["results"]["balanced_history_direct_uad"])
    names = (
    ...
    for model_name, rows in (
        ("frozen_full_ree", frozen_full),
        ("structured_uad_full_ree", structured),
        ("frozen_history_direct_uad", history),
    ):
        results[model_name] = {
            name: summarize([row[name] for row in rows]) for name in names
        }
    structured_macro = results["structured_uad_full_ree"]["macro_f1"]
    full_macro = results["frozen_full_ree"]["macro_f1"]
    history_macro = results["frozen_history_direct_uad"]["macro_f1"]
    structured_false = results["structured_uad_full_ree"]["false_ready_rate"]
    history_false = results["frozen_history_direct_uad"]["false_ready_rate"]
    gates = {
        "macro_f1_mean_at_least_history": (
            structured_macro["mean"] >= history_macro["mean"]
        ),
        "macro_f1_improves_over_frozen_full_in_at_least_two_seeds": sum(
            revised > frozen for revised, frozen in zip(
                structured_macro["values"], full_macro["values"]
            )
        ) >= 2,
        "mean_false_ready_no_higher_than_history": (
            structured_false["mean"] <= history_false["mean"]
        ),
    }
    solved = all(gates.values())
    value = {
        "schema_version": "revealnav-mf2-structured-uad-comparison/1",
        "status": (
            "STRUCTURED_UAD_GATE_PASS"
            if solved else "RELATIONAL_REVISION_REQUIRED"
        ),
        "scope": "primary-only development engineering comparison",
        "results": results,
        "deltas": {
            "structured_minus_frozen_full_macro_f1": (
                structured_macro["mean"] - full_macro["mean"]
            ),
            "structured_minus_history_macro_f1": (
                structured_macro["mean"] - history_macro["mean"]
            ),
            "structured_minus_frozen_full_false_ready": (
                structured_false["mean"]
                - results["frozen_full_ree"]["false_ready_rate"]["mean"]
            ),
        },
        "predeclared_success_gates": gates,
        "next_step": (
            "apply the frozen structured objective to augmented training"
            if solved else "enter versioned relational candidate-set revision"
        ),
        "sources": {
            "protocol_sha256": sha256_file(PROTOCOL),
            "manifest_sha256": sha256_file(MANIFEST),
            "baseline_aggregate_sha256": sha256_file(BASELINE_AGGREGATE),
        },
        "uses_secondary_expansion": False,
        "gold_payload_read": False,
        "paper_result": False,
