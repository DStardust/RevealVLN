#!/usr/bin/env python3
"""Evaluate MF3W lower-tail candidate on the sealed RxR val_seen selection."""
...
WORKER = ROOT / "scripts/rxr_uad_mf3w_val_seen_worker.py"
FREEZE = ROOT / "artifacts/evaluation/mf3v_horizon_freeze_v1/MF3V_VAL_SEEN_FREEZE.json"
DIAGNOSTIC = ROOT / "artifacts/evaluation/mf3v_lower_tail_shadow_diagnostic_v1/MF3V_LOWER_TAIL_SHADOW_DIAGNOSTIC.json"
DATASET = ROOT / "third_party/ETP-R1/data/datasets/RxR_VLNCE_v0_enc_xlmr/val_seen/val_seen_guide.json.gz"
OUT = ROOT / "artifacts/evaluation/mf3w_uad_rxr_val_seen_v1"
PROTOCOL = OUT / "MF3W_RXR_VAL_SEEN_PROTOCOL.json"
PROGRESS = OUT / "MF3W_RXR_VAL_SEEN_PROGRESS.json"
RESULT = OUT / "MF3W_RXR_VAL_SEEN_RESULT.json"
...
def protocol_value() -> dict:
    ...
    if freeze.get("status") != "MF3V_VAL_SEEN_FROZEN":
        raise RuntimeError("MF3V freeze missing")
    if diagnostic.get("status") != "TRAIN_ONLY_CANDIDATE_PASS":
        raise RuntimeError("MF3W train-only candidate gate missing")
    rows = selection()
    return {
        "schema_version": "revealnav-mf3w-rxr-val-seen-protocol/1",
        "status": "SEALED_BEFORE_MF3W_RXR_VAL_SEEN_TASK_METRICS",
        "scope": "RxR English val_seen scene-stratified MF3W candidate evaluation",
        "selection": rows,
        "runs": len(rows) * 3,
        "revision": "mf3w",
        "candidate": "MF3V lower-tail-only gate; frozen lower threshold; upper cap disabled",
        "threshold_tuned_on_val_seen": False,
        "public_unseen_accessed": False,
        "sources": {
            "freeze": sha256_file(FREEZE),
            "train_only_diagnostic": sha256_file(DIAGNOSTIC),
...
def seal() -> int:
    ...
    if PROTOCOL.exists() and json.loads(PROTOCOL.read_text()) != value:
        raise RuntimeError("MF3W protocol drift")
    if not PROTOCOL.exists():
        atomic_json(PROTOCOL, value)
    print(json.dumps({"status": value["status"], "runs": value["runs"]}, indent=2))
    return 0


def execute(preflight: bool, gpus: tuple[int, ...], workers_per_gpu: int) -> int:
    protocol = protocol_value()
    if json.loads(PROTOCOL.read_text()) != protocol:
        raise RuntimeError("MF3W protocol is not sealed")
    ...
    queue = list(planned); active = []; completed = []; started = time.time()
    while queue or active:
        while queue and slots:
            mode, row = queue.pop(0); gpu = slots.pop(0)
            name = f"{mode}_ep_{row['episode_id']}"; run_dir = root / "runs" / name
...
def verify(preflight: bool) -> int:
    ...
    for mode, row in jobs(rows):
        ...
        if value.get("status") != "PASS" or value.get("split") != "val_seen" or value.get("public_unseen_accessed") is not False or not isinstance(value.get("metrics"), dict):
            raise RuntimeError("MF3W worker boundary or metric failure")
        if not all(math.isfinite(float(value["metrics"][metric])) for metric in METRICS):
            raise RuntimeError("MF3W non-finite metric")
        if mode != "baseline" and value["executed_action_validation"]["all_equal"] is not True:
            raise RuntimeError("MF3W action mismatch")
        summaries[(mode, row["episode_id"])] = value
    if preflight:
        atomic_json(OUT / "MF3W_RXR_VAL_SEEN_PREFLIGHT.json", {"status": "PREFLIGHT_PASS", "runs": len(summaries)})
        return 0
    per_scene = []
    for row in rows:
    ...
    for _ in range(10000):
        ...
        samples.append(sum(x["utility"] for x in sample) / len(sample))
    aggregate = {metric: {"mean": sum(row[metric] for row in per_scene) / len(per_scene)} for metric in (*METRICS, "utility")}
    aggregate["utility"]["scene_bootstrap_95pct"] = [percentile(samples, 0.025), percentile(samples, 0.975)]
    gates = {
        ...
        "learned_utility_exceeds_uncertainty": sum(x["learned_minus_uncertainty_utility"] for x in per_scene) / len(per_scene) > 0,
    }
    passed = all(gates.values())
    atomic_json(RESULT, {"schema_version": "revealnav-mf3w-rxr-val-seen-result/1", "status": "TASK_METRIC_GATE_PASS" if passed else "TASK_METRIC_GATE_FAIL", "aggregate_ensemble_minus_baseline": aggregate, "per_scene": per_scene, "gates": gates, "public_unseen_authorized": False, "promote_to_unseen": passed})
...
def main() -> int:
    ...
    sub.add_parser("seal")
    run_parser = sub.add_parser("execute"); run_parser.add_argument("--preflight", action="store_true"); run_parser.add_argument("--gpus", default="0,1,2,3,4,5,6,7"); run_parser.add_argument("--workers-per-gpu", type=int, default=2)
    verify_parser = sub.add_parser("verify"); verify_parser.add_argument("--preflight", action="store_true")
