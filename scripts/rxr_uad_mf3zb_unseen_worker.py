#!/usr/bin/env python3
"""Run one frozen MF3 episode on an authorized RxR val_unseen holdout."""
...
def main() -> int:
    ...
    if args.mode == "uncertainty":
        base._CONTROLLER = base.UncertaintyOnlyController(controller_trace, GATE)
    elif args.mode == "ensemble":
        base._CONTROLLER = base.MF3KTop2Controller(
            torch.device("cuda:0"), controller_trace, revision=REVISION
        )
    ...
    sys.argv = [
        "run.py", "--exp_name", f"{REVISION}_{args.mode}_{args.episode_id}",
    ...
    summary = {
        "schema_version": f"revealnav-{REVISION}-rxr-unseen-worker/1",
        "status": "RUNNING",
        ...
        "method_scope": f"frozen_{REVISION}_fresh_unseen_holdout",
    }
