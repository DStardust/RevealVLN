#!/usr/bin/env python3
"""Run one MF3ZI episode with the MF3ZG-first arbitration controller."""
...
from rxr_uad_mf3zi_controller import MF3ZIController  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--episode-id", required=True)
    parser.add_argument("--mode", choices=("baseline", "uncertainty", "ensemble"), default="ensemble")
    parser.add_argument("--split", choices=("train", "val_seen", "val_unseen"), default="val_unseen")
    parser.add_argument("--run-dir", type=Path, required=True)
    args = parser.parse_args()
    run_dir = args.run_dir.resolve()
    if ROOT not in run_dir.parents or run_dir.exists():
        raise SystemExit("MF3ZI run directory must be new and project-local")
    ...
    if args.mode == "ensemble":
        base._CONTROLLER = MF3ZIController(
    ...
    argv = [
        "run.py", "--exp_name", f"mf3zi_{args.mode}_{args.episode_id}",
    ...
    summary = {
        "schema_version": "revealnav-mf3zi-rxr-worker/1",
        "status": "RUNNING", "episode_id": args.episode_id,
        "mode": args.mode, "revision": "mf3zi", "split": args.split,
        ...
        "method_scope": "mf3zg_primary_gated_one_shot_uncertainty_residual",
        **base.MF3B_SCOPE,
    }
    sys.argv = argv
    ...
    try:
        ...
        summary["controller"] = None if base._CONTROLLER is None else {
            ...
            "authorized": sum(bool(row["authorized"]) for row in base._CONTROLLER.records),
            "actions_changed": sum(bool(row["action_changed"]) for row in base._CONTROLLER.records),
            "decision_sources": {
                source: sum(row.get("decision_source") == source for row in base._CONTROLLER.records)
                for source in ("learned_residual", "uncertainty_gate")
            },
